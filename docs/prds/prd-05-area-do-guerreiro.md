...
| `ProducaoDaMissao`     | equipe **ou** Guerreiro(a) — exatamente um —, missão, atividade, forma de entrega (texto, áudio ou foto), transcrição, devolutiva, momento — **sem a foto e sem o áudio, descartados na leitura**. Nasce inteira na fatia 9 do PRD-04, pela entrega da equipe no encontro; este PRD só acrescenta a porta individual (`POST /v1/eu/missoes/{id}/producao`) sobre a entidade já de pé, sem migração nova (documento 09, "A fatia 9 do PRD-04 entrega a `ProducaoDaMissao` inteira"). |
...
| Sem teto de uso no Ciclo 01, absorvido pelo fundador em _pay-as-you-go_                         | 03 §7 e 10 §4.3 | Corpus, cota e áudio dos assistentes                                                                              |
