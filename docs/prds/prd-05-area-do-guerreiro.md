...
| `RF-05-58` | Guerreiro(a) consulta o pedido de ajuda para atividades escolares a partir desta aplicação | desejável  |
...
PRD **acrescenta uma entidade** ao núcleo — `RespostaDeQuiz` — e escreve em outras quatro que
já existem.
...
Avatar (características)                Ponto / Nivel / Badge     (PRD-01)
                                        RecompensaDeMarco         (PRD-09)
...
pontos a partir do jogo (404, por não existir).
...
- **Rede instável**: a entrada exige rede, porque a conferência da imagem acontece no núcleo; o
  conteúdo já carregado do ponto de trilha continua legível durante uma queda.
...
A entidade `RespostaDeQuiz` foi acrescentada ao modelo do PRD-01, e `Nivel` passou a ser
derivado do percurso. O badge **de protagonismo** entrou no catálogo do documento 11 §7.

## 14. Pendências que permanecem

- **Quem atende o pedido de ajuda para atividades escolares**: o assistente por voz da App 02
  (PRD-06) ou uma fila para os Mestres responderem na App 09. **Trava** o `RF-05-58`, que por
  isso é desejável e não essencial.
...
- **Retenção do áudio da sugestão** depois de transcrito: apagar na transcrição, como se faz
  com a fotografia do onboarding, ou guardar por prazo declarado.
...
| `RF-05-58`              | 08 §PRD-05 (pedido de ajuda para atividades escolares)               |
