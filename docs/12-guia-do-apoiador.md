# 12 — Guia do Apoiador

> Resumo prático da relação do Apoiador com o Comunidade Game: o que é o projeto, por que
> apoiar, o que o Ciclo 01 precisa, como apoiar e como funcionam os desafios extras. É um
> documento **derivado** — as regras completas estão nos documentos 02, 04, 05 e 10.

## 1. Linhas gerais do projeto

O **Comunidade Game** é uma plataforma educacional gamificada, de código aberto, que conecta
jovens, mestres e apoiadores para promover educação, tecnologia e cidadania em comunidades
periféricas. Em uma frase: um "jogo" cujas partidas acontecem na vida real — aprender, criar,
ajudar os colegas e registrar dados do próprio território gera pontos, poderes e reconhecimento.

- **Público:** crianças e jovens de 6 a 16 anos, moradores das comunidades atendidas.
- **Primeira implantação:** Case 01 — Comunidade Guerreira Zeferina, Salvador (BA), **Ciclo 01
  de agosto a dezembro de 2026**.
- **Valores inegociáveis:** humanismo, educação transformadora, causa antirracista, combate à
  violência contra a mulher, valorização dos povos originários, território e identidade,
  letramento sobre a crise ambiental e protagonismo dos jogadores.
- **Transparência sobre IA:** os artefatos da plataforma são construídos com auxílio e
  potencialização de inteligência artificial, sob idealização e direção humanas — declaração
  pública do projeto.

## 2. Por que apoiar

- **O ciclo de evolução positiva**: o que a plataforma produz não sai da comunidade — volta para
  ela. O jogador aprende e realiza; a comunidade ganha dados, talento e visibilidade; o apoio
  custeia novas atividades; quem chega ao topo volta como multiplicador.
- **Transparência radical**: todo recurso aportado é registrado em livro-razão público e compõe
  o **Poder Econômico** do Apoiador — reconhecimento visível de quem sustenta o projeto.
- **Efetividade rastreável**: o histórico registra não só _quanto_ foi aportado, mas **o que
  aconteceu por causa do aporte** — as realizações dos jogadores nos desafios propostos.
- **Protagonismo dos jogadores**: o apoio não financia assistência, financia **autoria** —
  criações originais com crédito público na vitrine e jogadores que participam da evolução da
  própria plataforma.
- **Precedente concreto**: o **Goethe-Institut (Salvador)** é um dos primeiros Apoiadores —
  doou 298 livros e 30 kits MDF, com Termo de Doação assinado, dando lastro material às duas
  primeiras trilhas.

## 3. Estrutura necessária para o Ciclo 01

Pela regra de lastro, **nenhuma atividade acontece sem os recursos providos** — e a hipótese
**H3** do ciclo mede exatamente se mestres e apoiadores conseguem supri-los.

| Frente                     | Recursos                                                                                                       |
| -------------------------- | -------------------------------------------------------------------------------------------------------------- |
| **Ponto de apoio**         | Espaço físico na comunidade (hackerspace, fab lab, sala cedida) com guarda do acervo                           |
| **Equipamentos**           | Notebooks, smartphones e tablets de uso compartilhado — viabilizam a dinâmica assíncrona dos encontros         |
| **Operação dos encontros** | Hora-aula dos mentores, lanche dos jogadores, insumos das oficinas (LEDs, baterias, papel, kit de encapar)     |
| **Recompensas**            | Catálogo de troca de pontos (kits de alimentos e itens não alimentares)                                        |
| **Infraestrutura digital** | Servidores, armazenamento e execução das aplicações                                                            |
| **Reposições**             | Acervo e materiais — perda ou dano nunca gera cobrança à família; vira necessidade de reposição a ser aportada |

## 4. Como apoiar

1. **Cadastro por um Admin.** Todo Apoiador — pessoa física ou jurídica — é cadastrado
   exclusivamente pelos Admins, com o apoio **comprovado por materiais ou artefatos registrados
   na plataforma** (termo de doação, comprovante, registro do material).
2. **Doações em dinheiro** — por **PIX**, no canal oficial: chave `51.730.395/0001-19` (CNPJ),
   titular **Robô Educa — Kits Robóticos Educacionais**, pessoa jurídica vinculada ao projeto.
3. **Doações de materiais e serviços** — equipamentos, insumos, acervo, hora de trabalho: tudo
   entra no livro-razão com o mesmo rigor do dinheiro.
4. **Todo aporte compõe o Poder Econômico** do Apoiador, visível publicamente na vitrine.

## 5. Desafios extras

Aportar é o começo da relação; a interação com os jogadores acontece pelos **desafios extras**:

- O Apoiador propõe um desafio **vinculado a uma trilha em andamento** e custeia a recompensa,
  em quantidade declarada (uma única, para quem concluir primeiro, ou várias, até esgotar).
- O desafio passa por **validação pedagógica do Mestre da trilha** e **aprovação de um Admin**
  antes de ir ao ar; a recompensa precisa estar **provida antes da publicação**.
- Quem conclui recebe a recompensa e **pontos extras**, computados isoladamente.
- **Duas modalidades:** **aberto** (a todos os jogadores da trilha, ninguém é barrado de
  disputar) e **direcionado** (a um jogador específico, único que pode receber a recompensa,
  mediante justificativa registrada do vínculo — por exemplo, um parente próximo — e aprovação
  de Admin).
- **Salvaguarda inegociável:** **nenhum contato direto entre Apoiador e jogador**. Proposta,
  entrega e reconhecimento são sempre mediados pela plataforma, e o relatório de efetividade é
  agregado e por avatar.

## 6. O que o Apoiador acompanha

- **Seu card público** na vitrine: identidade, Poder Econômico, desafios propostos e o que o
  apoio produziu.
- **Relatório de efetividade** dos seus desafios — quantos concluíram, em que trilhas, com que
  retenção (formato ainda em definição).
- **Prestação de contas** dos recursos: o primeiro relatório real será o do acervo do
  Goethe-Institut, ao fim do Ciclo 01.
