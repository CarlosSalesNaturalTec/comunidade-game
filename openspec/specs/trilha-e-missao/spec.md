...
### Requirement: A trilha tem situação de rascunho, publicada ou despublicada
...
trilha em rascunho SHALL ser visível apenas ao Mestre autor e a Admin, e NEVER SHALL aparecer em
consulta pública. A trilha **despublicada** SHALL ser editável pelo Mestre autor como se fosse
rascunho, sem que a situação dela vire `rascunho`, e NEVER SHALL aparecer em consulta pública. O
fluxo que muda a situação — publicação, travas e despublicação — é do PRD-09; aqui nasce a
situação. (`RF-01-20`, `RF-09-04`, `RF-09-11`, PRD-01 §8, PRD-09 §8)
...
nem de outro Mestre, conferindo antes de qualquer escrita: **missão de sondagem** declarada, ao
menos **um desafio de coleta de dados reais** em alguma missão da trilha, e **culminância**
...
marco, registro de coleta e auditoria ficam **todos** com a trilha de origem. A trilha de
...
#### Scenario: A cópia traz as perguntas do quiz de cada missão

- **WHEN** um Mestre duplica uma trilha cujas missões têm quiz de desbloqueio com várias
  perguntas
...
- **WHEN** a trilha de origem tem inscritos, desbloqueios, resultados e recompensas de marco
  declaradas
- **THEN** a trilha nova nasce sem inscrição, sem desbloqueio, sem submissão de desbloqueio, sem
  resultado e sem recompensa de marco alguma
