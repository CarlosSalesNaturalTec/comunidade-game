...
- [ ] 1.1 Acrescentar `--largura-de-miniatura: 320px` e `--altura-de-miniatura: 240px` a
      `comum/tokens.css`, na camada semântica, e verificar que nenhum token existente muda.
- [ ] 1.2 Criar `MidiaDoNucleo` em `comum/react`: recebe a função que busca os bytes, o `tipo`
...
- [ ] 1.3 Testar `MidiaDoNucleo`: imagem exibida com a classe/estilo da moldura fixa, vídeo com
...
- [ ] 2.1 Em `PreVisualizacaoDaMissao.tsx`, trocar `ConteudoPreVisualizado` por `MidiaDoNucleo`
      para imagem e vídeo do conteúdo da missão (`RF-09-25`).
- [ ] 2.2 Na mesma tela, exibir a imagem da pergunta do desafio de desbloqueio/sondagem via
      `MidiaDoNucleo`, chamando `lerImagemDaPergunta` para cada pergunta que tiver
      `imagem_referencia` (`RF-09-25`, `RF-09-119`).
- [ ] 2.3 Em `DesafioDeDesbloqueio.tsx` (App 09), trocar `ImagemDaPergunta` por `MidiaDoNucleo`
      para a imagem anexada na edição (`RF-09-119`).
- [ ] 2.4 Testar em `trilhas.test.tsx`: a pré-visualização exibe conteúdo e imagem da pergunta
      ...
      moldura"). `vitest run` do arquivo passa.

## 3. App 05 — telas reais do Guerreiro(a)

- [ ] 3.1 Em `Missao.tsx`, trocar `ConteudoDaMissao` por `MidiaDoNucleo` para imagem e vídeo do
      conteúdo (`RF-05-11`).
- [ ] 3.2 Em `DesafioDeDesbloqueio.tsx` (App 05), trocar `ImagemDaPergunta` por `MidiaDoNucleo`
      para a imagem da pergunta (`RF-05-11`, `RF-09-119`).
- [ ] 3.3 Testar em `Missao.test.tsx` e `DesafioDeDesbloqueio.test.tsx`: conteúdo e imagem da
...
- [ ] 4.1 Acrescentar em `docs/15-identidade-visual.md` §12 a nota dos dois tokens novos
      (`--largura-de-miniatura`, `--altura-de-miniatura`).
- [ ] 4.2 Marcar a linha "Moldura fixa de mídia na missão" como "implementado" em
