...
- [x] 1.1 Criar `backend/src/nucleo/assistente/modelo.py` com `ConsultaAoAssistente` — `equipe_id`
      ...
      decisão 6).
- [x] 1.2 Acrescentar a operação `consulta_ao_assistente` à matriz de `permissoes.py`, no
...
- [x] 2.1 Criar `porta.py`, `local.py`, `nuvem.py` e `fabrica.py` do assistente no padrão de
...
- [x] 2.2 Implementar em `regra.py` a montagem do corpus — equipe → atividade corrente → missão →
...
- [x] 2.3 Implementar em `regra.py` a gravação da consulta e os três desfechos — respondida,
...
- [x] 3.1 Criar `POST /v1/assistente/trilhas/consultas` em `rotas.py`, em `multipart/form-data`
...
- [x] 3.2 Registrar o roteador em `principal.py` e conferir que a rota aparece no OpenAPI com a
      chave de aplicação exigida, como as demais rotas de dados sob `/v1` (`RF-01-02`).

## 4. App 01 — tela do assistente

- [x] 4.1 Criar `src/api/assistente.ts` e a tela `src/trilhas/TelaDoAssistente.tsx`, alcançável da
      ...
      `RF-04-28`, `RN-04-20`).
- [x] 4.2 Apresentar recusa, encaminhamento à App 05 e indisponibilidade como resposta em tela —
      nunca como erro — e bloquear a pergunta sem rede sem enfileirar nada (`RF-04-37`,
      `RF-04-38`, `RF-04-58`).

## 5. App 01 — rede fora e fila local

- [x] 5.1 Elevar o estado "sem rede" a `sessao-de-trabalho/AparelhoDaAula.tsx`, alimentado pela
      ...
      decisão 9).
- [x] 5.2 Criar `src/fila/filaDePresenca.ts` — `{ aula_id, nick, momento_do_fato }` em
...
- [x] 5.3 Implementar a sincronização automática na volta da rede, refazendo por item a sequência
      ...
      — decisão 7).
- [x] 5.4 Bloquear, sem rede, o caminho do onboarding e a entrada por reconhecimento, com aviso
      na tela dizendo por quê, e encaminhar à confirmação pelo Mestre ou Admin (`RF-04-24`,
      `RN-04-09`, `RN-04-12`).
- [x] 5.5 Apresentar ao Mestre ou Admin da sessão de trabalho a lista do que está na fila e do
...
- [x] 6.1 Criar `src/direitos/AreaDetalhadaDeDireitos.tsx` com o conteúdo do PRD-04 §11 em
...
- [x] 6.2 Acrescentar o aviso discreto de coleta à `inicio/TelaInicial.tsx` e à
      `onboarding/TelaDeCaptura.tsx`, com o caminho para a área detalhada (`RF-04-26`).
- [x] 6.3 Encerrar o `onboarding/FluxoDeOnboarding.tsx` com a despedida que diz como entrar da
...
- [x] 7.1 `src/trilhas/assistente.test.tsx` — os cenários do assistente: as duas formas de
...
- [x] 7.2 `src/fila/fila.test.tsx` — os cenários da fila: presença enfileirada com a rede fora,
...
- [x] 7.3 `src/direitos/direitos.test.tsx` e complemento de `src/onboarding/fluxo.test.tsx` — os
...
- [x] 7.4 Complemento de `src/inicio/inicio.test.tsx` e `src/onboarding/onboarding.test.tsx` — o
...
- [x] 8.1 Marcar as fatias 10, 11 e 12 do PRD-04 como implementadas em
