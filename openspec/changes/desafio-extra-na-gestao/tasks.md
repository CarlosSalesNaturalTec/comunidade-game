...
- [ ] 1.1 Em `docs/04-modelo-economico-e-sustentabilidade.md` §3, gravar na tabela de definições
...
- [ ] 1.2 Em `docs/09-topicos-em-aberto-e-sugestoes.md` §1, acrescentar a linha da decisão em
      "Já decididos", apontando o documento 04 §3 como fonte
- [ ] 1.3 Em `docs/prds/prd-02-frontend-de-gestao.md`, acrescentar `RF-02-106` (o Admin encerra o
...
- [ ] 2.1 Em `backend/src/nucleo/reservas/modelo.py`, tornar `aula_id` opcional, acrescentar
...
- [ ] 2.2 Em `backend/src/nucleo/reservas/regra.py`, acrescentar `reservar_recompensa_do_desafio`
...
- [ ] 2.3 Criar a migração Alembic que afrouxa `reserva.aula_id`, cria `reserva.desafio_extra_id`
      com a FK, o `CHECK` de exclusividade e o índice

## 3. Fila, aprovação, recusa e encerramento no núcleo

- [ ] 3.1 Em `backend/src/nucleo/desafios_extras/modelo.py`, acrescentar `encerrado_em` e
      `admin_encerrador_id` ao `DesafioExtra`, e criar a migração Alembic das duas colunas
      (`RF-02-106`, `RF-07-40`, design — decisão 1)
- [ ] 3.2 Em `backend/src/nucleo/desafios_extras/regra.py`, acrescentar
...
- [ ] 3.3 Em `backend/src/nucleo/desafios_extras/regra.py`, acrescentar `aprovar_desafio_extra`,
...
- [ ] 3.4 Em `backend/src/nucleo/desafios_extras/regra.py`, acrescentar `recusar_desafio_extra`,
...
- [ ] 3.5 Em `backend/src/nucleo/desafios_extras/regra.py`, acrescentar `encerrar_desafio_extra`
      ...
      decisões 1 e 8)
- [ ] 3.6 Em `backend/src/nucleo/desafios_extras/rotas.py`, acrescentar as quatro rotas sob
...
- [ ] 4.1 Em `backend/tests/test_desafio_extra_regra.py`, cobrir a aprovação e a recusa: desafio
...
- [ ] 4.2 Em `backend/tests/test_desafio_extra_regra.py`, cobrir a reserva e o encerramento: a
...
- [ ] 4.3 Em `backend/tests/test_conclusao_de_desafio_extra.py`, acrescentar o cenário da
      conclusão recusada em desafio encerrado (`RF-07-40`, `RF-14-42`)
- [ ] 4.4 Em `backend/tests/test_desafio_extra_rota.py`, cobrir as quatro rotas: a fila só traz
...
- [ ] 5.1 Em `apps/app-03-gestao/src/filas/api.ts`, acrescentar `listarDesafiosExtrasPendentes`,
...
- [ ] 5.2 Em `apps/app-03-gestao/src/filas/TelaDeFilas.tsx`, acrescentar a natureza "Desafios
...
- [ ] 5.3 Criar `apps/app-03-gestao/src/filas/AvaliacaoDoDesafioExtra.tsx` com o que a proposta
...
- [ ] 5.4 Na mesma tela, acrescentar a lista dos publicados com a quantidade restante e a
      ...
      `RF-14-38`)
- [ ] 5.5 Em `apps/app-03-gestao/src/filas/filas.test.tsx`, cobrir a natureza nova: o filtro
...
- [ ] 6.1 Marcar a fatia 15 do PRD-02 como implementada no
