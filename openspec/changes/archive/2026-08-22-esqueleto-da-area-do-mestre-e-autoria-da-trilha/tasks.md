...
suíte do backend e o Biome rodam **uma vez**, ao fechar o grupo. O grupo 7 toca `docs/`, e por
isso é o único que exige `npm run lint` e `mkdocs build --strict`.

## 1. Modelo e migração (PRD-09 §8)

- [x] 1.1 Acrescentar a `Missao` as colunas `titulo`, `etapa_do_ciclo` e `cadencia_de_retomada`
...
- [x] 1.2 Acrescentar a `Atividade` as colunas `titulo` (obrigatório) e `descricao` (anulável)
      em `trilhas/modelo.py` (`RF-09-69`, decisão 5). Verificar:
      `uv run pytest tests/test_atividade.py -x` verde
- [x] 1.3 Estender `criar_missao` e `criar_atividade` em `trilhas/regra.py` para receber e
...
- [x] 1.4 Escrever a função `declarar_cadencia_de_retomada` em `trilhas/regra.py`, com a posse
...
- [x] 1.5 Gerar a migração Alembic das cinco colunas em duas etapas — coluna anulável,
...
- [x] 1.6 Conferir que os 31 arquivos de teste que chamam `criar_trilha`, `criar_missao` e
...
- [x] 2.1 Criar `backend/src/nucleo/trilhas/rotas.py` com `POST /trilhas`, traduzindo o corpo
...
- [x] 2.2 Acrescentar `GET /trilhas/minhas`, devolvendo as trilhas do Mestre autor com missões
...
- [x] 2.3 Acrescentar `POST /trilhas/{id}/missoes` (`RF-09-02`, `RF-09-03`, `RF-09-80`,
...
- [x] 2.4 Acrescentar `POST /missoes/{id}/atividades` (`RF-09-69`, `RF-09-70`). Verificar:
...
- [x] 2.5 Acrescentar `POST /missoes/{id}/retomada` (`RF-09-83`, `RF-09-101`, decisão 4).
      Verificar: `tests/test_trilha_rota.py` cobrindo declarar, substituir e Mestre não autor
- [x] 2.6 Registrar o roteador em `principal.py` e conferir que as três rotas órfãs passam a
...
- [x] 2.7 Fechar o backend: `ruff format .`, `ruff check --fix .` e `uv run pytest` — uma vez

## 3. Camada de acesso em `comum/` (decisão 6)

- [x] 3.1 Criar `comum/api/` com `cliente.ts` e `tipos.ts` movidos da App 03, mantendo a
...
- [x] 3.2 Criar `comum/autenticacao/` com `ContextoDeSessao`, `armazenamentoDeSessao` e
...
- [x] 3.3 Apontar as 61 chamadas dos 11 `api.ts` da App 03 para `comum/api`, e chamar
      ...
      comportamento
- [x] 3.4 Conferir que a App 03 continua construindo. Verificar: `npm run build -w
      app-03-gestao` sem erro de tipo

## 4. App 09 — esqueleto e entrada (PRD-09 §4)

- [x] 4.1 Criar `apps/app-09-mestre/` espelhando a pilha da App 03 — Vite, React 19,
...
- [x] 4.2 Montar a entrada por login social consumindo `comum/autenticacao`, com a chave e a
...
- [x] 4.3 Recusar Guerreiro(a) na entrada, em linguagem simples, e apresentar a orientação de
...
- [x] 4.4 Devolver o Mestre à entrada quando o núcleo recusa a sessão, distinguindo essa recusa
...
- [x] 5.1 Tela de criação de trilha, com seletor que oferece só poder ativo e de natureza de
...
- [x] 5.2 Lista das trilhas do Mestre, com nome, poder, área e situação, alimentada por
      `GET /trilhas/minhas` (`RF-09-04`). Verificar: teste cobrindo rascunho próprio presente e
      rascunho alheio ausente
- [x] 5.3 Tela de missões da trilha — acrescentar com título, posição, dificuldade,
...
- [x] 5.4 Marcação da missão de sondagem, com a recusa da segunda sondagem e da sondagem fora
      da primeira posição em linguagem simples; rascunho sem sondagem é aceito (`RF-09-81`).
      Verificar: teste cobrindo os três casos
- [x] 5.5 Tela de atividades da missão, com título, descrição, modalidade, formato, natureza e
...
- [x] 5.6 Declaração da cadência de retomada da missão, com o caminho de deixá-la sem retomada
      (`RF-09-83`). Verificar: teste cobrindo declarar e deixar sem retomada
- [x] 5.7 Conferir que nenhum campo da autoria pede código, marcação ou configuração técnica, e
...
- [x] 6.1 Acrescentar o alvo `mestre` ao `.firebaserc` — o `firebase.json` já o declara.
      Verificar: `firebase target:list` reconhece o alvo
- [x] 6.2 Criar `.github/workflows/app-09-deploy.yml`, espelho do `app-03-deploy.yml`, filtrado
...
- [x] 6.3 Fechar o frontend: `npx biome format --write .`, `npx biome check .` e
      `npm test` nas duas aplicações — uma vez

## 7. Documentação da change (`CLAUDE.md` §3)

- [x] 7.1 Atualizar `docs/prds/index.md` com a primeira fatia do PRD-09 — o que entrou, as três
...
- [x] 7.2 Registrar no documento 09 a decisão nova do fundador — o formato da cadência de
...
- [x] 7.3 Conferir o documento 99 §8 contra a App 09 e a relação entre PRD-09 e PRD-02.
      Nenhuma edição foi necessária: a App 09 já estava mapeada ao PRD-09 desde antes desta
      fatia, e não nasceu dependência nova — o `RF-02-71` ficou pendência, não rota entregue.
      Verificado: os invariantes do §6 seguem válidos
- [x] 7.4 Rodar a esteira de documentação, que só agora é exigida porque o grupo 7 tocou
