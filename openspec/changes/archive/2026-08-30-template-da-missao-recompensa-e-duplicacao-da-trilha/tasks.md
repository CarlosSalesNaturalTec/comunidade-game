...
- [x] 1.1 Gravar a decisão do poder técnico no documento-fonte e na pauta: uma linha nas
...
- [x] 1.2 Aplicar a decisão no PRD-01: o atributo no `Poder` da §8 e a menção no `RN-01-54` ou
...
- [x] 1.3 Corrigir a linha de `SugestaoDeEstrutura` no PRD-09 §8, retirando o atributo "custo de
...
- [x] 2.1 Acrescentar a coluna `tecnico` ao `Poder` (padrão falso, não nula), aceitá-la no
...
- [x] 2.2 Escrever a migração Alembic desta change — `poder.tecnico` e a tabela
...
- [x] 2.3 Apresentar a marca no formulário e na lista de poderes da App 03, em linguagem simples
...
- [x] 3.1 Criar o módulo `template_de_missao` com o modelo `SugestaoDeEstrutura` — missão,
...
- [x] 3.2 Escrever a porta, a fábrica e os dois adaptadores no padrão de `armazenamento/`: o
...
- [x] 3.3 Implementar o cálculo das lacunas no núcleo — missão sem atividade, atividade com
      ...
      vazia.
- [x] 3.4 Implementar a regra do pedido de estrutura: só o Mestre autor (403), tópico
...
- [x] 3.5 Publicar `POST /v1/missoes/{id}/estrutura`, devolvendo estrutura e lacunas, e
      ...
      modelo indisponível.
- [x] 3.6 Registrar o desfecho da sugestão — aceita, recusada ou alterada — sem que ele altere a
...
- [x] 4.1 Escrever `missoes_desbloqueadas_pelo_guerreiro` em `trilhas/regra.py`, contando só o
...
- [x] 4.2 Implementar a fila de entregas pendentes do Mestre, filtrada pelo vínculo vigente com
...
- [x] 5.1 Implementar a duplicação numa transação — trilha nova em rascunho sob a autoria de quem
...
- [x] 6.1 Criar a tela do template na missão: campo de texto corrente para o tópico, estrutura
...
- [x] 6.2 Ligar aceitar, recusar e alterar cada sugestão às rotas de autoria que já existem —
...
- [x] 6.3 Criar a declaração da recompensa junto da missão — tipo e quantidade, sem preço, sem
      ...
      preço.
- [x] 6.4 Apresentar a fila de entregas pendentes em Minhas turmas, com confirmação da entrega
...
- [x] 6.5 Acrescentar a duplicação na lista de trilhas, com o aviso do que a cópia traz e do que
...
- [x] 7.1 Marcar as fatias **12** e **13** do PRD-09 como implementadas em
