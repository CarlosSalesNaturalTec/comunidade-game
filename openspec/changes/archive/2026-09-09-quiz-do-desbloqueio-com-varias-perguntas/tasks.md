...
- [x] 1.1 Em `backend/src/nucleo/trilhas/modelo.py`, criar `PerguntaDoDesbloqueio` (missão,
      ...
      mapeamento.
- [x] 1.2 Escrever a revisão do Alembic que cria as três tabelas, transporta cada quiz de
...
- [x] 2.1 `declarar_desafio_de_desbloqueio` passa a receber a lista de perguntas do quiz,
...
- [x] 2.2 `submeter_desafio_de_desbloqueio` passa a receber a resposta de todas as perguntas,
...
- [x] 2.3 Gravar `SubmissaoDoDesbloqueio` e as `RespostaDaSubmissao` em **toda** tentativa — a
      ...
      duas submissões.
- [x] 2.4 `duplicar_trilha` passa a copiar as perguntas do quiz de cada missão, na ordem da
      origem, e segue sem copiar submissão nem desbloqueio (`RF-09-13`). Verificar por
      `test_duplicacao_de_trilha.py`.

## 3. Núcleo — rotas

- [x] 3.1 `POST /v1/missoes/{id}/desbloqueio` passa a receber `perguntas`; a leitura da missão
...
- [x] 3.2 `POST /v1/eu/missoes/{id}/desbloqueio` passa a receber `respostas`
...
- [x] 4.1 Em `apps/app-09-mestre/src/trilhas/`, `api.ts` e `DesafioDeDesbloqueio.tsx` passam a
      ...
      (`RF-09-118`, `RN-09-43`).
- [x] 4.2 O resumo do bloco recolhível do desafio, em `ListaDeMissoes.tsx`, passa a dizer
      quantas perguntas o quiz tem (`RF-09-118`).

## 5. App 05 — área do Guerreiro(a)

- [x] 5.1 Em `apps/app-05-guerreiro/src/`, `api/trilha.ts` e
...
- [x] 6.1 `backend/tests/test_desbloqueio_da_missao.py` cobre os cenários da spec
...
- [x] 6.2 `backend/tests/test_percurso_da_trilha.py` e `test_duplicacao_de_trilha.py` cobrem a
      regressão do percurso com o modelo novo e a cópia das perguntas na duplicação.
- [x] 6.3 Os testes da App 09 cobrem acrescentar e remover pergunta, a recusa do quiz vazio e o
      resumo com a contagem de perguntas.
- [x] 6.4 Os testes da App 05 cobrem a submissão única com todas as respostas, a pergunta sem
      resposta sinalizada, a devolutiva com os acertos e a sondagem sem linguagem de aprovação.

## 7. Documentação

- [x] 7.1 Marcar a fatia 18 do PRD-09 como implementada em
