...
- [x] 1.1 Extrair de `_conferir_perguntas_do_quiz` a conferência de **uma** pergunta e fazer os
...
- [x] 1.2 Escrever em `trilhas/regra.py` `acrescentar_pergunta_do_desbloqueio`, com posse do
      Mestre autor, conferência de completude e inserção em `max(ordem) + 1` das vigentes
      (`RF-09-120`, `RN-09-44`, design — decisões 3 e 4).
- [x] 1.3 Escrever `corrigir_pergunta_do_desbloqueio`: sem resposta gravada, atualiza a linha no
...
- [x] 1.4 Escrever `remover_pergunta_do_desbloqueio`, que carimba a pergunta sem renumerar as
      demais e recusa com 422 a remoção da **única** pergunta do quiz (`RF-09-120`, `RN-09-43`,
      design — decisões 3 e 5).

## 2. Rotas

- [x] 2.1 Publicar em `trilhas/rotas.py` as três rotas — `POST
...
- [x] 2.2 Conferir que a rota de abertura do envio da imagem aceita pergunta criada
      isoladamente, sem exigir declaração prévia do desafio (`RF-09-119`, `RF-09-120`).
      A rota endereça a pergunta pelo id e já a aceita; a apuração acrescentou à `specs` o caso
      da **primeira** pergunta, que declara a missão como quiz, e a recusa da missão de
      desafio prático (`RF-09-26`).

## 3. Tela da App 09

- [x] 3.1 Acrescentar em `trilhas/api.ts` as três funções cliente das rotas novas, com os tipos
      de entrada e saída da pergunta (`RF-09-120`).
- [x] 3.2 Dar a cada pergunta de `DesafioDeDesbloqueio.tsx` gravação, marca de gravação e erro
...
- [x] 3.3 Fazer o anexo da imagem em pergunta ainda não gravada ser um gesto só: grava a
...
- [x] 3.4 Conferir que o bloco da **sondagem** herda os três comportamentos acima sem texto
      próprio novo, já que é o mesmo componente (`RF-09-81`, `RF-09-120`).

## 4. Testes

- [x] 4.1 Em `backend/tests/test_desbloqueio_da_missao.py`, cobrir os cenários da escrita por
...
- [x] 4.2 No mesmo arquivo, cobrir a convivência dos dois caminhos: declarar o desafio inteiro
      ...
      `RF-09-120`).
- [x] 4.3 Em `apps/app-09-mestre/src/trilhas/trilhas.test.tsx`, cobrir os cenários de tela:
...
- [x] 5.1 Gravar a decisão nova no documento-fonte — `docs/03-plataforma-e-arquitetura.md` §11,
      ...
      fundador, 2026-09-12).
- [x] 5.2 Aplicar a decisão em `docs/prds/prd-09-area-do-mestre.md`: `RF-09-120` e `RN-09-44`
...
- [x] 5.3 Marcar a fatia 20 como `implementado` em `openspec/cronograma-de-fatias.md`, com o
