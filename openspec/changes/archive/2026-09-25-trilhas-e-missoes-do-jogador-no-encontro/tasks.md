...
- [x] 0.1 Conferir que o recorte está fechado, sem pergunta pendente: **opção B** com sondagem e
...
- [x] 0.2 Conferir que os PRs de revisão do **documento 03 §3** e do **PRD-04** entraram, com o
...
- [x] 0.3 Conferir que a change do **temperamento Arena** entrou antes: o delta da tela inicial aqui
      já traz o parágrafo do glifo (design — decisão 7).

## 1. Promover os componentes de trilha para `comum/`

- [x] 1.1 Criar `comum/trilha/` com `GuiaDaTrilha`, `Missao`, `Sondagem`, `DesafioDeDesbloqueio` e
...
- [x] 1.2 Tornar opcional **cada ato de escrita separadamente** — a inscrição, a submissão do
      ...
      decisão 3).
- [x] 1.3 Exportar a pasta em `comum/package.json` e conferir o `comum/tsconfig.json`.
- [x] 1.4 Em `apps/app-05-guerreiro/src/trilha/` e `src/api/trilha.ts`, passar a consumir o que foi
      promovido, **ligando** a escrita, sem mudar comportamento algum da App 05 (`RF-05-13`,
      `RF-05-14`, `RF-05-74`).
- [x] 1.5 Rodar os testes de trilha da App 05 e confirmar que passam sem alteração de asserção — é o
      que prova que a promoção não mudou o que a App 05 faz (design — Risks).

## 2. O caminho das trilhas na App 01

- [x] 2.1 Em `apps/app-01-aula-presencial/src/api/`, acrescentar as leituras que faltam nesta
...
- [x] 2.2 Em `entrada/TelaDeEntradaDoGuerreiro.tsx`, acrescentar `trilhas` ao tipo do caminho e ao
...
- [x] 2.3 Em `inicio/TelaInicial.tsx`, acrescentar o caminho das trilhas, atrás da entrada do
      Guerreiro(a) e da `GuardaDePresenca`, como os das equipes, do quiz e da troca (`RF-04-72`,
      `RF-04-01`, `RN-04-40`).

## 3. A tela do percurso no encontro

- [x] 3.1 Em `apps/app-01-aula-presencial/src/trilhas/`, montar a tela do percurso sobre os
      ...
      decisão 6).
- [x] 3.2 Apresentar, junto do percurso, as atividades das equipes do Guerreiro(a) na aula em curso,
...
- [x] 3.3 Ligar a **inscrição** e o **desbloqueio** nos componentes promovidos — o segundo traz a
...
- [x] 3.4 Conferir que feita a inscrição o percurso abre na sondagem no mesmo atendimento, que
...
- [x] 3.5 Tratar a ausência de rede como os demais caminhos que pedem o Guerreiro(a), sem
      enfileirar nada — nem resposta de sondagem, nem submissão de desbloqueio (`RF-04-58`,
      `RF-04-68`).

## 4. Testes

- [x] 4.1 Em teste do `comum`, cobrir os componentes promovidos com a escrita ligada e desligada —
      é o contrato novo da decisão 3.
- [x] 4.2 Em `apps/app-01-aula-presencial/src/trilhas/trilhas.test.tsx`, cobrir os catorze cenários
...
- [x] 4.3 Em `entrada/entrada.test.tsx`, cobrir "O desfecho da presença oferece as trilhas" e o
      título próprio do caminho das trilhas nas duas formas da entrada (`RF-04-67`, `RF-04-72`).
- [x] 4.4 Em `inicio/inicio.test.tsx`, cobrir "A tela inicial leva às trilhas de quem já tem
...
- [x] 5.1 Marcar a fatia 21 como implementada em `openspec/cronograma-de-fatias.md`.
- [x] 5.2 Acrescentar ao **documento 03 §1.2** a linha que descreve `comum/trilha/`, porque a
      estrutura de pastas é definida ali (design — decisão 1).
- [x] 5.3 Conferir que a decisão nova do fundador está gravada no documento 09 §1 e no documento 03
