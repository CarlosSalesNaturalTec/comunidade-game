...
- [x] 1.1 Migração Alembic e modelo: `persona.pin_verificador` (JSON, nulo) e
...
- [x] 1.2 Módulo `pin_de_confirmacao` (regra e rotas): derivação PBKDF2-HMAC-SHA256 com sal e
      iterações no verificador, `PUT /v1/eu/pin-de-confirmacao` (4 dígitos, só Mestre e
      Admin, pela operação de confirmação da matriz) e `GET /v1/eu/pin-de-confirmacao/verificador`
      (só a chave do App 01, erro `pin_nao_cadastrado` sem PIN); `GET /v1/eu` ganha `tem_pin_de_confirmacao` —
      `RF-01-75`, `RN-04-38`. Verificar: testes da 4.1.
- [x] 1.3 Conferência do PIN e bloqueio: erros `pin_recusado`, `pin_bloqueado` e
...
- [x] 1.4 `POST /v1/sessoes/guerreiro/confirmacao`: com a chave do App 01 e persona Mestre ou
...
- [x] 1.5 `POST /v1/aulas/{id}/presencas/sem-rede`: só a chave do App 01 e o Mestre ou Admin
...
- [x] 1.6 Painel do dia: o campo `aguardando_aparelho` vira `sem_equipe` em
...
- [x] 2.1 Sessão de trabalho: ao abrir com rede, busca o verificador e o guarda em
...
- [x] 2.2 `TelaDeEntradaDoGuerreiro`: a tela de "Chamar Mestre ou Admin" pede nick e PIN
      ...
      testes da 4.4.
- [x] 2.3 Sem rede: confere o PIN no aparelho com `SubtleCrypto` contra o verificador; só
...
- [x] 2.4 `fila/sincronizacao.ts`: sincroniza cada item pela rota `presencas/sem-rede`, sem
...
- [x] 3.1 App 09, área do perfil: tela de cadastro e troca do PIN (4 dígitos, digitado duas
...
- [x] 3.2 App 03: a mesma tela para o Admin, com entrada no menu — `RF-02-110`. Verificar:
      `vitest run` do teste novo, com os cenários da spec `aplicacao-de-gestao`.
- [x] 3.3 App 03, painel do dia: rótulo, seção e estado vazio passam a "Sem equipe", lendo
...
- [x] 4.1 `backend/tests/test_pin_de_confirmacao.py`: cadastro e troca, formato inválido,
...
- [x] 4.2 `backend/tests/` da confirmação do Guerreiro(a): sem PIN no App 01, PIN errado
...
- [x] 4.3 `backend/tests/` da presença sem rede: gravação com o confirmador, reenvio sem
      duplicar, nick que não resolve indistinguível, outra chave recusada, sem PIN cadastrado —
      spec `aula-e-presenca`.
- [x] 4.4 `apps/app-01-aula-presencial/src/entrada/entrada.test.tsx` e o teste da sessão de
...
- [x] 4.5 Testes da fila (`src/fila/`): enfileira só com o PIN conferido, PIN errado sem rede
...
- [x] 5.1 A decisão já foi gravada neste PR: documentos 03 (§§1.1, 3.2, 3.4), 08, 09 e 16;
