...
- [x] 1.1 Reordenar os nove `BlocoRecolhivel` de `ListaDeMissoes.tsx` para template da missão,
...
- [x] 2.1 Agrupar as missões por `etapa_do_ciclo` em `TelaDaTrilha.tsx` e navegar entre as
...
- [x] 2.2 Passar à `ListaDeMissoes` apenas as missões da etapa aberta, mantendo dentro dela a
      ordem crescente da posição (`RF-09-02`, `RF-09-03`, design — decisão 3).
- [x] 2.3 Fazer o formulário de nova missão nascer com a etapa aberta pré-selecionada, ainda
...
- [x] 3.1 Derivar na `TelaDaTrilha` as três travas pendentes — missão de sondagem, desafio de
      ...
      design — decisão 1).
- [x] 3.2 Apresentar o painel onde a publicação é oferecida — trilha em rascunho ou
...
- [x] 4.1 Em `trilhas.test.tsx`, cobrir a paginação por etapa: missões separadas pela etapa
...
- [x] 4.2 Em `trilhas.test.tsx`, cobrir o painel de pendências: as três travas apontadas antes
...
- [x] 4.3 Em `trilhas.test.tsx`, cobrir a ordem dos blocos da missão: template abre, ODS fecha
...
- [x] 5.1 Marcar a fatia 17 do PRD-09 como implementada em `openspec/cronograma-de-fatias.md`,
