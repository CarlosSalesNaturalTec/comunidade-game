...
- [x] 1.1 Em `producoes/regra.py`, extrair da `registrar_producao` o trecho comum às duas
      ...
      sem edição.
- [x] 1.2 Escrever `registrar_producao_individual` sobre esse trecho comum (`RF-05-74`,
...
- [x] 1.3 Expor `POST /v1/eu/missoes/{id}/producao` em `producoes/rotas.py`, com a mesma
...
- [x] 2.1 Em `trilhas/regra.py`, derivar as retomadas em aberto do Guerreiro(a) a partir da
...
- [x] 2.2 Expor `GET /v1/eu/retomadas` em `trilhas/rotas.py` (design — decisão 5): missão,
...
- [x] 3.1 Em `tests/test_producao_da_missao.py`, os cenários da regra individual: entrega por
...
- [x] 3.2 Ainda em `tests/test_producao_da_missao.py`, os cenários do contrato compartilhado
...
- [x] 3.3 Em `tests/test_producao_da_missao_porta.py`, os cenários HTTP da rota nova: 201 do
      Guerreiro(a) em sessão, 403 de Mestre e de Admin, recusa sem persona em sessão e a porta
      de equipe intacta (`RF-05-74`, `RF-01-16`).
- [x] 3.4 Em `tests/test_retomadas.py` (novo), os cenários da retomada: cadência virando
...
- [x] 4.1 Em `src/api/trilha.ts`, os dois clientes: a entrega individual em
      `multipart/form-data` e a leitura das retomadas, com os erros 422 e 503 tratados em
      mensagem para a criança.
- [x] 4.2 `EntregaDaProducao` dentro de `trilha/Missao.tsx` (design — decisão 7): as três formas
...
- [x] 4.3 Ainda em `EntregaDaProducao`, a devolutiva como próximo passo, com o aviso, na mesma
      ...
      `RF-05-77`, `RN-05-05`).
- [x] 4.4 `Retomadas` como tela do bloco Trilha (design — decisão 7): missão, trilha e prazo de
      ...
      `RN-05-38`).
- [x] 4.5 Testes de `EntregaDaProducao.test.tsx` e `Retomadas.test.tsx` cobrindo os cenários de
...
- [x] 5.1 Marcar a fatia 7 do PRD-05 como implementada em `openspec/cronograma-de-fatias.md`,
