...
- [x] 1.1 `equipes/rotas.py`: `POST /v1/trilhas/{id}/equipes`, sob
...
- [x] 1.2 `equipes/rotas.py`: `POST /v1/equipes/{id}/homologacao`, sob
...
- [x] 1.3 `equipes/rotas.py` e `trilhas/`: `ItemDaProgramacaoSaida` ganha `trilha_id` e
...
- [x] 2.1 `producoes/modelo.py`: `ProducaoDaMissao` com equipe, Guerreiro(a), missão,
...
- [x] 2.2 `backend/alembic/`: uma migração criando `producao_da_missao`, com a restrição da
      2.1. Verificar que `alembic upgrade head` e `downgrade` correm limpos no banco de teste.
- [x] 2.3 `producoes/porta.py`, `local.py`, `nuvem.py` e `fabrica.py`: `LeituraDaProducao`
...
- [x] 2.4 `producoes/regra.py`: `registrar_producao` — integrância na equipe (403 a quem não a
...
- [x] 2.5 `producoes/regra.py`: a gravação **não** emite `Resultado`, lançamento, ponto, nível
...
- [x] 2.6 `permissoes.py`: `Operacao.producao_da_equipe`, no Guerreiro(a), em escreve e lê
      (`RF-04-45`, design — decisão 6). Verificar em `tests/test_permissoes.py`.
- [x] 2.7 `producoes/rotas.py`: `POST /v1/equipes/{id}/producao` em `multipart/form-data` —
...
- [x] 3.1 `tests/test_equipe_da_trilha_rota.py`: os cenários das duas rotas novas de `equipe` —
...
- [x] 3.2 `tests/test_producao_da_missao.py`: os cenários de `producao-da-missao` — as três
...
- [x] 3.3 `tests/test_producao_da_missao_porta.py`: descarte de foto e áudio (nada persistido,
...
- [x] 4.1 `src/api/equipes.ts` e `src/api/producao.ts`: as três chamadas novas — criar equipe
...
- [x] 4.2 `src/trilhas/TelaDaProgramacao.tsx`: na atividade corrente, formar ou entrar na
...
- [x] 4.3 `src/trilhas/`: a homologação oferecida só ao Mestre em sessão de trabalho, com a
      ...
      (`RF-04-62`, `RN-04-18`).
- [x] 4.4 `src/trilhas/`: a entrega da produção nas três formas, com a produção esperada em
...
- [x] 4.5 `src/trilhas/trilhas.test.tsx`: os cenários de `aplicacao-da-aula-presencial` desta
...
- [x] 5.1 Gravar a decisão nova de 2026-08-30 — a produção entregue no App 01 é da equipe, num
...
- [x] 5.2 PRD-04: a `ProducaoDaMissao` na §8, a rota `POST /v1/equipes/{id}/producao` na §9 com
...
- [x] 5.3 `openspec/cronograma-de-fatias.md`: corrigir o recorte da fatia 8 para `RF-04-61` e
