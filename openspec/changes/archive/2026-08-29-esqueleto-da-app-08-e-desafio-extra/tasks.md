...
- [x] 1.1 Criar `backend/src/nucleo/desafios_extras/modelo.py` com `DesafioExtra` e os atributos
...
- [x] 1.2 Declarar os dois `StrEnum` da entidade — `Modalidade` (aberto, direcionado) e
...
- [x] 1.3 Escrever a migração Alembic da tabela, com o teto de 10 pontos extras como
      `CheckConstraint` (`RF-14-74`, `RN-14-41`)

## 2. Núcleo — a regra da proposta

- [x] 2.1 Implementar em `desafios_extras/regra.py` o registro da proposta: exigir trilha em
...
- [x] 2.2 Recusar com 422 os pontos extras acima de 10 e registrar a proposta sempre em
      `em_validacao_do_mestre`, mantendo os pontos do desafio isolados da pontuação regular
      (`RF-14-74`, `RN-14-41`, `RN-14-19`, `RN-14-13`)
- [x] 2.3 Aceitar a proposta direcionada **sem consultar a existência do nick** e recusar com
      422 a que não traga nick ou justificativa do vínculo (`RF-14-32`, `RF-14-33`, `RN-14-17`,
      `RN-14-18`)
- [x] 2.4 Derivar `lastro_provido` do custeio declarado — aporte do proponente **homologado**
...
- [x] 2.5 Recusar com 405 toda alteração de desafio **publicado**, preservando a proposta
      anterior com o desfecho que teve (`RF-14-38`)

## 3. Núcleo — as rotas do proponente

- [x] 3.1 Expor `POST /v1/desafios-extras` sob a operação `propostas_de_desafio_extra`, restrita
      ao Apoiador em sessão, e registrar o roteador na aplicação (`RF-14-29` a `RF-14-34`,
      `RF-14-74` a `RF-14-76`)
- [x] 3.2 Expor `GET /v1/eu/desafios-extras` com a situação de cada desafio do proponente, o
...
- [x] 4.1 Em `tests/test_desafio_extra_regra.py`, cobrir a proposta: trilha fora de andamento
...
- [x] 4.2 Em `tests/test_desafio_extra_rota.py`, cobrir as duas rotas: papel que não é Apoiador
...
- [x] 5.1 Criar `apps/app-08-apoiador/` no molde de `apps/app-09-mestre/` — Vite, React, TS,
      ...
      `RN-01-32`)
- [x] 5.2 Implementar `src/autenticacao/` sobre `comum/`: entrada por login social e por usuário
      e senha, sessão guardada como nas demais aplicações e nenhuma tela do Apoiador acessível
      sem sessão (`RF-14-08`)
- [x] 5.3 Trancar todas as demais telas até a troca da senha provisória, sem caminho de
      contorno, e apresentar a recusa do login sem cadastro com a orientação de usar o
      pré-cadastro (`RF-14-09`, `RF-14-10`, `RN-14-02`)
- [x] 5.4 Conferir que nenhuma tela oferece convite, delegação ou segundo acesso ao mesmo
      cadastro (`RF-14-11`, `RN-14-04`)

## 6. App 08 — as telas do desafio extra

- [x] 6.1 Implementar `src/desafiosExtras/` com a proposição: trilha em andamento, recompensa,
      ...
      `RF-14-74` a `RF-14-76`)
- [x] 6.2 Implementar o acompanhamento: estado no fluxo, motivo da recusa em linguagem simples,
...
- [x] 6.3 Conferir que nenhuma tela de desafio exibe nome real ou contato de Guerreiro(a), nem
      oferece campo de mensagem, telefone ou e-mail (`RF-14-39`, `RN-14-20`)

## 7. Testes da App 08

- [x] 7.1 Em `apps/app-08-apoiador/src/testes/`, cobrir a entrada: sem sessão só a entrada
...
- [x] 7.2 Cobrir as telas do desafio: teto de pontos recusado na tela, direcionado com nick
...
- [x] 8.1 Marcar a fatia 1 do PRD-14 como `implementado` em `openspec/cronograma-de-fatias.md`,
