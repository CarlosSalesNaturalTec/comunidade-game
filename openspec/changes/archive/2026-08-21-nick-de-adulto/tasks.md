...
- [x] 1.1 Tornar o índice único de `nick.valor` insensível a caixa em
...
- [x] 2.1 Em `backend/src/nucleo/personas/regra.py`, estender a unicidade e a exigência de nick
...
- [x] 2.2 Expor no mesmo módulo a função de conferência de disponibilidade restrita a nicks de
...
- [x] 3.1 Em `backend/src/nucleo/fila/regra.py`, gravar o nick declarado no pré-cadastro de
...
- [x] 4.1 Criar `GET /v1/nicks/disponibilidade` em `backend/src/nucleo/personas/rotas.py`, com
...
- [x] 4.2 Criar `PUT /v1/eu/apoiador/identidade` e `PUT /v1/eu/mestre/identidade`, que definem
...
- [x] 4.3 Conferir que a nova rota pública entra no freio por origem já vigente para consulta
...
- [x] 5.2 Criar `backend/tests/test_identidade_do_adulto.py` cobrindo os cenários do delta de
...
- [x] 5.3 Em `backend/tests/test_fila.py`, cobrir os cenários do delta de `fila-de-avaliacao`:
...
- [x] 6.1 Gravar as decisões novas nos documentos-fonte e nos PRDs, no mesmo PR: documento 02
