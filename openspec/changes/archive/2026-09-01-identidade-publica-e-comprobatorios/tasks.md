...
- [x] 1.1 Em `backend/src/nucleo/poder_sustentador/regra.py`, escrever `moedas_acumuladas_de` —
      ...
      (`RF-14-14`, `RN-14-11`)
- [x] 1.2 Em `backend/src/nucleo/erros.py`, declarar `PisoDeMoedasNaoAlcancado` (409) levando
      quantas moedas faltam, no molde dos demais 409 (`RF-14-14`, `RF-14-16`, PRD-14 §9)
- [x] 1.3 Em `backend/src/nucleo/personas/rotas.py`, aceitar `avatar` opcional no
      ...
      `RN-14-11`)
- [x] 1.4 No mesmo arquivo, criar `GET /v1/eu/apoiador/identidade` com nick, avatar, moedas
...
- [x] 2.1 Em `backend/src/nucleo/personas/modelo.py`, acrescentar `anexado_por_id` e `anexado_em`
      a `ArtefatoComprobatorio`, nulos, com o comentário do que é pendente pela decisão 5 do
      design (`RF-14-19`, `RN-14-12`)
- [x] 2.2 Escrever a migração Alembic que acrescenta as duas colunas sem reescrever linha alguma,
      com `downgrade` que as remove; verificar com `alembic upgrade head` seguido de
      `alembic downgrade -1` (`RF-14-19`)
- [x] 2.3 Em `backend/src/nucleo/personas/regra.py`, escrever a declaração do documento pelo
...
- [x] 2.4 Em `backend/src/nucleo/personas/rotas.py`, expor `POST /v1/eu/apoiador/documentos` e
...
- [x] 3.1 Em `backend/tests/test_poder_sustentador.py`, cobrir `moedas_acumuladas_de`: soma dos
...
- [x] 3.2 Em `backend/tests/test_persona_rota.py`, cobrir a identidade do Apoiador: avatar
...
- [x] 3.3 Criar `backend/tests/test_documentos_do_apoiador_rota.py`: documento declarado nasce
      ...
      `RN-14-12`, `RF-02-101`)
- [x] 3.4 Em `backend/tests/test_artefatos_do_mestre_rota.py`, confirmar que o artefato do Mestre
...
- [x] 4.1 Criar `apps/app-08-apoiador/src/identidade/api.ts` com a leitura e a gravação de
...
- [x] 4.2 Implementar `src/identidade/TelaDeIdentidadePublica.tsx`: define ou troca nick e avatar
...
- [x] 4.3 Na mesma tela, apresentar abaixo do piso o **avatar padrão do projeto** — marca neutra
...
- [x] 4.4 Criar `src/documentos/api.ts` e `src/documentos/TelaDeComprobatorios.tsx`: envio por
...
- [x] 4.5 Em `src/App.tsx`, acrescentar as duas áreas à navegação que já existe, sem roteador
      (`RF-14-12`, `RF-14-18`)

## 5. Testes da App 08

- [x] 5.1 Criar `apps/app-08-apoiador/src/identidade/identidade.test.tsx`: nick do pré-cadastro
...
- [x] 5.2 Criar `src/documentos/comprobatorios.test.tsx`: a tela pede endereço e rótulo e não
...
- [x] 6.1 Gravar a decisão nova nos documentos-fonte: o **ato do Admin que anexa ao cadastro o
      ...
      (`RF-14-19`, `RN-14-12`)
- [x] 6.2 Acrescentar o `RF-02-101` a `docs/prds/prd-02-frontend-de-gestao.md` §6.2, a rota de
      anexação à tabela da §9 e a linha de rastreabilidade na §15 (`RF-02-101`)
- [x] 6.3 Acrescentar à tabela de rotas de `docs/prds/prd-14-area-do-apoiador.md` §9 e de
...
- [x] 6.4 Marcar a fatia 3 do PRD-14 como `implementado` em
