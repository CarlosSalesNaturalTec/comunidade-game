...
- [x] 1.1 Expor `GET /v1/eu/guerreiros` em `responsaveis/rotas.py`, restrita ao responsável em
...
- [x] 2.1 Criar `backend/src/nucleo/evolucao/regra.py`, sem `modelo.py`, com a montagem do
...
- [x] 2.2 Acrescentar à regra as **criações originais validadas**, com título, trilha e data da
      validação, e omitir as que ainda não foram validadas (`RF-13-10`)
- [x] 2.3 Garantir na montagem que nenhum dado identificável de outra criança entra no payload —
...
- [x] 2.4 Implementar em `evolucao/regra.py` a leitura das ocorrências de conduta do vinculado,
...
- [x] 2.5 Expor em `evolucao/rotas.py` as duas rotas — `GET /v1/eu/guerreiros/{id}/evolucao` e
...
- [x] 3.1 Em `tests/test_evolucao_regra.py`, cobrir a montagem: Guerreiro(a) com histórico traz
...
- [x] 3.2 Em `tests/test_evolucao_rota.py`, cobrir as três rotas com os critérios de aceite do
...
- [x] 4.1 Criar `apps/app-07-responsaveis/` no molde de `apps/app-08-apoiador/` — Vite, React,
      ...
      (`RF-01-02`, `RN-01-32`)
- [x] 4.2 Implementar `src/entrada/` sobre `comum/`: login social e usuário e senha, troca
...
- [x] 4.3 Conferir que nenhuma tela oferece autocadastro, cadastro de responsável, criação,
      edição ou remoção de vínculo, nem mudança do grau de parentesco (`RF-13-06`, `RN-13-01`)

## 5. App 07 — os vinculados e a evolução

- [x] 5.1 Implementar `src/vinculados/` com a lista dos vinculados, cada um com o grau de
      ...
      `RN-13-04`)
- [x] 5.2 Implementar `src/evolucao/` com o painel do vinculado: presença, atividades realizadas,
      ...
      `RF-13-10`)
- [x] 5.3 Apresentar as ocorrências de conduta com motivo e data, em linguagem simples e sem
...
- [x] 5.4 Conferir que nenhuma tela apresenta consulta ao assistente, transcrição de apoio
...
- [x] 6.1 Em `apps/app-07-responsaveis/src/testes/`, cobrir a entrada e a lista: sem sessão só a
...
- [x] 6.2 Cobrir o painel: evolução com presença, atividades, pontos, poderes, badges e nível;
...
- [x] 7.1 Marcar as fatias 1 e 2 do PRD-13 como `implementado` em
...
- [x] 7.2 Registrar a lacuna do **estado da reparação** na §14 do PRD-13 e na tabela de decisões
