...
- [x] 1.1 `GET /solicitacoes-de-dados` em `fila/rotas.py`, no molde da rota de participação da
...
- [x] 1.2 `POST /solicitacoes-de-dados/{id}/avaliacao`, consumindo `avaliar_solicitacao_de_dados`
...
- [x] 2.1 `GET /solicitacoes-de-chave` em `fila/rotas.py`: paginada, restrita a Admin, com quem
...
- [x] 2.2 `POST /solicitacoes-de-chave/{id}/avaliacao`, consumindo `avaliar_solicitacao_de_chave`
...
- [x] 3.1 `GET /sugestoes` em `fila/rotas.py`: paginada, restrita a Admin, reunindo numa fila só
      ...
      `RN-01-49`).
- [x] 3.2 `POST /sugestoes/{id}/avaliacao`, consumindo `avaliar_sugestao` **sem alterá-la**:
...
- [x] 4.1 Em `backend/tests/test_fila_rota.py`, os cenários da solicitação de dados: Admin lê a
...
- [x] 4.2 Ainda em `test_fila_rota.py`, os cenários da guarda de entrega: nenhum conjunto sai de
...
- [x] 4.3 Em `backend/tests/test_chave_de_terceiro_rota.py`, o caminho de ponta a ponta que
...
- [x] 4.4 Ainda em `test_fila_rota.py`, os cenários da sugestão: adotada credita 20 extras e o
...
- [x] 5.1 `apps/app-03-gestao/src/filas/`: as três naturezas entram no filtro e na lista já
...
- [x] 5.2 Avaliação da solicitação de dados: os **três critérios** apresentados antes da
...
- [x] 5.3 Avaliação da solicitação de chave em **dois atos**: primeiro o desfecho com parecer,
...
- [x] 5.4 Avaliação da sugestão: motivo do retorno exigido na não adotada, apontado no campo
...
- [x] 6.1 `apps/app-03-gestao/src/chaves/`: área ao lado das Filas (design — decisão 4), sobre
...
- [x] 7.1 Em `apps/app-03-gestao/src/filas/filas.test.tsx`, os cenários das três naturezas: o
...
- [x] 7.2 Ainda em `filas.test.tsx`, os cenários da chave: aprovar não emite e passa a oferecer
      ...
      `RF-02-89`, `RN-02-28`).
- [x] 7.3 `apps/app-03-gestao/src/chaves/chaves.test.tsx`: o painel mostra prazo, URL e
...
- [x] 8.1 `docs/prds/prd-02-frontend-de-gestao.md`, as duas correções decididas em 2026-08-22:
...
- [x] 8.2 Nenhum outro documento muda: as duas correções não criam regra de produto, e por isso
