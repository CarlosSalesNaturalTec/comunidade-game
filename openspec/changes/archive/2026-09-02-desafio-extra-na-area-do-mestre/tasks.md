...
- [x] 1.1 Acrescentar `parecer_do_mestre` (texto, nulo) a `DesafioExtra` em
...
- [x] 2.1 `propor_desafio_extra` passa a aceitar **Mestre** além de Apoiador e decide a situação
...
- [x] 2.2 Exigir a justificativa do direcionado também do Mestre, no campo
      `justificativa_do_vinculo`, mantendo as guardas do nick (`RF-09-111`, design — decisão 5).
- [x] 2.3 `validar_desafio_extra`: só o Mestre autor da trilha, sobre desafio em
...
- [x] 4.1 `apps/app-09-mestre/src/desafiosExtras/api.ts` com as quatro chamadas — fila a
      validar, validação, proposta e os próprios desafios (`RF-09-51`, `RF-09-105`).
- [x] 4.2 Tela da fila e do ato de validar: a lista do que há por validar com o que cada
      ...
      (`RF-09-51`, `RF-09-52`).
- [x] 4.3 Formulário da proposta do Mestre: trilha em andamento, recompensa, quantidade,
...
- [x] 4.4 Lista dos desafios que o Mestre propôs, com a situação, o motivo da recusa e a
      quantidade restante do publicado, sem identificar Guerreiro(a) (`RF-09-105`,
      `RF-09-112`, `RN-14-20`).
- [x] 4.5 Ligar a área no `App.tsx`, no molde das demais.

## 5. Testes

- [x] 5.1 `backend/tests/test_desafio_extra_regra.py`: proposta do Mestre autor nascendo em
      ...
      `RN-09-40`, `RN-09-41`).
- [x] 5.2 `backend/tests/test_desafio_extra_rota.py`: as rotas `/validacao` e `/a-validar` com
      ...
      `RF-09-105`).
- [x] 5.3 `apps/app-09-mestre/src/desafiosExtras/desafiosExtras.test.tsx`: a fila listando só o
...
- [x] 6.1 Marcar a fatia 15 do PRD-09 como implementada em
...
- [x] 6.2 PRD-09 §9: as duas rotas novas — `POST /v1/desafios-extras` pelo Mestre e
