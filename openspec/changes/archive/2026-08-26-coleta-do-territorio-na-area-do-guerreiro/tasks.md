...
- [x] 1.1 Derivar a **próxima medição** da série em `coletas/regra.py`, a partir de
...
- [x] 1.2 Escrever a consulta do **histórico da própria série** em `coletas/regra.py` —
      ...
      `RN-05-21`).
- [x] 1.3 Escrever a consulta dos **desafios que o Guerreiro(a) pode assumir** em
...
- [x] 1.4 Escrever a consulta das **próprias solicitações de local** em `locais/regra.py`,
      ...
      `RN-05-11`, `RN-05-21`).
- [x] 1.5 Publicar as três rotas de coleta em `coletas/rotas.py` e a de solicitações em
...
- [x] 2.1 Criar o cliente das rotas de coleta em `apps/app-05-guerreiro/src/api/`, no molde de
...
- [x] 2.2 Tela das **minhas séries**: o que cada uma mede, local, estado, próxima medição e
...
- [x] 2.3 Fluxo de **abrir série**: escolha do desafio entre os disponíveis e do local entre os
...
- [x] 2.4 Fluxo de **solicitar local que falta** e a lista das próprias solicitações, com
      situação, motivo da recusa e o local criado pela aprovação, deixando claro que o pedido
      não cria local (`RF-05-32`).
- [x] 2.5 Tela de **registrar medição**: valor digitado (origem `manual`), ditado por voz
...
- [x] 2.6 Devolutiva da gravação: pontuou na hora, ou entrou **a conferir** com a explicação
      acolhedora de que o Mestre vai olhar, sem acusação e sem código técnico (`RF-05-34`,
      `RF-05-35`, `RN-05-08`).
- [x] 2.7 Tela do **histórico da série**: data, valor, situação e pontos de cada registro, e o
      motivo do registro invalidado, com a frase de que só ele perdeu os pontos (`RF-05-37`,
      `RF-05-38`, `RN-05-09`).
- [x] 2.8 **Recusa sem rede** no registro, com o motivo em linguagem simples e nenhuma fila,
      medição ou mídia guardada no aparelho (`RF-05-85`).
- [x] 2.9 **Aviso discreto de coleta de dados** nas telas que coletam, com acesso à área
      detalhada, sem bloquear a tela (`RF-05-57`).
- [x] 2.10 Ligar o bloco da coleta à área do Guerreiro(a) em sessão, sem exibir nenhum dado de
      outra criança em nenhuma das telas novas (`RN-05-21`).

## 3. Testes do núcleo

- [x] 3.1 `tests/test_serie_de_coleta.py`: os cenários novos da capacidade `serie-de-coleta` —
...
- [x] 3.2 `tests/test_registro_de_coleta.py`: os cenários do histórico da própria série —
...
- [x] 3.3 `tests/test_desafio_de_coleta.py` e `tests/test_solicitacao_de_local.py`: os cenários
...
- [x] 3.4 `tests/test_coletas_rota.py` e `tests/test_solicitacao_de_local_rota.py`: as quatro
      rotas no ar, com o contrato de listagem — cursor, tamanho, 422 do parâmetro não
      declarado — e o recorte por papel.

## 4. Testes da App 05

- [x] 4.1 Testes das telas de série e abertura: lista com próxima medição e pontos, série
...
- [x] 4.2 Testes do registro: origem `manual` na digitação, origem `voz` no ditado com o áudio
...
- [x] 4.3 Testes do histórico, das solicitações de local e do aviso de coleta: motivo do
...
- [x] 5.1 Gravar a decisão do fundador de 2026-08-26 — **o ditado por voz do registro de coleta
...
- [x] 5.2 Corrigir a redação da tabela de rotas do PRD-05 §9 para as rotas que existem —
