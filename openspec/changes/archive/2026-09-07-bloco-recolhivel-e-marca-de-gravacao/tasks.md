...
- [x] 1.1 `BlocoRecolhivel.tsx`: sobre `details`/`summary` nativo, nasce fechado a cada
...
- [x] 1.2 `MarcaDeGravacao.tsx`: apresentação pura que recebe o instante da última gravação e o
...
- [x] 1.3 `estilos.css` recebe o estilo dos dois, na camada semântica e de tema dos tokens, e
      `indice.ts` os exporta (documento 15 §12).

## 2. Área do Mestre — a tela da trilha recolhe

- [x] 2.1 `ListaDeMissoes.tsx`: os dez blocos por missão passam a `BlocoRecolhivel`, cada um
...
- [x] 2.2 `TelaDaTrilha.tsx`: etiquetas ODS da trilha, cobertura de ODS e culminância passam a
...
- [x] 2.3 Os componentes de bloco que hoje trazem título próprio — `EtiquetasOds`,
...
- [x] 3.1 As cinco escritas que acontecem no próprio bloco — cadência de retomada, desafio de
      ...
      design — decisão 5).
- [x] 3.2 As seis escritas em formulário que fecha ao gravar — atividade, conteúdo,
...
- [x] 3.3 Escrita recusada ou falha não produz marca: conferir que o instante só é registrado
      no caminho de sucesso, e que a recusa segue aparecendo como já aparece (documento 15
      §6.2).
- [x] 3.4 `TemplateDaMissao.tsx`: o campo "O que você quer ensinar nesta missão?" ganha, em uma
      linha, a nota de que o tópico não é guardado — só o que o Mestre aceitar vira registro
      (`RN-09-33`).

## 4. Testes

- [x] 4.1 `comum/react/BlocoRecolhivel.test.tsx`: nasce fechado com o resumo na linha; o
...
- [x] 4.2 `comum/react/MarcaDeGravacao.test.tsx`: instante nulo não apresenta marca; instante
...
- [x] 4.3 `apps/app-09-mestre/src/trilhas/trilhas.test.tsx`: os testes da tela passam a abrir o
...
- [x] 5.1 Marcar a fatia transversal como implementada em `openspec/cronograma-de-fatias.md`,
