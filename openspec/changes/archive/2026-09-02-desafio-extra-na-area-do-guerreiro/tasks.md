...
- [x] 1.1 Em `backend/src/nucleo/desafios_extras/regra.py`, criar
...
- [x] 1.2 Ordenar o retorno pelo fim da vigência mais próximo e, no empate, pelo registro mais
      recente — o que está para acabar aparece primeiro (`RF-05-21`).

## 2. A rota do Guerreiro(a)

- [x] 2.1 Em `backend/src/nucleo/trilhas/rotas.py`, criar `DesafioExtraDoGuerreiroSaida` com
...
- [x] 2.2 Trocar o retorno de `GET /v1/eu/desafios` por um objeto `MeusDesafiosSaida` com
...
- [x] 3.1 Em `apps/app-05-guerreiro/src/api/desafiosEEquipes.ts`, tipar
...
- [x] 3.2 Criar `apps/app-05-guerreiro/src/desafios/MeusDesafiosExtras.tsx`: cada desafio com a
...
- [x] 3.3 Em `MeusDesafios.tsx`, montar os dois blocos apartados a partir da mesma chamada —
      ...
      decisão 6).
- [x] 3.4 Acrescentar ao `index.css` da App 05 o que o bloco novo precisa, no padrão das
      classes `cg-` já usadas em `cg-lista-de-desafios` e `cg-cartao-de-desafio`.

## 4. Testes

- [x] 4.1 Em `backend/tests/test_desafio_extra_regra.py`, cobrir a elegibilidade: o aberto
      ...
      `RF-05-21`).
- [x] 4.2 Em `backend/tests/test_meus_desafios.py`, cobrir a rota no formato novo: os dois
      ...
      `RN-14-20`).
- [x] 4.3 Criar `apps/app-05-guerreiro/src/desafios/MeusDesafiosExtras.test.tsx`: o cartão
...
- [x] 4.4 Ajustar `apps/app-05-guerreiro/src/desafios/MeusDesafios.test.tsx` ao formato novo da
      resposta e cobrir que semanais e extras aparecem em blocos distintos (`RF-05-19`,
      `RF-05-20`).

## 5. Documentação

- [x] 5.1 Marcar a fatia 8 do PRD-05 como `implementado` em
