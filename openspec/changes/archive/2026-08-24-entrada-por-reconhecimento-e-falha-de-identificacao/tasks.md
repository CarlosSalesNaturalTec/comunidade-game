...
- [x] 1.1 `aulas/rotas.py`: `POST /v1/aulas/{id}/presencas` passa a exigir também
...
- [x] 1.2 `aulas/rotas.py`: no modo reconhecimento a rota **não preenche o confirmador**, e
...
- [x] 1.3 Conferir que a resposta da rota devolve o `momento_do_fato` **gravado**, não o
...
- [x] 2.1 `api/sessoesDeGuerreiro.ts`: cliente de `POST /v1/sessoes/guerreiro`, com nick e
...
- [x] 2.2 `api/presencas.ts` (novo): cliente de `POST /v1/aulas/{id}/presencas` nos dois modos,
...
- [x] 2.3 `TelaDeEntradaDoGuerreiro.tsx`: nick, depois vivacidade e descritor pelo módulo de
...
- [x] 2.4 A mesma tela avisa a **presença já registrada** comparando o momento do fato devolvido
...
- [x] 3.1 A recusa do núcleo vira **nova tentativa** com uma frase única, igual para nick
...
- [x] 3.2 A confirmação humana passa a **registrar a presença** no modo confirmação, com o adulto
...
- [x] 3.3 Tela de recadastro da imagem de referência, atrás da sessão do Guerreiro(a) aberta por
...
- [x] 4.1 `tests/` do núcleo, presença: os cenários da spec de `aula-e-presenca` — reconhecimento
...
- [x] 4.2 `tests/` do núcleo, matriz: nenhuma operação de escrita de presença é concedida ao
      Guerreiro(a), e a tentativa dele pela rota é recusada (`RF-04-18`, design — decisão 1).
- [x] 4.3 `entrada.test.tsx` da App 01: os cenários da spec de `aplicacao-da-aula-presencial` —
...
- [x] 4.4 Teste de recadastro da imagem na App 01, cobrindo que o identificador vem da sessão e
      não de consulta por nick (`RF-04-22`, `RN-01-22`).

## 5. Documentação

- [x] 5.1 No mesmo PR: PRD-04 §9 corrigido no código da presença duplicada — idempotente e sem
