...
- [x] 1.1 Copiar para `comum/fontes/` os quatro `.woff2` variáveis — Atkinson Hyperlegible
...
- [x] 1.2 Escrever `comum/fontes.css` com os quatro `@font-face`, nomeando as famílias como o
...
- [x] 1.3 Ampliar o mapa de `exports` e a lista de `files` de `comum/package.json` para
...
- [x] 2.1 Criar `comum/tsconfig.json` e a configuração de Vitest de `comum/`, e acrescentar
      `--largura-de-leitura` à camada semântica de `comum/tokens.css` com o valor que o
      documento 15 §4 já fixa (design — Decisions).
- [x] 2.2 `Botao` e `Moldura`: alvo de toque de ao menos 48 px, ao menos 8 px de separação
...
- [x] 2.3 `Campo`: gera o próprio identificador e amarra rótulo, mensagem de erro e estado
...
- [x] 2.4 `Aviso`, `Cabecalho` e `EstadoDaLista`, e o `comum/react/indice.ts` exportando os
...
- [x] 3.1 `apps/app-03-gestao/src/index.css`: importar `comum/fontes.css` e remover o
      `min-height` global, que hoje alcança inclusive o botão desenhado pelo Google
      Identity Services (design — Decisions).
- [x] 3.2 `TelaDeEntrada` consumindo `Moldura`, `Cabecalho`, `Botao` e `Aviso`, sem tocar em
...
- [x] 3.3 `TelaDeComunidades` e `ListaDeComunidades` como lista densa do temperamento
...
- [x] 3.4 `FormularioDeComunidade` consumindo `Campo` e `Aviso`, preservando a validação e as
      recusas que já existem (`RN-02-04`, `RN-08-01`). Cobre o cenário "Erro de campo
      anunciado no próprio campo".

## 4. Testes

- [x] 4.1 Testes de `comum/react`: a fiação de `Campo` — rótulo, mensagem e estado inválido
...
- [x] 4.2 Ampliar `apps/app-03-gestao/src/comunidades/comunidades.test.tsx` com o erro de
...
- [x] 5.1 `firebase.json`: regra de cache longo para os arquivos com impressão digital no
      nome, que passam a incluir as fontes (design — Decisions).
- [x] 5.2 Conferir à mão, uma vez, o que o jsdom não prova — contraste de 4,5:1 em texto e
...
- [x] 5.3 Gravar as decisões novas: `docs/03-plataforma-e-arquitetura.md` §1.2 (o que
