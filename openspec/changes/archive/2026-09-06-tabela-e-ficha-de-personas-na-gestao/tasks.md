...
- [x] 1.1 `ListaDeAdultos.tsx` passa a `Tabela`, com as colunas nome, e-mail, nick e
      ...
      design — decisão 2).
- [x] 1.2 `ListaDeGuerreiros.tsx` passa a `Tabela` com as colunas que ela já traz — nick,
      avatar, comunidade e início do vínculo (`RF-02-01`, `RF-02-15`, design — decisão 4).

## 2. Ficha do adulto

- [x] 2.1 `FichaDoAdulto.tsx` sobre o `Dialogo`: nome, e-mail, WhatsApp, nick e os artefatos
...
- [x] 2.2 A ficha oferece o caminho de gravar o nick quando ele está vazio, reaproveitando
      `FormularioDeNick` sem mudar a regra dele — nenhum nick é sugerido (`RF-02-01`,
      `RN-01-30`, `RN-14-10`).
- [x] 2.3 `TelaDeAdultos.tsx` liga a linha da tabela à ficha e recarrega a lista quando o nick
      é gravado.

## 3. Forma que faltava

- [x] 3.1 `index.css`: dar forma a `.cg-campo-de-artefato`, hoje usada sem definição, e apagar
      `.lista-de-personas` e as suas variações, que a tabela substitui.

## 4. Testes

- [x] 4.1 `personas.test.tsx`: a tabela de Mestres lista os cadastrados; a ficha de um Mestre
...
- [x] 5.1 Marcar a fatia 18 como implementada em `openspec/cronograma-de-fatias.md` e mover
