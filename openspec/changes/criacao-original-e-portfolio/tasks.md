...
- [x] 1.1 Estender `criacoes_originais/modelo.py`: `equipe_id` e `producao` opcionais,
...
- [x] 1.2 Criar a revisão Alembic da tabela `criacao_original` na ordem do plano de migração,
...
- [x] 2.1 `RF-05-40`, `RF-09-30`: reescrever `entregar_criacao_original` para seguir a
...
- [x] 2.2 `RF-05-42`: fazer a nova entrega antes da validação substituir a produção e devolver a
      situação a "entregue", e recusar com 409 a entrega depois de validada (design — decisão 3).
- [x] 2.3 `RF-05-42`, `RF-09-34`: exigir e gravar o motivo na devolução, recusando com 422 a
      devolução sem motivo, sem alterar a autoria (`RN-05-13`, `RN-09-04`).
- [x] 2.4 `RF-09-31`: acrescentar o caminho individual em
...
- [x] 3.1 `RF-05-40`, `RF-05-41`: criar `criacoes_originais/rotas.py` com
...
- [x] 3.2 `RF-05-40`: acrescentar `POST /v1/criacoes/{id}/arquivo` e `PATCH
      /v1/criacoes/{id}/arquivo`, espelhando a sessão retomável de `conteudos/rotas.py`
      (design — decisão 4).
- [x] 3.3 `RF-05-43`, `RF-05-44`: expor `GET /v1/eu/portfolio` com trilha, data da validação,
      ...
      (`RN-05-21`).
- [x] 3.4 `RF-09-31`, `RF-09-32`, `RF-09-34`: expor a fila das criações entregues das trilhas do
      Mestre autor, com a produção e a autoria, e as rotas de validar e devolver com motivo, sob
      a posse estrita da trilha.
- [x] 3.5 `RF-09-33`, `RN-09-19`: estender `condicao_de_autorizacao_vigente` em
...
- [x] 4.1 `RF-05-39`: tela da culminância com descrição, critério de validação e modalidade
      escritos pelo Mestre autor, e o aviso de culminância não declarada sem oferecer entrega.
- [x] 4.2 `RF-05-40`, `RF-05-41`: tela de entrega nos cinco tipos, com progresso do envio de
      mídia e o papel de cada integrante na modalidade de equipe, consultando a equipe
      homologada sem oferecer formá-la (`RN-05-12`).
- [x] 4.3 `RF-05-42`: exibir a criação devolvida com o motivo em linguagem simples, a autoria
      intacta e o caminho de reenvio.
- [x] 4.4 `RF-05-43`, `RF-05-44`: tela do portfólio com trilha, data e autoria de cada criação
      validada, marcando o que é público e o que depende de autorização do responsável, sem
      oferecer alterá-la (`RN-05-14`).
- [x] 4.5 Acrescentar as chamadas de criação original e portfólio ao cliente de API da App 05.

## 5. App 09 — Área do Mestre

- [x] 5.1 `RF-09-31`, `RF-09-32`: tela da fila das criações a validar, com trilha, critério
      declarado, produção e autoria — cada integrante com o papel, na modalidade de equipe.
- [x] 5.2 `RF-09-31`, `RF-09-34`: validar, confirmando autoria creditada e badge liberado, e
      devolver exigindo o motivo, sem oferecer editar a produção nem reatribuir a autoria
      (`RN-09-04`).
- [x] 5.3 `RF-09-33`: informar na criação validada que ela só vai à vitrine com autorização de
      todos os creditados, sem oferecer conceder nem revogar (`RN-09-19`).
- [x] 5.4 Acrescentar as chamadas da fila, da validação e da devolução ao cliente de API da
      App 09.

## 6. Testes

- [x] 6.1 Estender `backend/tests/test_criacao_original.py` com os cenários da entrega
      ...
      `RF-09-34`).
- [x] 6.2 Cobrir o crédito da modalidade individual — 50 pontos, nível 5 e badge de autoria ao
      Guerreiro(a) que entregou — e confirmar que o crédito de equipe segue integral a cada
      integrante, sem rateio (`RF-09-31`).
- [x] 6.3 Cobrir as rotas novas: entrega sob a culminância, envio de mídia, portfólio com a
...
- [x] 6.4 Cobrir na vitrine que a criação individual credita quem entregou e não aparece sem
      autorização vigente (`RF-09-33`, `RN-09-19`).
- [x] 6.5 Testes das telas da App 05 — culminância, entrega nos cinco tipos com papel de
      integrante, devolução com motivo e reenvio, portfólio com as duas situações de exposição
      (`RF-05-39` a `RF-05-44`).
- [x] 6.6 Testes das telas da App 09 — fila com critério e papéis, validação, devolução recusada
      sem motivo e o aviso da autorização de divulgação (`RF-09-31` a `RF-09-34`).

## 7. Documentação

- [x] 7.1 Acrescentar uma linha à tabela de fatias do PRD-05 em `docs/prds/index.md`, marcando
