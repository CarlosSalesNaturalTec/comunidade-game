...
- [ ] 1.1 Gravar no **documento 04 §1** as duas decisões desta change — o **teto da receita
...
- [ ] 2.1 Criar o enum `DestinacaoDoAporte` (`lastro`, `ressarcimento`) e a coluna `destinacao`
...
- [ ] 2.2 Filtrar `livro_razao.regra.saldo_de` pela destinação `lastro`, mantendo a coluna local
...
- [ ] 2.3 Aceitar `destinacao` em `POST /aportes`, recusando com 422 o valor fora dos dois
...
- [ ] 3.1 Acrescentar `aula_id` anulável a `Aporte` e aceitá-la em `POST /aportes/absorcao`,
...
- [ ] 3.2 Exigir `valor_de_origem` na absorção de natureza consumível, durável ou financeira, e
...
- [ ] 4.1 Admitir em `livro_razao.regra.lancar_ajuste` o ajuste de **quantidade zero** com
...
- [ ] 5.1 Criar `src/nucleo/ressarcimentos/modelo.py` com a entidade `Ressarcimento` — aporte
...
- [ ] 5.2 Criar `src/nucleo/ressarcimentos/regra.py` com a fila por antiguidade — absorções
      ressarcíveis em aberto, da mais antiga à mais nova, com o saldo de receita destinada em
      aberto (`RF-07-24`, `RN-07-17`)
- [ ] 5.3 Acrescentar à mesma `regra.py` o registro do ressarcimento: `FOR UPDATE` sobre os
      ressarcimentos da receita declarada, conferência do teto, exigência do comprovante em PDF,
      JPG ou PNG, e recusa de aporte não ressarcível, já ressarcido ou de origem em aporte de
      destinação lastro (`RF-07-22`, `RN-07-17`, design — Decisions 3)
- [ ] 5.4 Emitir, no mesmo ato e na mesma transação, o ajuste que reverte as moedas do crédito
...
- [ ] 5.5 Criar `src/nucleo/ressarcimentos/rotas.py` com `GET /aportes/ressarciveis` e
...
- [ ] 6.1 Migração Alembic em passo único: `aporte.destinacao`, `aporte.aula_id`,
...
- [ ] 7.1 `tests/test_aporte_destinacao.py` — a doação destinada credita o Poder Sustentador e
      ...
      (`RF-07-23`, `RN-07-38`)
- [ ] 7.2 `tests/test_aporte_absorcao_necessidade.py` — a absorção declara a aula que atende,
...
- [ ] 7.3 `tests/test_ressarcimento_fila.py` — a fila sai da mais antiga à mais nova, o aporte
      ressarcido some dela, o aporte da gestão e a absorção de serviço nunca entram, e o Mestre
      recebe 403 (`RF-07-24`)
- [ ] 7.4 `tests/test_ressarcimento_registro.py` — registro sem comprovante é 422; aporte não
...
- [ ] 7.5 `tests/test_ressarcimento_reversao.py` — o Poder Sustentador volta ao valor anterior
      ...
      (`RF-07-25`, `RN-07-18`)
- [ ] 7.6 `tests/test_meus_aportes_ressarciveis.py` — o Mestre lê a situação só dos próprios
      aportes, não alcança os de outro provedor e não tem operação de escrita; Apoiador recebe
      403 (`RF-07-24`, `RN-07-17`)

## 8. Documentação

- [ ] 8.1 Atualizar `docs/prds/index.md` com o que a sexta fatia entregou, e conferir que as
