...
- [ ] 1.1 `RF-13-32`, `RF-13-33`: criar `src/nucleo/termos/` com `Termo(tipo, versao, texto,
...
- [ ] 1.2 `RF-13-32`, `RF-13-33`, `RF-13-34`: escrever a regra de consulta — vigente por tipo e
...
- [ ] 1.3 `RF-13-32`, `RN-13-19`: expor `GET /v1/termos` sob `exigir_persona`, de qualquer
...
- [ ] 1.4 `RF-13-34`, `RN-13-19`: semear o texto do termo aprovado pelo fundador na versão que a
...
- [ ] 2.1 `RF-13-30`: acrescentar `acesso_ao_dado_do_guerreiro` (auditoria, Guerreiro(a)), com
      índice por Guerreiro(a) e por momento, mantendo a trilha somente inserção (design —
      decisão 1).
- [ ] 2.2 `RF-13-30`: fazer o `MiddlewareDeAuditoria` colher todo `guerreiro_id` dos
...
- [ ] 2.3 `RF-13-30`, `RN-13-04`: expor `GET /v1/eu/guerreiros/{id}/acessos`, paginada no
...
- [ ] 2.4 `RF-13-29`, `RN-13-20`: criar `src/nucleo/transparencia/` com o catálogo declarado das
...
- [ ] 2.5 `RF-13-29`, `RN-13-04`: expor `GET /v1/eu/guerreiros/{id}/dados`, restrita ao
      responsável vinculado, sem devolver conteúdo de dado algum.

## 3. Núcleo — ato assistido, responsáveis e crédito da proposta

- [ ] 3.1 `RF-13-35`, `RN-13-03`: expor `GET /v1/guerreiros/{id}/responsaveis` sob
...
- [ ] 3.2 `RF-13-35`, `RF-13-36`, `RF-13-38`, `RN-13-16`: expor `POST
      ...
      decisão 5).
- [ ] 3.3 `RN-13-18`: em `avaliar_sugestao`, creditar os 20 pontos extras e o badge de
...
- [ ] 3.4 Gerar a migração Alembic das três tabelas novas e da semente do termo, e verificar que
      `alembic upgrade head` e `downgrade` correm limpos.

## 4. App 07 — telas

- [ ] 4.1 `RF-13-29`, `RN-13-20`: tela de transparência do vinculado — dado, finalidade, prazo,
...
- [ ] 4.2 `RF-13-30`, `RF-13-31`: histórico de acessos com data, hora, quem acessou, papel e
      dado, o acesso de rotina apresentado como rotina, e o esclarecimento aberto da própria
      linha, com a referência do acesso.
- [ ] 4.3 `RF-13-32`, `RF-13-33`, `RF-13-34`: tela do termo — texto vigente em linguagem
...
- [ ] 4.4 `RF-13-39`, `RF-13-40`, `RN-13-15`, `RN-13-18`: tela de propostas — registro em texto
      na fila única e acompanhamento com o motivo em linguagem simples, sem prometer e-mail nem
      ponto.
- [ ] 4.5 `RF-13-41`: `AvisoDeColeta` e `ContextoDeDireitos` da App 07, no padrão das Apps 03,
...
- [ ] 4.6 `RF-13-35`, `RF-13-36`, `RF-13-38`: modo assistido — `App.tsx` passa a admitir sessão
      ...
      (design — decisão 7).
- [ ] 4.7 `RF-13-42`, `RN-13-17`: conferir, tela a tela, que nada na App 07 abre canal, campo de
      mensagem ou dado de contato de Apoiador, de terceiro ou de outro parente.

## 5. Testes

- [ ] 5.1 `tests/test_termos_rota.py` — os cenários de `catalogo-de-termos`: vigente com texto,
      ...
      inexistente.
- [ ] 5.2 `tests/test_auditoria_recorte.py` — os cenários de `auditoria`: escrita sobre uma
...
- [ ] 5.3 `tests/test_transparencia_rota.py` — os cenários de `transparencia-de-dados`: catálogo
      ...
      papel.
- [ ] 5.4 `tests/test_autorizacao_assistida.py` — os cenários de `consentimento` e de
...
- [ ] 5.5 `tests/test_fila_rota.py` — acrescentar os cenários do crédito: proposta de
...
- [ ] 5.6 `apps/app-07-responsaveis/src/transparencia/transparencia.test.tsx` — os cenários de
      tela da transparência, do histórico de acessos e do esclarecimento aberto da linha.
- [ ] 5.7 `apps/app-07-responsaveis/src/termos/termos.test.tsx` — texto vigente, leitura
      registrada, histórico das versões, a declaração da entrega de dados e a ausência de
      decisão separada sobre ela.
- [ ] 5.8 `apps/app-07-responsaveis/src/propostas/propostas.test.tsx` e
...
- [ ] 5.9 `apps/app-07-responsaveis/src/assistido/assistido.test.tsx` — sessão de Mestre que só
...
- [ ] 6.1 Registrar as decisões do fundador de 2026-09-01 e fechar a fatia: no **documento 03**,
