...
- [ ] 1.1 Criar `apps/app-05-guerreiro` no padrão das três aplicações existentes — `package.json`
...
- [ ] 1.2 Declarar a chave de aplicação e o endereço do núcleo por variável de ambiente do Vite,
...
- [ ] 1.3 Publicar o endereço da App 05: alvo de _hosting_ no `firebase.json` e no `.firebaserc`
...
- [ ] 2.1 Mover `apps/app-01-aula-presencial/src/biometria/biometria.ts` para `comum/biometria/`,
...
- [ ] 2.2 Replicar no `vite.config.ts` da App 05 o `alias` do `@vladmandic/human` para o build
...
- [ ] 3.1 Tela de entrada por nick e imagem, obtendo vivacidade e descritor no aparelho e
      ...
      nick existe.
- [ ] 3.2 Recusa em aparelho sem câmera, em linguagem de criança de 6 anos, sem código de erro
      nem termo técnico, dizendo o que fazer (`RF-05-02`).
- [ ] 3.3 Sessão assistida por Mestre ou Admin presente, nos dois caminhos — conferência que
      ...
      abre.
- [ ] 3.4 Encerramento ao sair e por inatividade, com aviso um minuto antes e opção de continuar
...
- [ ] 3.5 Guarda do aparelho compartilhado: nenhuma imagem nem descritor armazenados, nenhum
      ...
      aberta.
- [ ] 3.6 Testes da entrada e da sessão, cobrindo os cenários de `specs/area-do-guerreiro` e os
...
- [ ] 4.1 Acrescentar `valor_debitado` e `encerrada_em` a `ocorrencia_de_conduta`, gravando o
...
- [ ] 4.2 Estreitar o _trigger_ `trg_ocorrencia_de_conduta_somente_insercao` para admitir
...
- [ ] 4.3 Migração Alembic das duas colunas e da função do _trigger_, com preenchimento das
...
- [ ] 5.1 Regra do encerramento do ciclo: expurga o motivo de todas as ocorrências que ainda o
...
- [ ] 5.2 Rota do ato, restrita a Admin, herdando o registro de auditoria de toda escrita
      (`RF-02-99`, design — decisão 5). Verificar que Mestre, Apoiador, responsável e
      Guerreiro(a) recebem recusa.
- [ ] 5.3 O ranking público passa a somar de volta o `valor_debitado` das ocorrências com
...
- [ ] 5.4 Testes do ato e do ranking, cobrindo os cenários de `specs/fim-de-ciclo`,
...
- [ ] 6.1 Área do encerramento do ciclo na App 03, com confirmação explícita que enuncia os dois
      ...
      resultado do ato.
- [ ] 6.2 Testes da tela, cobrindo os cenários de `specs/aplicacao-de-gestao`: confirmação
