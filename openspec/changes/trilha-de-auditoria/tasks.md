...
- [ ] 1.1 Modelar `Auditoria` em `backend/src/nucleo/auditoria/modelo.py` com autor, papel do
...
- [ ] 1.2 Aplicar o par _listener_ de mapeador + trigger Postgres que recusa `UPDATE` e
...
- [ ] 1.3 Escrever a migração Alembic da tabela `auditoria`, indexada por autor e por momento.
      Verificável: a migração sobe e desce, e o banco volta a vazio
- [ ] 1.4 Adicionar `AuditoriaImutavel` ao catálogo de `erros.py`, no mesmo formato de
...
- [ ] 2.1 Gravar o `ContextoDaSessao` resolvido em `request.state` dentro de `exigir_persona`
...
- [ ] 2.2 Gravar o `ContextoDaChave` resolvido em `request.state` dentro de
...
- [ ] 3.1 Implementar o middleware ASGI em `backend/src/nucleo/auditoria/middleware.py`, que
...
- [ ] 3.2 Gravar o registro só quando a resposta final tiver `status_code < 400`, lendo persona,
      papel e origem de `request.state` (`RF-01-29`). Verificável: chamada de escrita recusada
      não gera registro
- [ ] 3.3 Derivar `ação` do método HTTP mais o nome da rota (`request.scope["route"].name`) e
...
- [ ] 3.4 Abrir uma sessão de banco própria do middleware, independente da sessão já fechada da
      rota, e fechá-la ao final (`design.md`). Verificável: teste que confirma a gravação
      mesmo quando a sessão da rota já terminou
- [ ] 3.5 Tornar a gravação _best-effort_: falha ao gravar o registro é registrada em log e não
...
- [ ] 3.6 Registrar o middleware em `principal.py`, para toda a aplicação. Verificável: uma
      escrita aceita em qualquer roteador já existente gera um registro de auditoria

## 4. Rota de consulta

- [ ] 4.1 Implementar `GET /v1/auditoria` em `backend/src/nucleo/auditoria/rotas.py`, exigindo o
...
- [ ] 4.2 Paginar e filtrar a consulta pelo `contrato_de_listagem`, com os filtros universais de
...
- [ ] 4.3 Incluir o roteador de auditoria em `principal.py` via `incluir_roteador_de_dados`.
      Verificável: a rota exige chave de aplicação como as demais

## 5. Verificação contra os critérios de aceite

- [ ] 5.1 Testar que toda escrita bem-sucedida em uma rota já existente (por exemplo,
...
- [ ] 5.2 Testar que uma chamada de leitura e uma chamada de escrita recusada não geram
      registro algum (`RF-01-29`)
- [ ] 5.3 Testar que Admin consulta a trilha paginada e filtrada, e que persona de outro papel
      recebe recusa por permissão (`RF-01-29`, PRD-01 §9)
- [ ] 5.4 Testar que a trilha não traz nenhuma escrita anterior à existência do middleware —
      confirmando por ausência, não reconstruindo nada (`design.md`, Non-Goals)
- [ ] 5.5 Testar que uma tentativa de alterar ou apagar um registro de auditoria já gravado é
      recusada, preservando o original (PRD-01 §8, "Imutabilidade")

## 6. Documentação

- [ ] 6.1 Conferir que a interpretação de `RF-01-29` registrada em `proposal.md` — a trilha
      ...
      decisão
- [ ] 6.2 Conferir que `docs/prds/index.md` e o documento 99 seguem corretos: o PRD-01 continua
...
- [ ] 6.3 Conferir que nenhum arquivo novo entrou em `docs/`. Verificável: a `nav` do
      `mkdocs.yml` não precisa de entrada nova
- [ ] 6.4 Rodar `npm run fix`, `npm run lint` e `mkdocs build --strict` antes de abrir o PR.
