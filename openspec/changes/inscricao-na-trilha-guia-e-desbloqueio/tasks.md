...
- [ ] 1.1 Gravar no documento 11 §2.2 que o **desbloqueio da missão é fato do Guerreiro(a) na
...
- [ ] 1.2 Mover no documento 09 §1 as duas decisões para "Já decididos", com data e origem
...
- [ ] 1.3 Aplicar as decisões no PRD-09 (§§6.4 e 9 — a rota do julgamento do desafio prático,
...
- [ ] 2.1 Criar o modelo `InscricaoNaTrilha` e a migração, com unicidade em
      (`guerreiro_id`, `trilha_id`) — design, decisão 1. Verificar que a migração sobe e
      desce limpa no banco de teste.
- [ ] 2.2 Implementar a regra da inscrição em `trilhas/regra.py`: exige trilha publicada
...
- [ ] 2.3 Expor `POST /v1/eu/trilhas/{id}/inscricao` e `GET /v1/eu/trilhas` — a lista traz as
...
- [ ] 3.1 Acrescentar o **desafio de desbloqueio** à `Missao` — coluna anulável, quiz ou
      ...
      publicável.
- [ ] 3.2 Expor `POST /v1/missoes/{id}/desbloqueio` para o **Mestre autor** declarar o
      desafio, com 403 para quem não é autor (`RF-09-26`). Verificar por
      `uv run pytest tests/test_desbloqueio_da_missao.py -x`.
- [ ] 3.3 Criar o modelo `DesbloqueioDaMissao` e a migração, com unicidade em
      (`guerreiro_id`, `missao_id`) — design, decisão 1.
- [ ] 3.4 Implementar a submissão do **quiz** pelo Guerreiro(a) inscrito: o núcleo afere,
...
- [ ] 3.5 Implementar o **desafio prático**: o Guerreiro(a) declara que cumpriu, o Mestre
...
- [ ] 3.6 Implementar a derivação do percurso em `trilhas/regra.py`: próxima missão pela
...
- [ ] 3.7 Expor `GET /v1/eu/progresso`: nível, obrigatórias desbloqueadas, quantas faltam
...
- [ ] 4.1 Em `pontuacao/regra.py`, fazer `avaliar_niveis` exigir **inscrição e** primeira
...
- [ ] 5.1 `tests/test_inscricao_na_trilha.py` — os cenários da capacidade
...
- [ ] 5.2 `tests/test_desbloqueio_da_missao.py` — os cenários da capacidade
...
- [ ] 5.3 `tests/test_percurso_da_trilha.py` — a derivação: próxima missão, bloqueio com
      ...
      fora do denominador.
- [ ] 5.4 Estender `tests/test_pontuacao.py` com os três cenários do nível 1 modificados —
...
- [ ] 6.1 Criar `apps/app-05-guerreiro/src/api/trilha.ts` com as chamadas do bloco, e
...
- [ ] 6.2 `EscolhaDoPoder.tsx` — catálogo de poderes do ciclo, trilhas publicadas do poder
      escolhido e inscrição; nenhuma ação de desinscrever (`RF-05-09`).
- [ ] 6.3 `GuiaDaTrilha.tsx` — abre na próxima missão com o que fazer e o que ela desbloqueia,
      alterna entre trilhas preservando o contexto, e sem inscrição leva à escolha do poder
      (`RF-05-08`, `RF-05-17`).
- [ ] 6.4 `Missao.tsx` — conteúdo na ordem do autor com crédito e licença, bibliografia com
      ...
      `RF-05-81`).
- [ ] 6.5 `Sondagem.tsx` — primeiro passo da trilha recém inscrita, dizendo que serve para o
      Mestre ajustar e que não muda o nível, sem apresentar acerto e erro como nota
      (`RF-05-72`, `RF-05-73`).
- [ ] 6.6 `DesafioDeDesbloqueio.tsx` — realiza o desafio; passando no quiz a seguinte abre na
...
- [ ] 6.7 `Progresso.tsx` — nível e quantas obrigatórias faltam, pontos, badges e recompensas
      ...
      `RN-05-06`).
- [ ] 6.8 Testes do bloco — um arquivo por tela, cobrindo os cenários da capacidade
      `area-do-guerreiro` desta fatia, no padrão dos testes de `coleta/` e `carteira/`.

## 7. App 09 — a bancada do desafio

- [ ] 7.1 Tela de autoria do desafio de desbloqueio na missão, com quiz ou desafio prático,
      sinalizando a missão que ainda não o tem e sem oferecer edição em trilha alheia
      (`RF-09-26`).
- [ ] 7.2 Bancada dos **desafios práticos a julgar**: declarações não julgadas das próprias
      ...
      `RF-05-14`).
- [ ] 7.3 Testes das duas telas, cobrindo os cenários da capacidade `area-do-mestre` desta
      fatia.

## 8. Fechamento da documentação

- [ ] 8.1 Acrescentar a linha desta fatia à seção do PRD-05 em `docs/prds/index.md` — uma
