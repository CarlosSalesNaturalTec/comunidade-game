...
- [x] 1.1 Em `src/recursos/api.ts`, acrescentar `cadastrarTipoDeRecurso` ao lado da
...
- [x] 1.2 Criar `src/catalogos/api.ts` com `listarTiposDeColeta` e `cadastrarTipoDeColeta`. A
...
- [x] 2.1 Criar `src/catalogos/TelaDeCatalogos.tsx` reunindo os dois catálogos, no molde de
      ...
      design — decisão 1).
- [x] 2.2 Registrar a área em `src/App.tsx`, ao lado de `Poderes`, com chave `catalogos` e rótulo
...
- [x] 3.1 Criar `src/catalogos/ListaDeTiposDeRecurso.tsx` — lista densa com nome, natureza,
...
- [x] 3.2 Criar `src/catalogos/FormularioDeTipoDeRecurso.tsx` — nome, natureza em escolha entre
...
- [x] 3.3 Apresentar a recusa do núcleo em linguagem simples, no campo que a originou — campo em
...
- [x] 4.1 Criar `src/catalogos/ListaDeTiposDeColeta.tsx` — lista densa com nome, forma de
      ...
      (`RF-02-108`, `RF-08-06`).
- [x] 4.2 Criar `src/catalogos/FormularioDeTipoDeColeta.tsx` — nome e forma de registro em
...
- [x] 4.3 Apresentar a recusa do núcleo em linguagem simples, no campo que a originou — campo em
      falta, forma fora das três, faixa invertida. Verificar pelo cenário "A recusa da faixa é
      apresentada no campo" (`RF-02-108`).

## 5. Testes

- [x] 5.1 Criar `src/catalogos/catalogos.test.tsx` cobrindo os vinte cenários da spec do delta,
...
- [x] 5.2 Criar `src/catalogos/api.test.ts` cobrindo as três funções de API que a fatia estreia:
      a rota e o método de `cadastrarTipoDeRecurso` e `cadastrarTipoDeColeta`, e o laço da
      paginação de `listarTodosOsTiposDeColeta`. O teste de tela dubla os módulos inteiros, então
      sem este arquivo nada executaria o corpo das três — inclusive o laço que a tarefa 1.2 manda
      verificar. `recursos.test.tsx` não é tocado: `cadastrarTipoDeRecurso` não é consumida por
      nenhuma tela daquela pasta (`RF-02-107`, `RF-02-108`).

## 6. Documentação

- [x] 6.1 No PRD-02: acrescentar `RF-02-107` e `RF-02-108` à tabela da §6.1, as quatro rotas
...
- [x] 6.2 Marcar a fatia 19 como implementada em `openspec/cronograma-de-fatias.md`, com o slug
