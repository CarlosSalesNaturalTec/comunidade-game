...
- [ ] 1.1 Criar `nucleo/ocorrencias_de_conduta/modelo.py` com `OcorrenciaDeConduta` —
...
- [ ] 1.2 Criar `nucleo/ocorrencias_de_conduta/regra.py` com `lancar_ocorrencia_de_conduta`:
...
- [ ] 1.3 Ligar o débito: a regra chama `debitar_ponto_regular` com a trilha derivada, na mesma
...
- [ ] 1.4 Fechar a imutabilidade fora do ORM, no padrão de `Lancamento`: alterar ou remover
      ocorrência gravada é recusado (`RF-09-46`, `RN-01-52`).

## 2. As três portas HTTP

- [ ] 2.1 Extrair de `aulas/rotas.py` para `resultados/regra.py` a montagem comum de
...
- [ ] 2.2 Criar `nucleo/resultados/rotas.py` com `POST /atividades/{id}/lancamentos` — Mestre
...
- [ ] 2.3 Acrescentar a `aulas/rotas.py` o `POST /aulas/{id}/presencas`, exigindo
      ...
      `RF-01-20`, `RF-01-17`).
- [ ] 2.4 Acrescentar a `aulas/rotas.py` o `GET /minhas-turmas`, exigindo `Operacao.suas_turmas`,
      ...
      `RF-09-73`, `RN-09-08`).
- [ ] 2.5 Criar `nucleo/ocorrencias_de_conduta/rotas.py` com a rota de lançamento e registrar os
      roteadores novos em `principal.py`; verificar que as rotas aparecem no OpenAPI sob `/v1`
      (`RF-09-46`).

## 3. Testes do núcleo

- [ ] 3.1 `tests/test_ocorrencia_de_conduta.py` — cenários da spec `ocorrencia-de-conduta`:
      ...
      motivo apagado.
- [ ] 3.2 `tests/test_lancamento_por_atividade.py` — cenários da spec `resultado-de-atividade`:
      ...
      editar lançamento.
- [ ] 3.3 `tests/test_presenca_do_mestre.py` e cobertura de `GET /minhas-turmas` — cenários das
...
- [ ] 4.1 Criar a área Minhas turmas com a lista de aulas e atividades separadas por formato,
      exibindo Guerreiros e Guerreiras por nick e avatar (`RF-09-42`, `RF-09-73`, `RN-09-18`).
- [ ] 4.2 Criar a tela de lançamento com seleção de participantes e desfecho de cada um, envio
      num ato só e recusa apresentada em linguagem simples (`RF-09-43`, `RF-09-44`, `RF-09-74`,
      `RN-09-16`).
- [ ] 4.3 Criar a confirmação de presença e a tela da ocorrência de conduta — escolha da
...
- [ ] 4.4 Cobrir as três telas em Vitest, nos cenários da spec `area-do-mestre`.

## 5. Documentação

- [ ] 5.1 Atualizar `docs/09-topicos-em-aberto-e-sugestoes.md` §1 com a pendência do fim de
