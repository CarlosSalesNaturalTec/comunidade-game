...
- [ ] 1.1 Em `backend/src/nucleo/quiz/regra.py`, escrever `partidas_da_aula`, que exige
...
- [ ] 1.2 Em `backend/src/nucleo/quiz/rotas.py`, expor `GET /aulas/{id}/partidas` sobre
...
- [ ] 2.1 Ampliar `pergunta_para_equipe` e `PerguntaParaEquipeSaida` para, quando a pergunta no
...
- [ ] 2.2 Corrigir a spec consolidada de `quiz-ao-vivo` onde ela promete que o aparelho da equipe
...
- [ ] 3.1 Em `backend/tests/test_quiz.py`, cobrir a descoberta e o resultado: equipe derivada com
...
- [ ] 3.2 Em `backend/tests/test_quiz_rota.py`, cobrir as duas portas: `GET /aulas/{id}/partidas`
      pelo Guerreiro(a) e o 403 de Mestre e Admin nela, e o 403 do Guerreiro(a) em
      `GET /partidas-de-quiz/{id}` (`RF-04-41`, `RF-04-42`).

## 4. App 01 — cliente e caminho

- [ ] 4.1 Em `apps/app-01-aula-presencial/src/api/`, escrever o cliente do quiz — partidas da
...
- [ ] 4.2 Em `src/inicio/TelaInicial.tsx`, acrescentar o caminho do quiz, sempre presente na
...
- [ ] 5.1 Criar `src/quiz/TelaDaPartida.tsx`: abertura relendo as partidas da aula, equipe vinda
...
- [ ] 5.2 Implementar a sondagem a cada 2 segundos da pergunta no ar, com a espera entre
      perguntas, o aviso de perda de contato que mantém a tela e a volta à pergunta corrente
      (`RF-04-41`, `RF-04-58`; design — decisão 7).
- [ ] 5.3 Implementar o envio da resposta — uma por equipe e pergunta, segunda tentativa recusada
...
- [ ] 5.4 Implementar a exibição do resultado liberado — alternativa correta, se a equipe acertou
      e qual chegou primeiro —, oculto antes da liberação e sem pontuação da partida em tela
      alguma (`RF-04-44`).
- [ ] 5.5 Implementar o comportamento sem rede: pergunta carregada segue legível, resposta
      apresentada como indisponível e nada enfileirado para envio posterior (`RF-04-58`;
      design — decisão 6).

## 6. App 01 — testes

- [ ] 6.1 Em `src/quiz/quiz.test.tsx`, cobrir a abertura e a partida: entrada sem sessão levando
...
- [ ] 6.2 No mesmo arquivo, cobrir resposta e resultado: escolha mantida em tela, segunda
      ...
      `RF-04-58`).
- [ ] 6.3 Em `src/inicio/inicio.test.tsx`, cobrir o quarto caminho: presente com o momento de
      troca fechado e levando à entrada do Guerreiro(a) sem sessão aberta (`RF-04-01`,
      `RF-04-41`).

## 7. Documentação

- [ ] 7.1 Corrigir no `docs/prds/prd-04-aula-presencial.md` o que contraria o documento 05 §5: a
      §3.2 deixa de excluir o vínculo aparelho–equipe como sendo da App 03, a jornada 5.9 item 1
      deixa de atribuir o vínculo ao Mestre na App 03 e o `RF-04-42` deixa de falar em aparelho
