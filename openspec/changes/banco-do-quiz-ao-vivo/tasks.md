...
- [ ] 1.1 Acrescentar `missao_id` e `trilha_id` a `PerguntaDeQuiz` em
...
- [ ] 1.2 Criar a revisão Alembic que acrescenta as duas colunas com índice em cada uma, sem
...
- [ ] 2.1 Fazer `cadastrar_pergunta` receber a missão, recusar cadastro sem ela com
...
- [ ] 2.2 Escrever `perguntas_do_mestre` em `backend/src/nucleo/quiz/regra.py` — perguntas do
...
- [ ] 3.1 Criar `backend/src/nucleo/quiz/rotas.py` com `POST /v1/perguntas` e
...
- [ ] 4.1 Estender `backend/tests/test_quiz.py` com o vínculo obrigatório: os quatro cenários do
...
- [ ] 4.2 Cobrir a leitura do banco: Mestre lê o próprio, filtro por missão, filtro por trilha
...
- [ ] 5.1 Criar `apps/app-09-mestre/src/quiz/api.ts` com o cliente das duas rotas, no padrão de
      `trilhas/api.ts` e `turmas/api.ts` (`RF-09-36`, `RF-09-40`). Verificar: tipos compilam e
      o `vitest` do módulo passa.
- [ ] 5.2 Criar o formulário de cadastro — enunciado, as quatro alternativas, a indicação da
...
- [ ] 5.3 Criar a lista do banco com os filtros por trilha e por missão (`RF-09-40`).
- [ ] 5.4 Ligar a área "Banco do Quiz" à navegação da App 09, ao lado de Minhas turmas e
...
- [ ] 6.1 Gravar a decisão do fundador de 2026-08-23 e o que a fatia entregou: riscar
