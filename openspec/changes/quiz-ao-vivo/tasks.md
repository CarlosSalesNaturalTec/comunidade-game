...
- [ ] 1.1 Criar o modelo `PerguntaDeQuiz` em `backend/src/nucleo/quiz/modelo.py`, com autoria
...
- [ ] 1.2 Criar o modelo `PartidaDeQuiz` com `aula_id`, `atividade_id`, situação fechada em
...
- [ ] 1.3 Criar o modelo `EquipeNaPartida`, a lista de equipes disputantes fixada na abertura,
      com `UniqueConstraint` por `(partida, equipe)` (`RF-01-39`, design — decisão 4).
- [ ] 1.4 Criar o modelo `RespostaDeQuiz` com partida, pergunta, equipe, alternativa escolhida,
      `momento_de_chegada` carimbado pelo servidor e a marca de pergunta anulada, com
      `UniqueConstraint` por `(partida, pergunta, equipe)` — é ela que torna o reenvio
      inofensivo (`RF-01-36`, design — Migration Plan).
- [ ] 1.5 Escrever a décima migração Alembic na ordem do design — Migration Plan: criar
      `pergunta_de_quiz`, `partida_de_quiz`, `equipe_na_partida` e `resposta_de_quiz`, com o
      `downgrade` derrubando as quatro. Nenhuma tabela existente é alterada.

## 2. Pergunta de quiz

- [ ] 2.1 Implementar o cadastro da pergunta restrito ao Mestre e ao Admin pela matriz de
      permissões, gravando autoria, data e hora (`RF-01-36`, `RF-01-03`, `RF-01-16`).
- [ ] 2.2 Implementar a recusa com 422, indicando o campo em falta, da pergunta com número de
      alternativas diferente de quatro e da pergunta sem alternativa correta declarada
      (`RF-01-36`, `RF-01-27`).
- [ ] 2.3 Verificar: pergunta com quatro alternativas e uma correta grava com autoria; pergunta
...
- [ ] 3.1 Implementar a abertura da partida restrita à operação
      ...
      `RF-01-03`).
- [ ] 3.2 Implementar a recusa com 422 da partida sem atividade declarada e da partida cuja
      atividade não tenha natureza de competição ao vivo (`RF-01-21`, specs — partida sobre
      atividade da trilha).
- [ ] 3.3 Implementar a materialização das equipes disputantes na abertura, recusando com 422
      quando a interseção de integrantes entre quaisquer duas delas não for vazia (`RF-01-39`,
      design — decisão 4).
- [ ] 3.4 Implementar a recusa com 422 da equipe da trilha como disputante: só equipe da aula,
      e da aula da partida (`RF-01-37`, `RF-01-39`, `RF-01-18`).
- [ ] 3.5 Verificar: Mestre da aula abre com autoria gravada; Mestre de outra aula recebe 403;
...
- [ ] 4.1 Implementar o registro da resposta pela operação `resposta_de_quiz_da_equipe` do
      Guerreiro(a), gravando a equipe, a alternativa e a autoria de quem enviou (`RF-01-36`,
      `RF-01-03`, `RF-01-16`).
- [ ] 4.2 Implementar o carimbo de `momento_de_chegada` **pelo núcleo**, ignorando qualquer
      instante declarado pelo chamador (`RF-01-36`, design — decisão 2).
- [ ] 4.3 Implementar a idempotência do reenvio: achando resposta já gravada para
...
- [ ] 4.4 Implementar a recusa com 422 da resposta a pergunta de partida já encerrada e da
      resposta de equipe que não disputa aquela partida (`RF-01-36`, `RF-01-39`).
- [ ] 4.5 Verificar: resposta grava equipe, alternativa e momento carimbado; momento declarado
...
- [ ] 5.1 Implementar a anulação de pergunta pelo Mestre que conduz a partida ou por um Admin,
      com autoria, data e hora gravadas (`RF-01-03`, `RF-01-16`, documento 05 §5).
- [ ] 5.2 Implementar a recusa com 422 da anulação em partida já encerrada — é o que mantém
      `RN-01-38`, já que a partida encerrada creditou (`RN-01-38`, design — decisão 1).
- [ ] 5.3 Implementar a marca de pergunta anulada nas respostas já gravadas, que seguem
      consultáveis (`RF-01-36`).
- [ ] 5.4 Verificar: pergunta anulada não credita a nenhuma equipe; nenhum saldo de ponto
      regular diminui em consequência da anulação; respostas seguem consultáveis marcadas;
      anulação depois do encerramento recebe 422 (`RN-01-38`).

## 6. Apuração e crédito no encerramento

- [ ] 6.1 Implementar o encerramento da partida aceito apenas na transição `aberta → encerrada`,
      recusando com 422 a segunda chamada sem tocar em `PontoRegular` (design — decisão 6).
- [ ] 6.2 Implementar a apuração por equipe sobre as perguntas **não anuladas**: 1 por acerto e
...
- [ ] 6.3 Implementar o teto de 10 pontos por partida, aplicado **antes** do crédito
      (`RF-01-21`, documento 11 §5).
- [ ] 6.4 Implementar o crédito do valor apurado **integral a cada integrante** da equipe, na
      ...
      decisão 5).
- [ ] 6.5 Implementar a apuração parcial como **leitura**, para o placar ao vivo da App 03, sem
      creditar nada (`RF-01-21`, design — Risks).
- [ ] 6.6 Verificar: acerto credita 1 por integrante; a primeira a acertar recebe 2 e a outra 1;
...
- [ ] 6.7 Verificar que o gatilho de `PontoRegular` continua recusando débito com o quiz no
...
- [ ] 8.1 Conferir que as duas decisões seguem registradas onde foram gravadas antes desta
