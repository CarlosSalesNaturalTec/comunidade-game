...
- [ ] 2.1 Criar o arquivo de configuração de CORS do bucket, versionado no repositório,
...
- [ ] 2.2 Acrescentar ao `backend/README.md` §2 a linha que aplica esse arquivo ao bucket, na
...
- [ ] 3.1 Fazer `abrir_sessao` do adaptador de nuvem declarar, ao abrir a sessão retomável, a
...
- [ ] 3.2 Levar a origem da requisição até a abertura da sessão nas rotas que a abrem — imagem
...
- [ ] 3.3 Cobrir em `backend/tests/test_armazenamento_porta.py` que o adaptador de nuvem abre a
...
- [ ] 4.1 Fazer `enviarParteComProgresso` em `comum/api/cliente.ts` entregar a recusa que traga
      ...
      decisão 5).
- [ ] 4.2 Cobrir em `comum/api/cliente.test.ts` os dois desfechos do envio de uma parte: a
...
- [ ] 4.3 Conferir que as telas que anexam arquivo — a imagem da pergunta na App 09, o conteúdo
