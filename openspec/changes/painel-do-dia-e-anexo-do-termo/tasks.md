...
- [ ] 1.1 Acrescentar à equipe da aula a **atividade corrente**, sobrescrita a cada declaração e
...
- [ ] 1.2 Implementar a regra de declaração da escolha: só integrante daquela equipe declara
...
- [ ] 1.3 Dar porta HTTP à declaração, sob a sessão do Guerreiro(a), e servir a escolha corrente
...
- [ ] 2.1 Criar o `AnexoDoTermo` como registro próprio que aponta para o consentimento, com quem
...
- [ ] 2.2 Implementar a regra do anexo pela `PortaDeArmazenamento`, no caminho simples do
...
- [ ] 2.3 Expor `POST /v1/consentimentos/{id}/anexo` restrita a Admin (403 para as demais
      personas), com a digitalização fora de rota pública e a escrita na trilha de auditoria
      (`RF-02-68`, `RN-02-21`)

## 3. Painel do dia no núcleo

- [ ] 3.1 Criar `backend/src/nucleo/painel_do_dia/` como módulo só de leitura, sem modelo e sem
...
- [ ] 3.2 Compor as presenças com o modo de comprovação e quem confirmou, e derivar **quem
      aguarda aparelho** — presente sem equipe naquela aula — em consulta, sem entidade nem fila
      (`RF-02-41`, `RF-02-43`)
- [ ] 3.3 Compor as equipes com integrantes e a missão da atividade corrente declarada, deixando
      em branco a equipe que não declarou (`RF-02-42`, `RF-02-08`)
- [ ] 3.4 Compor a atividade prevista, as reservas da aula e o saldo dos tipos de recurso do ponto
      de apoio, tomando os tipos do catálogo configurável e sem fixar tipo algum em código
      (`RF-02-44`, `RF-02-45`, `RN-07-36`)
- [ ] 3.5 Compor os lançamentos pendentes do encontro — a atividade realizada ainda não lançada e
...
- [ ] 3.6 Expor `GET /v1/painel-do-dia` para Admin e Mestre, com o Mestre limitado às comunidades
      dele e 403 para Guerreiro(a), responsável e Apoiador, devolvendo a criança só por nick e
      avatar (`RN-02-20`, `RN-02-22`)

## 4. Testes do núcleo

- [ ] 4.1 `tests/test_equipe_escolha.py` — declaração, troca que substitui, atividade fora da
      programação (422), não integrante (403), leitura que não grava escolha e escolha que não
      sobrevive à aula (`RF-02-42`, `RF-04-35`)
- [ ] 4.2 `tests/test_consentimento_anexo.py` — anexo aceito nos três formatos, formato recusado
...
- [ ] 4.3 `tests/test_painel_do_dia.py` — encontro em andamento numa leitura, encontro vazio fora
...
- [ ] 4.4 Cobrir no mesmo arquivo os critérios de aceite do PRD-02 §12 que esta fatia atende:
...
- [ ] 5.1 Na tela da programação, oferecer a escolha da atividade, mostrar qual está corrente e
      declará-la ao núcleo, sem eleger nenhuma por conta própria quando há mais de uma
      (`RF-04-35`, `RF-02-42`)
- [ ] 5.2 Sem rede, manter legível o conteúdo já carregado, dizer que a escolha está indisponível
      e não enfileirar a declaração, como já vale para a resposta de quiz (`RF-04-58`)
- [ ] 5.3 Testes da tela da programação cobrindo escolha, troca, programação com duas atividades
      sem corrente e o comportamento sem rede (`RF-04-35`, `RF-04-58`)

## 6. App 03 — área Painel do dia

- [ ] 6.1 Criar `apps/app-03-gestao/src/painel-do-dia/` com a tela de leitura do encontro:
...
- [ ] 6.2 Manter a tela atualizada por sondagem, no padrão da condução da partida, avisando a
      queda de rede sem apagar o que carregou e retomando sozinha (`RF-02-48`)
- [ ] 6.3 Oferecer ao Admin, a partir da pendência, o envio da digitalização em PDF, JPG ou PNG,
      com a recusa explicada em linguagem simples e sem oferecer o caminho ao Mestre
      (`RF-02-68`, `RN-02-20`)
- [ ] 6.4 Garantir que a tela apresenta a criança só por nick e avatar, sem imagem real
      (`RN-02-22`)
- [ ] 6.5 Testes da área cobrindo a tela com encontro, a frase de "não há encontro em andamento",
...
- [ ] 7.1 Em Minhas turmas, oferecer o caminho para o painel do dia da App 03 apenas na aula em
...
- [ ] 8.1 Corrigir no PRD-02 as três respostas do fundador de 2026-08-25: o `RF-02-43` como
