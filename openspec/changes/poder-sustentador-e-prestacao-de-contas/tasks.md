...
- [ ] 1.1 Acrescentar `aula_id` anulável e indexado a `Lancamento`, com a migração Alembic que
...
- [ ] 1.2 Passar a aula pela emissão do débito: `livro_razao.regra.lancar_debito` aceita e grava
...
- [ ] 2.1 Criar `src/nucleo/poder_sustentador/regra.py` com a derivação do Poder Sustentador pela
      cadeia aporte → crédito → ajuste, agregada em SQL e sem total guardado à parte
      (`RF-07-10`, `RN-07-15`, design — Decisions 1, 5)
- [ ] 2.2 Acrescentar à mesma `regra.py` a contagem de absorções, derivada de `Aporte` com
      `forma = absorcao` e sem tocar no livro-razão (`RF-07-26`, `RN-07-19`, design —
      Decisions 2)
- [ ] 2.3 Criar `src/nucleo/poder_sustentador/rotas.py` com a rota
...
- [ ] 2.4 Acrescentar `GET /meus-aportes`, restrita ao Apoiador em sessão, devolvendo os aportes
      dele e o Poder Sustentador dele, em moedas e sem o valor de origem em reais (`RF-07-17`,
      `RN-07-05`)

## 3. Prestação de contas pública

- [ ] 3.1 Criar `src/nucleo/prestacao_de_contas/regra.py` com o movimentado total e por provedor,
...
- [ ] 3.2 Acrescentar à mesma `regra.py` o consumo por aula e por comunidade, somado dos débitos
...
- [ ] 3.3 Criar `src/nucleo/prestacao_de_contas/rotas.py` com `GET /prestacao-de-contas` e
      ...
      design — Decisions 7)
- [ ] 3.4 Registrar os dois roteadores em `principal.py` por `incluir_roteador_de_dados`, que já
      exige a chave de aplicação e a cota de leitura (`RF-01-02`, `RN-01-32`)

## 4. Testes

- [ ] 4.1 Ampliar `tests/test_lancamento.py` com os cenários do delta de `livro-razao`: a baixa
      ...
      `RF-07-16`, `RN-07-15`)
- [ ] 4.2 Criar `tests/test_poder_sustentador.py` com os cenários de derivação da spec: o aporte
...
- [ ] 4.3 Criar `tests/test_poder_sustentador_rota.py`: leitura pública sem token, 401 sem chave,
...
- [ ] 4.4 Criar `tests/test_prestacao_de_contas.py`: o aporte novo aparece na leitura seguinte
...
- [ ] 4.5 Criar `tests/test_prestacao_de_contas_rota.py`: visitante sem persona lê, 401 sem
...
- [ ] 5.1 Atualizar `docs/prds/index.md` com o que a quinta fatia do PRD-07 entregou. A §9 do
