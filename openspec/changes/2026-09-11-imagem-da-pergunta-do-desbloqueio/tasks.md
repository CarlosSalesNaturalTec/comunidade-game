...
- [ ] 1.1 Acrescentar a `PerguntaDoDesbloqueio` as colunas `imagem_referencia`, `imagem_tipo`,
...
- [ ] 1.2 Escrever a revisão do Alembic que cria as quatro colunas e troca a restrição pelo
...
- [ ] 2.1 Fazer `perguntas_do_desbloqueio` servir só as vigentes e
...
- [ ] 2.2 Aceitar em `declarar_desafio_de_desbloqueio` a `imagem_referencia` de cada pergunta,
...
- [ ] 2.3 Escrever `abrir_envio_da_imagem_da_pergunta` e `confirmar_envio_da_imagem_da_pergunta`
      ...
      (`RF-09-119`, `RF-09-115`).
- [ ] 2.4 Escrever a leitura dos bytes da imagem, autorizada ao Mestre autor da trilha e ao
      Guerreiro(a) inscrito nela, com 403 aos demais e 404 na pergunta sem imagem
      (`RF-09-119`, design — decisão 5).
- [ ] 2.5 Levar a referência da imagem na cópia de `duplicar_trilha`, sem copiar bytes
      (`RF-09-13`, `RF-09-119`).

## 3. Núcleo — rotas

- [ ] 3.1 Acrescentar `imagem_referencia` à entrada da pergunta em
      ...
      (`RF-09-119`, `RF-09-118`).
- [ ] 3.2 Abrir `POST` e `PATCH /v1/perguntas-do-desbloqueio/{id}/imagem` para o envio e a
      confirmação, e `GET /v1/perguntas-do-desbloqueio/{id}/imagem` para os bytes, com o tipo
      do envio (`RF-09-119`, design — decisão 5).

## 4. Testes do núcleo

- [ ] 4.1 Em `test_desbloqueio_da_missao.py`, cobrir o envio da imagem: anexar e confirmar,
      ...
      imagem opcional".
- [ ] 4.2 Cobrir a leitura dos bytes: Guerreiro(a) inscrito e Mestre autor recebem, não
...
- [ ] 4.3 Cobrir a redeclaração: referência de volta conserva a imagem, referência omitida a
...
- [ ] 4.4 Em `test_duplicacao_de_trilha.py`, cobrir que a cópia traz a imagem das perguntas pela
      referência, sem copiar bytes.

## 5. Telas

- [ ] 5.1 Acrescentar ao `comum/api` a leitura de bytes do núcleo, que devolve `Blob` com a
      chave da aplicação e a credencial da persona nos cabeçalhos (design — decisão 6);
      verificar pelo teste do cliente.
- [ ] 5.2 Na App 09, dar a cada pergunta do `DesafioDeDesbloqueio.tsx` anexar, trocar e remover
      ...
      (`RF-09-119`).
- [ ] 5.3 Na App 05, exibir a imagem da pergunta antes das alternativas, com texto alternativo,
      e manter a pergunta respondível quando a imagem não carrega (`RF-09-119`, `RF-05-89`).
- [ ] 5.4 Cobrir as duas telas com os cenários dos deltas de `area-do-mestre` e
...
- [ ] 6.1 Registrar as três decisões do fundador de 2026-09-11 — imagem preservada na
