...
- [ ] 1.1 `responsaveis/regra.py` e `responsaveis/rotas.py`: `cadastrar_responsavel` passa a
...
- [ ] 1.2 `permissoes.py`: operação de escrita nova do Mestre para o **registro do termo impresso
...
- [ ] 2.1 `consentimentos/rotas.py` (novo): `POST /v1/consentimentos` sob a operação da tarefa
...
- [ ] 2.2 `principal.py`: registrar o roteador de consentimentos sob `/v1`. Verificação: a rota
      aparece no OpenAPI e responde fora de 404.

## 3. App 01 — a fronteira da Human

- [ ] 3.1 `package.json` da App 01: dependência da Human, com os modelos carregados **sob
...
- [ ] 3.2 Módulo único de biometria da App 01, expondo provar vivacidade e gerar descritor, e
...
- [ ] 3.3 Clientes de API da App 01 para responsável, vínculo, consentimento e descritor, no
      desenho de `api/guerreiros.ts` (`RF-04-12`, `RF-04-13`, `RF-04-60`). Verificação: teste de
      cada cliente contra o contrato da §9 do PRD-04.

## 4. App 01 — o fluxo da jornada 5.2

- [ ] 4.1 Tela do responsável no onboarding: nome e grau de parentesco, encadeada depois do
...
- [ ] 4.2 Tela do termo e confirmação da assinatura pelo Mestre ou Admin como testemunha, antes
      de qualquer captura (`RF-04-11`, `RF-04-12`, `RN-04-07`). Verificação: a câmera não abre
      enquanto a confirmação não é dada.
- [ ] 4.3 Tela de captura: vivacidade, descritor e envio, com a recusa de captura sem
...
- [ ] 4.4 Detecção de câmera: sem ela, o onboarding **continua** pelo caminho sem imagem, com
      ...
      e sem imagem.
- [ ] 4.5 Retomada por passo concluído: o atendimento guarda `guerreiro_id` e `responsavel_id` e
...
- [ ] 5.1 `tests/test_consentimentos.py` (novo): os cenários da spec `consentimento` — registro
      com testemunha e fuso, 403 por papel, 422 por tipo, recusa sem vínculo, versão vinda da
      configuração e não do corpo, versão trocada que não reescreve o passado, e a biometria que
      não nasce da autorização de divulgação.
- [ ] 5.2 `tests/test_responsaveis.py`: os cenários novos da spec `responsavel-e-vinculo` — nome
      gravado, 422 sem nome, e nenhuma credencial nascendo com o cadastro.
- [ ] 5.3 Testes da App 01 para os cenários novos da spec `aplicacao-da-aula-presencial`: o
...
- [ ] 6.1 Documento 09 §1: mover para "Já decididos" as três decisões do fundador de 2026-08-24 —
...
- [ ] 6.2 PRDs afetados pelas decisões: matriz do **PRD-01 §4** ganha a operação do testemunho;
...
- [ ] 6.3 `docs/prds/index.md`: a narrativa da terceira fatia do PRD-04, dizendo o que ficou
