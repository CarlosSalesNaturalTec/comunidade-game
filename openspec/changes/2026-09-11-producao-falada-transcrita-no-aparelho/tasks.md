...
- [ ] 1.1 `producoes/regra.py`: `_conferir_forma_unica` passa a conferir o conteúdo esperado de
      ...
      sem ramo por porta.
- [ ] 1.2 `producoes/regra.py`: em `_ler_e_montar_producao`, o desvio da indisponibilidade
...
- [ ] 1.3 `producoes/rotas.py`: `arquivo` segue `UploadFile | None` nas duas rotas, agora só
...
- [ ] 1.4 `producoes/local.py`: a forma `audio` passa pelo caminho do texto — devolve o texto
...
- [ ] 1.5 `producoes/nuvem.py`: montar passada só de texto para `texto` e `audio`, com
...
- [ ] 2.1 `apps/app-01-aula-presencial/src/trilhas/EntregaDaProducao.tsx`: trocar
...
- [ ] 2.2 `EntregaDaProducao.tsx` da App 01: quando `existeTranscricaoDeFala()` for falso, não
...
- [ ] 2.3 `apps/app-05-guerreiro/src/trilha/EntregaDaProducao.tsx`: a forma "Gravar a fala"
      ...
      6).
- [ ] 2.4 `EntregaDaProducao.tsx` da App 05: o aviso antes da fala passa a dizer, em linguagem
...
- [ ] 2.5 `apps/app-01-aula-presencial/src/api/producao.ts` e
...
- [ ] 2.6 `apps/app-05-guerreiro/src/coleta/RegistrarMedicao.tsx`: trocar a cópia própria da
      ...
      `RN-05-32`, design — decisão 7). Verificação: nenhum `SpeechRecognition` sobra em
      `apps/app-05-guerreiro/src/`.

## 3. Testes

- [ ] 3.1 `backend/tests/test_producao_da_missao.py`: trocar as entregas por áudio com
...
- [ ] 3.2 `backend/tests/test_producao_da_missao_porta.py`: adequar os testes dos dois
      ...
      `producao-da-missao`).
- [ ] 3.3 `apps/app-05-guerreiro/src/trilha/EntregaDaProducao.test.tsx`: substituir os cenários
...
- [ ] 3.4 `apps/app-01-aula-presencial/src/trilhas/trilhas.test.tsx`: acrescentar aos cenários
...
- [ ] 4.1 `openspec/cronograma-de-fatias.md`: marcar a fatia do PRD-05 como `implementado` com
      ...
      Changes).
- [ ] 4.2 `docs/03-plataforma-e-arquitetura.md` §12.2: a linha "Foto e áudio da produção do
