...
- [ ] 1.1 `ponto_extra/rotas.py` (novo): `GET /v1/eu/pontos-extras`, sob `exigir_persona`,
...
- [ ] 1.2 Registrar o roteador em `principal.py`, no mesmo padrão dos demais módulos
      (`RF-04-51`). Verificação: a rota aparece no OpenAPI e responde sob `/v1`.

## 2. App 01 — os clientes das três rotas

- [ ] 2.1 `api/pontosExtras.ts` (novo): cliente de `GET /v1/eu/pontos-extras`, sempre com o
...
- [ ] 2.2 `api/catalogoAvulso.ts` (novo): cliente de `GET /v1/catalogo-avulso`, sem declarar
...
- [ ] 2.3 `api/trocas.ts` (novo): cliente de `POST /v1/aulas/{id}/trocas`, com o **token de
...
- [ ] 3.1 `AparelhoDaAula.tsx`: estado do momento de troca, em memória e nascendo fechado, com a
...
- [ ] 3.2 A abertura faz a sondagem do catálogo e só abre com resposta do núcleo; falha de rede
...
- [ ] 3.3 `TelaInicial.tsx`: terceiro caminho, visível apenas com o momento aberto, levando à
...
- [ ] 4.1 `troca/TelaDeTroca.tsx` (novo): lê o catálogo e o saldo sob o token do Guerreiro(a) e
...
- [ ] 4.2 A mesma tela oculta o item de **estoque zero** e recusa, antes de enviar, o item cujo
...
- [ ] 4.3 A confirmação da entrega pelo Mestre envia a troca num único `POST`, com o
...
- [ ] 4.4 O 422 do núcleo — saldo, estoque ou lastro mudados depois da leitura — é apresentado em
...
- [ ] 5.1 `tests/` do núcleo, ponto extra: os cenários da spec de `ponto-extra` — as duas contas
...
- [ ] 5.2 `troca.test.tsx` da App 01: os cenários da spec de `aplicacao-da-aula-presencial` para
...
- [ ] 5.3 No mesmo arquivo, os cenários da tela: catálogo com preço e estoque, saldo sem
      ...
      `RF-04-56`, PRD-04 §12).
- [ ] 5.4 `inicio.test.tsx`: o terceiro caminho aparece com o momento aberto e some quando ele
      fecha, e leva à entrada por nick e imagem, nunca ao cadastro (`RF-04-01`, `RF-04-49`).

## 6. Documentação

- [ ] 6.1 No mesmo PR: PRD-04 §9 com as três rotas do §6.3, que a tabela não tinha, e §13 com as
