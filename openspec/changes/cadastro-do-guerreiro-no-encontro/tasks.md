...
- [ ] 1.1 Extrair de `cadastrar_guerreiro_pela_gestao` a validação comum dos dados do
      ...
      gestão mude.
- [ ] 1.2 Criar em `personas/regra.py` a função do **caminho do encontro**, que cria a persona de
...
- [ ] 1.3 Criar a conferência de nick de **alcance total** e a geração de variações sobre ela, ao
...
- [ ] 2.1 Acrescentar à matriz a operação de **cadastro de Guerreiro(a) no encontro**, concedida a
...
- [ ] 2.2 Fazer `POST /v1/guerreiros` escolher o caminho pela **aplicação declarada na chave** —
...
- [ ] 2.3 Gravar persona, vínculo de comunidade e **presença** numa transação só no caminho do
...
- [ ] 2.4 Devolver as variações de alcance total no corpo da recusa 422 por nick em uso **apenas**
...
- [ ] 3.1 Criar `apps/app-01-aula-presencial/src/api/guerreiros.ts` com a chamada do cadastro do
      encontro, tipando a recusa por nick com as variações e a recusa por idade (`RF-04-08`,
      `RF-04-09`).
- [ ] 3.2 Criar `apps/app-01-aula-presencial/src/onboarding/TelaDeCadastro.tsx` — coleta de nome,
      ...
      decisão 6).
- [ ] 3.3 Habilitar o caminho do onboarding em `TelaInicial.tsx`, retirando o `disabled` e o
...
- [ ] 4.1 Em `backend/tests/`, cobrir a **faixa etária** nos dois caminhos: idade abaixo, acima e
...
- [ ] 4.2 Cobrir a **separação dos dois caminhos**: persona do encontro sem criador, persona da
...
- [ ] 4.3 Cobrir a **recusa de nick**: variações de alcance total no caminho do encontro, ausência
      ...
      nick").
- [ ] 4.4 Cobrir **cadastro e presença no mesmo ato**: presença gravada com o cadastro, cadastro
...
- [ ] 4.5 Em `apps/app-01-aula-presencial/src/onboarding/onboarding.test.tsx`, cobrir a tela:
...
- [ ] 5.1 Atualizar `docs/` com o que esta change decidiu e mudou: documento 03 §3.3 e documento
