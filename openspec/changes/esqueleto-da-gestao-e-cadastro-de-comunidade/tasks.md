...
- [ ] 1.1 Registrar em `backend/src/nucleo/principal.py` o middleware de origem: qualquer
...
- [ ] 1.2 Cobrir em `backend/tests/` os cenários da capacidade `convencoes-da-api`: o
...
- [ ] 2.1 Promover o `package.json` da raiz a manifesto do monorepo, declarando `workspaces`
      com `comum`, `apps/*` e `jogos/*`, e passar a excluir as pastas de código do
      `.prettierignore`. A esteira de texto continua como está.
- [ ] 2.2 Criar `comum/` como pacote do espaço de trabalho, com o arquivo de tokens em CSS nas
      ...
      tema.
- [ ] 2.3 Configurar o Biome para enxergar `comum/` e `apps/*`, sem colidir com o domínio do
      Prettier.

## 3. Esqueleto da App 03

- [ ] 3.1 Criar `apps/app-03-gestao/` com React, TypeScript, Vite e Vitest, consumindo os
...
- [ ] 3.2 Escrever o cliente de API: a chave em `X-Chave-Aplicacao` e a sessão em
...
- [ ] 3.3 Implementar a entrada: o `id_token` pelo Google Identity Services,
...
- [ ] 3.4 Proteger as telas de dados: sem sessão aberta, só a entrada aparece; sessão expirada
...
- [ ] 4.1 Apresentar as comunidades existentes a partir de `GET /v1/comunidades`, com a
...
- [ ] 4.2 Implementar o formulário de nome, localização e granularidade máxima e a criação por
      `POST /v1/comunidades`; a comunidade criada aparece entre as existentes (`RF-02-11`,
      `RN-02-04`).
- [ ] 4.3 Tratar as recusas na tela: campo obrigatório em falta apontado no campo, sem criar
...
- [ ] 5.1 Criar o workflow das pastas de JavaScript em `.github/workflows/`, disparado só
...
- [ ] 6.1 Testar o cliente de API: os dois cabeçalhos presentes em toda chamada, o corpo de
      erro único interpretado e a recusa da chave distinguida da recusa da sessão.
- [ ] 6.2 Testar a entrada e a sessão, pelos cenários das specs: Admin com cadastro entra e
...
- [ ] 6.3 Testar o cadastro de comunidade e o piso de acessibilidade: criação com os três
...
- [ ] 7.1 Atualizar a situação do PRD-02 em `docs/prds/index.md` e corrigir em
