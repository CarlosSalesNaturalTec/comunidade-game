...
- [ ] 1.1 Em `apps/app-01-aula-presencial/src/pin/pinDeConfirmacao.ts`, reunir a conferência do PIN
...
- [ ] 1.2 Em `entrada/TelaDeEntradaDoGuerreiro.tsx`, passar a confirmação de identidade a usar essa
...
- [ ] 2.1 Em `sessao-de-trabalho/AparelhoDaAula.tsx`, levar o encerramento da sessão de trabalho à
      `TelaInicial`, junto do que ela já recebe (`RF-04-71`, `RF-04-05`).
- [ ] 2.2 Em `inicio/TelaInicial.tsx`, apresentar o encerramento **só na tela inicial**, pedindo o
...
- [ ] 2.3 Ler a contagem da fila local para o aviso, em `fila/filaDePresenca.ts`, sem alterar o que
      a fila guarda nem quando ela sincroniza (`RF-04-23`, `RF-04-25`, design — decisão 5).
- [ ] 2.4 Conferir que o encerramento descarta o verificador do PIN e volta à tela de abertura sem
...
- [ ] 3.1 Em `inicio/TelaInicial.tsx`, pedir o PIN antes de abrir a `TelaDeMedicaoDoLimiar` pelo
      caminho do diagnóstico, sem preparar a câmera antes de o PIN conferir (`RF-04-63`,
      `RF-04-66`, `RN-04-41`).
- [ ] 3.2 Conferir que a medição **dentro do onboarding** não muda: ali o alcance é `guerreiro`,
      depois do consentimento, e o PIN não entra no meio do fluxo do termo (`RN-04-33`,
      `RN-04-07`).

## 4. Testes

- [ ] 4.1 Em `inicio/inicio.test.tsx`, cobrir os sete cenários do requisito novo: a saída existe na
...
- [ ] 4.2 Em `bancada/bancada.test.tsx`, cobrir "A bancada não abre sem o PIN" e "PIN bloqueado não
      abre a bancada", afirmando que o preparo da câmera não acontece antes da conferência
      (`RF-04-63`, `RF-04-66`, `RN-04-41`).
- [ ] 4.3 Em `entrada/entrada.test.tsx`, conferir que a confirmação de identidade segue com as
...
- [ ] 4.4 Em `fila/fila.test.tsx`, conferir que encerrar a sessão de trabalho não descarta a fila e
      que ela volta a sincronizar quando o aparelho reabre naquela aula (`RF-04-23`, `RF-04-25`).

## 5. Documentação

- [ ] 5.1 Marcar a fatia 20 como implementada em `openspec/cronograma-de-fatias.md`.
- [ ] 5.2 Conferir que o PR de revisão do PRD-04 entrou antes, com `RF-04-71` e `RN-04-41` na §6.1 e
