...
- [ ] 1.1 Acrescentar à saída de `GET /v1/guerreiros` a **comunidade do vínculo vigente** e a
...
- [ ] 1.2 Conferir que a rota continua restrita a Admin e que nenhuma escrita nasceu deste
...
- [ ] 2.1 Implementar em `backend/src/nucleo/coletas/regra.py` a consulta de Admin dos desafios
...
- [ ] 2.2 Compor cada item com tipo de coleta, cadência, vigência, granularidade exigida e a
...
- [ ] 2.3 Expor `GET /v1/desafios-de-coleta` como leitura de Admin, verificando pelo OpenAPI que
      a rota nasceu sob `/v1` e que ela não cria, altera nem apaga desafio (`RF-02-17`,
      `RF-01-16`)

## 3. Testes do núcleo

- [ ] 3.1 Em `tests/test_persona_rota.py`, cobrir a listagem com o vínculo: comunidade e data de
...
- [ ] 3.2 Criar `tests/test_desafio_de_coleta_rota.py` cobrindo a listagem de Admin: desafio de
...
- [ ] 4.1 Criar `apps/app-03-gestao/src/territorio/api.ts` com as chamadas da área: listar locais
...
- [ ] 5.1 Criar a tela da área com o seletor de comunidade no padrão de Pontos de Apoio e a
...
- [ ] 5.2 Oferecer ao Admin o cadastro de local com nível escolhido entre os seis, rótulo e local
...
- [ ] 5.3 Apresentar a fila das solicitações de novo local em aberto — solicitante por nick e
...
- [ ] 5.4 Oferecer ao Admin os dois desfechos: aprovar informando o local pai, o que cria o local
...
- [ ] 5.5 Apresentar, em leitura, os desafios de coleta publicados com tipo, cadência, vigência e
      séries ativas, sem oferecer caminho algum de escrita sobre eles (`RF-02-17`)
- [ ] 5.6 Registrar a área Território em `apps/app-03-gestao/src/App.tsx`, entre as áreas da
      navegação (`RF-02-16`)

## 6. App 03 — o vínculo na lista de Guerreiros e Guerreiras

- [ ] 6.1 Apresentar na lista de Guerreiros e Guerreiras a comunidade do vínculo vigente e a data
...
- [ ] 7.1 Criar `apps/app-03-gestao/src/territorio/territorio.test.tsx` cobrindo a área:
...
- [ ] 7.2 Em `apps/app-03-gestao/src/personas/personas.test.tsx`, cobrir a comunidade e a data na
      lista, a ausência de qualquer caminho de transferência e o Guerreiro(a) sem vínculo
      (`RF-02-15`, `RN-02-06`)

## 8. Documentação

- [ ] 8.1 Corrigir a redação do `RF-02-17` na §6.1 do PRD-02 para "desafios de coleta de trilha
