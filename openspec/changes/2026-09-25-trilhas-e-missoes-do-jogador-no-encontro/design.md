...
O que a App 05 já tem, e que esta fatia reusa: `Trilha`, `GuiaDaTrilha`, `Missao`, `Sondagem` e
`DesafioDeDesbloqueio`, em `apps/app-05-guerreiro/src/trilha/`, sobre o cliente
`src/api/trilha.ts`. A `GuiaDaTrilha` já abre na missão atual e mostra a seguinte trancada, que é
exatamente o recorte desta fatia.
...
1. **Os componentes promovidos vão para `comum/trilha/`, e a App 05 passa a consumi-los de lá.**
