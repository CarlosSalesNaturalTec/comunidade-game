...
- [ ] 1.1 Em `backend/src/nucleo/consentimentos/regra.py`, derivar o **estado da autorização**
      ...
      `RN-13-07`, `RN-13-11`)
- [ ] 1.2 Na mesma regra, montar o **histórico da autorização** de um Guerreiro(a): todas as
...
- [ ] 1.3 Na mesma regra, escrever `decidir_autorizacao`: guarda de vínculo vigente (403), os
      dois 409 da PRD-13 §9 — concessão com recusa mais recente de **outro** responsável,
      revogação sem concessão própria mais recente — e a idempotência do reenvio da mesma
      decisão, que devolve o registro existente sem gravar. Grava por `registrar_consentimento`,
      com origem `propria` e o próprio responsável como quem opera. (`RF-13-14`, `RF-13-15`,
      `RN-13-05`, `RN-13-10`)
- [ ] 1.4 Acrescentar em `backend/src/nucleo/erros.py` os erros dos dois 409, com a mensagem em
...
- [ ] 2.1 Em `backend/src/nucleo/solicitacoes_do_responsavel/modelo.py`, acrescentar o campo
...
- [ ] 2.2 Em `backend/src/nucleo/solicitacoes_do_responsavel/regra.py`, escrever
      ...
      `RN-13-14`)
- [ ] 2.3 Encadear a abertura ao ato de decidir, no mesmo commit da recusa. (`RF-13-19`)

## 3. Núcleo — rotas do responsável

- [ ] 3.1 Em `backend/src/nucleo/consentimentos/rotas.py`, expor
...
- [ ] 3.2 No mesmo arquivo, expor `POST /v1/eu/guerreiros/{id}/autorizacao`, recebendo só a
...
- [ ] 4.1 Em `backend/tests/test_consentimento.py`, os cenários de regra: os três estados
...
- [ ] 4.2 Em `backend/tests/test_consentimento_rota.py`, os cenários das duas rotas: concessão e
      ...
      `RN-13-04`, `RN-13-06`)
- [ ] 4.3 Em `backend/tests/test_solicitacao_do_responsavel.py`, os cenários da divergência:
...
- [ ] 5.1 Criar `apps/app-07-responsaveis/src/autorizacao/api.ts` com as chamadas das duas rotas
      e os tipos do estado, de quem motivou a suspensão e do histórico. (`RF-13-18`, `RF-13-21`)
- [ ] 5.2 Criar `apps/app-07-responsaveis/src/autorizacao/TelaDeAutorizacao.tsx`: a declaração do
...
- [ ] 5.3 Em `apps/app-07-responsaveis/src/vinculados/TelaDeVinculados.tsx`, acrescentar a
...
- [ ] 5.4 Criar `apps/app-07-responsaveis/src/autorizacao/autorizacao.test.tsx` com os cenários
...
- [ ] 6.1 Gravar a decisão do fundador de 2026-08-31 — a solicitação da divergência entra na fila
