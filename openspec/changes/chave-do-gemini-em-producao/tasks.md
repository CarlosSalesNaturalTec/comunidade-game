...
- [ ] 1.1 Em `backend/src/nucleo/configuracao.py`, renomear
...
- [ ] 2.1 Apontar as três fábricas para os campos novos e corrigir o _docstring_ de cada uma,
...
- [ ] 3.1 Em `template_de_missao/nuvem.py`, `producoes/nuvem.py` e `assistente/nuvem.py`,
...
- [ ] 4.1 Em `.github/workflows/backend-deploy.yml`, declarar `CG_GEMINI_MODELO` em
...
- [ ] 5.1 Em `backend/tests/test_template_de_missao.py`, cobrir os dois caminhos mudos do
  adaptador de nuvem: chave ausente e resposta fora do formato esperado devolvem `None` **e**
  emitem a linha de log esperada (`caplog`, _logger_ `nucleo.template_de_missao`). Repetir os
  dois cenários em `test_assistente_porta.py` e `test_producao_da_missao_porta.py`, nos
  _loggers_ `nucleo.assistente` e `nucleo.producoes` (design — decisão 3).
- [ ] 5.2 Cobrir a escolha da fábrica com os campos novos: em `ambiente="producao"` as três
...
- [ ] 6.1 Em `backend/README.md`: acrescentar `CG_GEMINI_CHAVE_DE_API` e `CG_GEMINI_MODELO` à
...
- [ ] 6.2 Em `backend/README.md`, acrescentar a seção de diagnóstico "Quando a sugestão, a
...
- [ ] 6.3 Registrar a change em `openspec/cronograma-de-fatias.md`: linha sem número no bloco
