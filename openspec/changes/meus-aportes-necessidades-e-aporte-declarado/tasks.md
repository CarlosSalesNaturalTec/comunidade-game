...
- [ ] 1.1 Criar `AporteDeclarado` em `backend/src/nucleo/aportes/modelo.py` — provedor, valor
...
- [ ] 1.2 Escrever a migração Alembic da tabela, da coluna e do valor novo do enum, com
...
- [ ] 1.3 Implementar em `backend/src/nucleo/aportes/regra.py` a declaração do Apoiador: exige
...
- [ ] 1.4 Implementar a resolução da declaração: `registrar_aporte` passa a aceitar a
...
- [ ] 1.5 Publicar as rotas em `backend/src/nucleo/aportes/rotas.py` e registrá-las em
...
- [ ] 2.1 Acrescentar o nome do tipo de recurso, da comunidade e do ponto de apoio à saída de
...
- [ ] 2.2 Acrescentar o nome do tipo de recurso e o destino à saída de `GET /v1/meus-aportes`,
...
- [ ] 3.1 Mover a escada de valores sugeridos de `src/preCadastro/escada.ts` para um módulo
...
- [ ] 3.2 Criar `src/aportes/api.ts` com as chamadas de "Meus aportes", necessidades em aberto,
      declaração e situação das declarações (`RF-14-21`, `RF-14-24`, `RF-14-25`, `RF-14-27`).
- [ ] 3.3 Criar a tela "Meus aportes": os aportes homologados com data, tipo e destino, o Poder
      Sustentador como total acumulado em moedas e o vazio explicado (`RF-14-21`, `RF-14-22`).
      Verificar pelos testes da tarefa 4.5.
- [ ] 3.4 Criar a tela das necessidades em aberto: atividade, comunidade e o que falta em
...
- [ ] 3.5 Criar a tela da declaração do aporte: os três caminhos — necessidade, valor sugerido
...
- [ ] 3.6 Criar a tela da situação das declarações — pendente, homologado ou recusado com
...
- [ ] 4.1 `backend/tests/test_aporte_declarado.py` — a declaração nasce pendente e não gera
...
- [ ] 4.2 No mesmo arquivo, o desfecho: a homologação grava o aporte com origem "App 08",
      ...
      `RN-14-07`, `RN-14-08`).
- [ ] 4.3 `backend/tests/test_aporte_declarado_rota.py` — as rotas do Apoiador alcançam só as
      declarações dele, exigem a credencial da persona e a recusa é restrita a Admin
      (`RF-14-25`, `RF-14-27`, `RN-14-08`).
- [ ] 4.4 Estender `backend/tests/test_necessidade_rota.py` e o teste de "Meus aportes" com os
      nomes na saída, o valor em moedas e a ausência de reais e de dado de pessoa (`RF-14-21` a
      `RF-14-24`, `RN-14-09`).
- [ ] 4.5 `apps/app-08-apoiador/src/aportes/meusAportes.test.tsx` — "Meus aportes" com total e
...
- [ ] 4.6 `apps/app-08-apoiador/src/aportes/declaracao.test.tsx` — os três caminhos da
...
- [ ] 5.1 Gravar a decisão nova do fundador de 2026-09-01 — o Admin recusa com motivo a
...
- [ ] 5.2 Marcar a fatia 4 do PRD-14 como implementada em `openspec/cronograma-de-fatias.md`,
