...
- [ ] 1.1 Em `backend/src/nucleo/fila/modelo.py`, declarar o `StrEnum` `PerfilDeApoiador`
      ...
      (`RF-14-01`, `RN-14-39`)
- [ ] 1.2 Escrever a migração Alembic que acrescenta a coluna `perfil`, com `downgrade` que a
      remove; verificar com `alembic upgrade head` seguido de `alembic downgrade -1`
      (`RF-14-01`)
- [ ] 1.3 Em `fila/regra.py`, gravar o perfil recebido sem verificá-lo, recusar com 422 o perfil
...
- [ ] 1.4 Em `fila/rotas.py`, aceitar `perfil` no formulário de
...
- [ ] 2.1 Em `backend/tests/test_fila.py`, cobrir a regra: perfil gravado como veio nas duas
...
- [ ] 2.2 Em `backend/tests/test_fila_rota.py`, cobrir a rota pública: envio com perfil e
...
- [ ] 3.1 Em `apps/app-08-apoiador/src/api/configuracao.ts` e `.env.example`, acrescentar
      `VITE_URL_DO_FORMULARIO_DA_VITRINE`, vazia no Ciclo 01 (`RF-14-07`)
- [ ] 3.2 Em `src/App.tsx`, apresentar a **porta pública** como tela padrão de quem não tem
...
- [ ] 3.3 Criar `src/preCadastro/api.ts` com o envio multipart de
...
- [ ] 3.4 Criar `src/preCadastro/escada.ts` com a escala fixa de 1 moeda = R$ 10,00 e as duas
      ...
      `RN-14-40`)
- [ ] 3.5 Implementar `src/preCadastro/TelaDePreCadastro.tsx`: identificação sem documento —
...
- [ ] 3.6 Apresentar, antes do envio, que o pré-cadastro **não cria cadastro nem acesso**, que
...
- [ ] 3.7 Apresentar a recusa do freio por origem com o tempo de espera em linguagem simples,
      preservando o que foi preenchido, e a recusa de formato com os formatos válidos
      (`RF-14-04`, `RF-14-06`)
- [ ] 3.8 Encaminhar ao formulário da vitrine quem apoia com material, serviço ou divulgação,
      sem registrar aporte; sem endereço configurado, explicar o caminho em texto, sem link
      (`RF-14-07`, `RN-14-05`)

## 4. Testes da App 08

- [ ] 4.1 Em `apps/app-08-apoiador/src/preCadastro/preCadastro.test.tsx`, cobrir a porta:
...
- [ ] 4.2 No mesmo arquivo, cobrir os desfechos: envio bem-sucedido confirmando a fila sem abrir
...
- [ ] 5.1 Marcar a fatia 2 do PRD-14 como `implementado` em
...
- [ ] 5.2 Acrescentar o **perfil declarado** à linha da `SolicitacaoDeParticipacao` no PRD-01
