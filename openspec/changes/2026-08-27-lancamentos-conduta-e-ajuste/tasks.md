...
- [ ] 1.1 Acrescentar a `Presenca`, em `backend/src/nucleo/aulas/modelo.py`, as colunas
...
- [ ] 1.2 Implementar `anular_presenca` em `backend/src/nucleo/aulas/regra.py`: exige motivo
...
- [ ] 1.3 Passar `registrar_presenca` a ignorar a presença anulada na idempotência, de modo que
...
- [ ] 1.4 Expor `POST /v1/aulas/{id_da_aula}/presencas/{id_da_presenca}/anulacao` em
...
- [ ] 2.1 Implementar em `backend/src/nucleo/livro_razao/regra.py` a paginação dos lançamentos
...
- [ ] 2.2 Expor `GET /v1/lancamentos` em `livro_razao/rotas.py` pelo `contrato_de_listagem`,
...
- [ ] 3.1 Em `backend/tests/test_presenca_do_mestre.py` (ou arquivo novo de presença, se couber
...
- [ ] 3.2 Em `backend/tests/test_lancamento_rota.py`, cobrir a listagem: lançamentos do ponto de
...
- [ ] 4.1 Criar `apps/app-03-gestao/src/lancamentos/api.ts` com as chamadas da área: lançar a
...
- [ ] 4.2 Acrescentar a `apps/app-03-gestao/src/pontos-de-apoio/api.ts` a listagem de lançamentos
...
- [ ] 5.1 Criar `TelaDeLancamentos.tsx` com a área ancorada na aula vigente do painel do dia,
...
- [ ] 5.2 Criar `LancamentoDaAtividade.tsx`: lista dos participantes com o desfecho de cada um
...
- [ ] 5.3 Criar `ConferenciaDePresencas.tsx`: presenças da aula com modo e confirmador, avatar e
...
- [ ] 5.4 Criar `RegistroDeInfracao.tsx`: encontro, atividade, Guerreiro(a) e motivo em texto
      ...
      `RN-02-14`)
- [ ] 5.5 Restringir a área ao papel: o Mestre vê apenas o registro da infração, e nem o
...
- [ ] 6.1 Criar `ExtratoDoPontoDeApoio.tsx` na área Pontos de Apoio, ao lado dos saldos e da
...
- [ ] 6.2 Criar `AjusteDeLancamento.tsx` com quantidade, moedas e motivo obrigatório, sem
      caminho de edição nem de remoção, atualizando o extrato depois de gravado (`RF-02-40`,
      `RN-02-12`)

## 7. Testes da App 03

- [ ] 7.1 Criar `apps/app-03-gestao/src/lancamentos/lancamentos.test.tsx` cobrindo os cenários da
      ...
      `RN-02-20`, `RN-02-22`)
- [ ] 7.2 Em `apps/app-03-gestao/src/pontos-de-apoio/pontos-de-apoio.test.tsx`, cobrir o extrato
...
- [ ] 8.1 Gravar as três decisões novas em `docs/09-topicos-em-aberto-e-sugestoes.md` §1, em "Já
...
- [ ] 8.2 Atualizar `docs/prds/prd-02-frontend-de-gestao.md`: a §4 e a `RN-02-20` com a infração
...
- [ ] 8.3 Marcar a fatia 10 como implementada em `openspec/cronograma-de-fatias.md`, trocando o
