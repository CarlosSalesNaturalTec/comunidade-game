...
- [ ] 1.1 Criar `PerguntaNaPartida` em `backend/src/nucleo/quiz/modelo.py` — partida,
...
- [ ] 1.2 Implementar `por_pergunta_no_ar` em `backend/src/nucleo/quiz/regra.py`: exige quem
...
- [ ] 1.3 Implementar `liberar_resultado` na mesma regra: marca a liberação da pergunta no ar,
...
- [ ] 1.4 Implementar a leitura do estado da partida na regra — situação, pergunta no ar,
...
- [ ] 2.1 Acrescentar `conducao_do_quiz_ao_vivo_das_suas_aulas` ao conjunto `le` do Mestre e
...
- [ ] 2.2 Escrever em `backend/src/nucleo/quiz/rotas.py` as rotas de escrita da condução —
...
- [ ] 2.3 Escrever as rotas de leitura — `GET /v1/partidas-de-quiz/{id}` para quem conduz e
      ...
      depois da liberação.
- [ ] 2.4 Escrever `POST /v1/partidas-de-quiz/{id}/respostas` sobre `registrar_resposta`, sem
...
- [ ] 3.1 Cobrir em `backend/tests/test_quiz.py` os cenários de regra do estado novo: pergunta
...
- [ ] 3.2 Cobrir em `backend/tests/test_quiz_rota.py` o percurso completo pelas rotas — abrir,
...
- [ ] 4.1 Criar `apps/app-03-gestao/src/quiz/` com o cliente das rotas da partida em
      `src/api/`, seguindo o padrão das áreas já existentes (`RF-02-59` a `RF-02-62`).
      Verificar pelos testes de cliente do módulo.
- [ ] 4.2 Implementar a tela de abertura — aula e atividade vindas de `GET /v1/minhas-turmas`,
...
- [ ] 4.3 Implementar a tela de condução com os quatro atos — pôr pergunta no ar do banco de
...
- [ ] 4.4 Implementar a sondagem a cada 2 segundos, com o aviso de perda de contato que
      ...
      sondagem da spec.
- [ ] 4.5 Ligar a área ao roteamento e ao menu da App 03 sob a permissão do Mestre, com a
      recusa dita em uma frase para a aula que não é dele (`RF-02-49`, `RN-02-20`). Verificar
      pelos dois cenários de alcance da spec.

## 5. Documentação

- [ ] 5.1 Gravar no documento 03 §1 a decisão do fundador de 2026-08-25 — a sincronização em
...
- [ ] 5.2 Corrigir os PRDs ao que já está decidido e ao que esta fatia entregou: no PRD-02, o
