...
- [ ] 1.1 Gravar as decisões 1 e 4 no documento 02 §8.2 — o encontro da troca é a `Aula` e o
...
- [ ] 1.2 Mover as quatro decisões para "Já decididos" no documento 09, e aplicá-las ao PRD-07:
      a rota `POST /aulas/{id}/trocas` na §9, a `Troca` e o débito sem aula na §8, e as quatro
      linhas na §13 (`RF-07-35`, `RF-07-46`).

## 2. Modelo e migração

- [ ] 2.1 Criar `backend/src/nucleo/trocas/modelo.py` com a `Troca` — item, Guerreiro(a),
      ...
      `RF-07-46`, `RF-01-27`).
- [ ] 2.2 Migração Alembic da tabela `troca`, com índice por Guerreiro(a) e por aula, para a
      leitura do histórico (`RF-07-35`).

## 3. Regra da troca

- [ ] 3.1 Escrever em `trocas/regra.py` a validação das quatro recusas, na ordem do spec — item
...
- [ ] 3.2 Ler o preço em `catalogo_de_tipos_de_recurso` pela vigência corrente na data e recusar
      com 422 quando não houver vigência que a cubra (`RF-07-46`, `RF-07-38`, `RN-07-29`).
- [ ] 3.3 Escrever a operação única de entrega — grava a `Troca` com o preço cobrado, debita o
...
- [ ] 3.4 Estender `ponto_extra/regra.py` com o débito do saldo disponível, recusado quando
      deixaria o saldo negativo, sem caminho algum que reduza o acumulado (`RF-01-56`,
      `RN-01-39`, `RN-01-40`).
- [ ] 3.5 Escrever a leitura do histórico filtrada por persona — Guerreiro(a) só as próprias,
...
- [ ] 4.1 Criar `trocas/rotas.py` com `POST /aulas/{id}/trocas`, exigindo Mestre em sessão
      vinculado à comunidade da aula, recusando preço declarado no corpo com 422 e devolvendo
      201 (`RF-07-35`, `RF-07-46`, `RF-01-16`).
- [ ] 4.2 Acrescentar `GET /trocas` no mesmo roteador e registrá-lo em `principal.py` por
      `incluir_roteador_de_dados` (`RF-07-35`, `RF-01-01`).

## 5. Testes

- [ ] 5.1 `backend/tests/test_trocas.py` — grupo do registro e das permissões: Mestre registra e
      ...
      (`RF-07-35`).
- [ ] 5.2 Grupo do preço: cobra a vigência corrente e grava o valor; mudança posterior da tabela
      não altera troca gravada; preço no corpo devolve 422; tipo sem preço vigente devolve 422
      (`RF-07-46`, `RF-07-38`).
- [ ] 5.3 Grupo das recusas: as quatro condições, cada uma com a resposta nomeando o motivo, e o
...
- [ ] 5.4 Grupo da entrega: as quatro escritas juntas com os números do cenário do spec (saldo
...
- [ ] 5.5 Grupo do histórico: Guerreiro(a) lê só as próprias, troca de outro nunca aparece,
      Mestre lê as das suas comunidades, Apoiador recebe 403, e nenhuma resposta traz moedas nem
      reais (`RF-07-35`, `RN-07-24`).
- [ ] 5.6 Acrescentar em `backend/tests/test_livro_razao.py` os dois cenários do delta — o
      débito da troca é gravado sem aula, e o consumo por aula soma só o débito da baixa
      (`RF-07-36`, `RF-07-16`, `RN-07-15`).

## 6. Documentação

- [ ] 6.1 Atualizar `docs/prds/index.md` com a oitava fatia do PRD-07: a troca entregue, as
