...
- [ ] 1.1 Criar `backend/src/nucleo/patrimonio/modelo.py` com `ItemPatrimonial` — aporte de
...
- [ ] 1.2 Migração Alembic das duas tabelas, com `UNIQUE (ponto_de_apoio_id,
...
- [ ] 2.1 `patrimonio/regra.py`: `tombar_item` — exige Admin, valida título, tombo, ponto de
...
- [ ] 2.2 Conferência do **teto do aporte** dentro de `tombar_item`, com `SELECT ... FOR UPDATE`
...
- [ ] 2.3 `anotar_ficha_de_vida` — exige Admin ou Mestre, grava a anotação e reescreve o estado
...
- [ ] 2.4 `listar_acervo` — itens com responsável **derivado** por junção com
...
- [ ] 3.1 `aulas/regra.py`: recusar com 422 o recurso declarado de tipo de natureza durável, no
      ...
      Decisions 1).
- [ ] 3.2 `catalogo_avulso/regra.py`: recusar com 422 em `cadastrar_item` o item cujo tipo é de
      natureza durável, preservando intacta a regra do item sem lastro, que segue aceito e
      inativo (`RF-07-34`, `RN-07-07`, `RN-07-26`).

## 4. Rotas

- [ ] 4.1 `patrimonio/rotas.py` com `POST /v1/itens-patrimoniais` (Admin),
...
- [ ] 5.1 `tests/test_patrimonio_tombamento.py`: tombamento por Admin com e sem aporte de
...
- [ ] 5.2 `tests/test_patrimonio_ficha_de_vida.py`: anotação gravada com autoria, 403 do
...
- [ ] 5.3 `tests/test_patrimonio_rota.py`: leitura do acervo por Admin e por Mestre vinculado,
...
- [ ] 5.4 `tests/test_saldo_duravel_inerte.py`: aporte durável credita Poder Sustentador;
...
- [ ] 6.1 Gravar as quatro decisões novas no documento-fonte de cada uma — documento 04 §1
...
- [ ] 6.2 Aplicar as decisões ao PRD-07: §8 (atributos do `ItemPatrimonial`, sem responsável
