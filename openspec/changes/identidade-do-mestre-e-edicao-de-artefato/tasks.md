...
- [x] 1.1 `ArtefatoComprobatorio` ganha `endereco_original`, `rotulo_original` e `editado_em`,
...
- [x] 2.1 `definir_avatar_do_mestre`: grava o avatar do Mestre em sessão **sem** consultar
      moeda acumulada, opaco à forma do valor, no molde do avatar do Apoiador menos o piso
      (`RF-09-114`, `RN-14-11`, design — decisão 1).
- [x] 2.2 `editar_artefato_do_mestre`: altera rótulo e endereço de artefato do próprio perfil,
...
- [x] 3.1 `PUT /v1/eu/mestre/identidade` passa a receber `nick` e `avatar`, cada um opcional, e
      a devolver os dois; a entrada só de nick deixa de existir (`RF-09-114`, design —
      decisão 1).
- [x] 3.2 `GET /v1/eu/mestre/identidade`, restrita ao Mestre, devolvendo nick e avatar
      vigentes e nenhum dado de moeda; outro papel recebe 403 (`RF-09-114`, design —
      decisão 2).
- [x] 3.3 `PATCH /v1/mestres/{id}/artefatos/{artefato_id}`, com a mesma guarda de perfil
...
- [x] 3.4 A saída dos adultos em `GET /v1/mestres` e `GET /v1/apoiadores` passa a trazer, por
...
- [x] 4.1 `perfil/api.ts` ganha a leitura e a gravação da identidade e a edição de artefato.
- [x] 4.2 `TelaDoPerfil.tsx` apresenta o nick e o avatar vigentes e oferece definir ou trocar
...
- [x] 4.3 `TelaDoPerfil.tsx` oferece editar rótulo e endereço de cada artefato, o do cadastro
...
- [x] 5.1 `FichaDoAdulto.tsx` marca o artefato de cadastro editado e mostra o valor original ao
      lado do vigente, sem oferecer editar, restaurar ou remover (`RF-02-04`, `RN-02-01`,
      `RN-09-14`, design — decisão 5).

## 6. Testes

- [x] 6.1 Núcleo, identidade do Mestre: grava nick e avatar juntos e cada um em separado; grava
...
- [x] 6.2 Núcleo, edição de artefato: corrige o publicado pelo próprio; corrige o do cadastro e
...
- [x] 6.3 App 09, `perfil.test.tsx`: o perfil mostra nick e avatar vigentes ou a ausência
...
- [x] 6.4 App 03, `personas.test.tsx`: a ficha marca o artefato de cadastro editado com o valor
...
- [x] 7.1 Documento 02 §1: a prova declarada pelo Admin no cadastro passa a ser **editável pelo
