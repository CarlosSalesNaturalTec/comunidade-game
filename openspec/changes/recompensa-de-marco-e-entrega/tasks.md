...
- [x] 1.1 Criar `backend/src/nucleo/recompensas_de_marco/modelo.py` com `RecompensaDeMarco`
...
- [x] 1.2 Registrar as duas tabelas na criação do schema, seguindo o que `trocas/` e
      `patrimonio/` já fazem, e conferir que nenhuma coluna nova é exigida de `Lancamento`

## 2. Declaração do marco

- [x] 2.1 Em `recompensas_de_marco/regra.py`, escrever `declarar_recompensa_de_marco`: só o
...
- [x] 2.2 Escrever a leitura das recompensas de marco de uma trilha, para a gestão (`RF-09-71`)

## 3. Entrega e baixa

- [x] 3.1 Tornar pública em `pontuacao/regra.py` a derivação das missões concluídas por
      Guerreiro(a) numa trilha, hoje `_missoes_concluidas_pelo_guerreiro`, sem mudar o que ela
      calcula (`RF-07-13`)
- [x] 3.2 Escrever `_validar_entrega` com as cinco recusas na ordem do delta — durável, lastro
      ...
      invariante 9)
- [x] 3.3 Escrever `registrar_entrega` como operação atômica única: grava a entrega e emite o
...
- [x] 3.4 Escrever `listar_entregas` filtrada por persona — o Guerreiro(a) lê as próprias, o
      Mestre e o Admin as da comunidade —, sem valor em moedas nem em reais na saída
      (`RF-07-13`, `RN-07-05`, invariante 16)

## 4. Rotas

- [x] 4.1 Criar `recompensas_de_marco/rotas.py` com `POST` e `GET`
...
- [x] 4.2 Registrar o roteador em `principal.py` por `incluir_roteador_de_dados`, como os de
      `trocas` e `patrimonio`

## 5. Testes

- [x] 5.1 `backend/tests/test_recompensa_de_marco.py` — declaração: Mestre autor grava; Mestre
      não autor é recusado; marco que não é missão é recusado; declaração com preço é recusada
      (`RF-09-71`, `RN-09-26`, `RN-09-39`)
- [x] 5.2 `backend/tests/test_entrega_de_recompensa.py` — as cinco recusas, uma a uma, e o
      cenário de que a recusa não move nada: nem entrega, nem lançamento, nem saldo
      (`RF-07-13`, `RN-07-07`, invariante 9)
- [x] 5.3 No mesmo arquivo, a entrega feliz: grava a entrega, o saldo do tipo naquele ponto de
...
- [x] 5.4 No mesmo arquivo, a atomicidade e o histórico: falha no lançamento desfaz a entrega;
...
- [x] 5.5 Em `backend/tests/test_prestacao_de_contas.py`, o cenário de que a entrega fica fora
...
- [x] 6.1 Gravar as quatro decisões novas no documento 02 §8.1 — quem confirma a entrega, o
...
- [x] 6.2 Aplicar as correções nos PRDs: PRD-02 (`RF-02-50` e `RF-02-51` passam a mostrar),
...
- [x] 6.3 Atualizar `docs/prds/index.md` com a décima fatia do PRD-07 e o que resta dele, e o
