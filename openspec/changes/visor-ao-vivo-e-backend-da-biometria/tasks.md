...
- [ ] 1.1 Acrescentar ao documento 03 §3.3 o item **"Visor ao vivo, imagem capturada nunca
...
- [ ] 1.2 Ajustar o **invariante 12** do documento 99 §6 para distinguir **visor ao vivo** de
...
- [ ] 1.3 Registrar a decisão em `docs/09-topicos-em-aberto-e-sugestoes.md` §1, linha nova
...
- [ ] 1.4 Criar no PRD-04 os identificadores `RF-04-64` (visor ao vivo e retorno abstrato),
...
- [ ] 2.1 Declarar o backend `webgl` na configuração da Human (`biometria.ts`), com o porquê em
...
- [ ] 2.2 Separar o **preparo** (abrir câmera e carregar modelos) da prova de vivacidade, com
...
- [ ] 2.3 Expor a função que **acopla o espelho** a um elemento contêiner fornecido pela tela,
...
- [ ] 2.4 Trocar o `detect()` de tiro único por **detecção em laço** até a vivacidade passar ou
...
- [ ] 3.1 `TelaDeCaptura` (onboarding): apresentar o visor ao vivo e o retorno abstrato durante
      o laço, e distinguir na tela falha de preparo, reprovação de vivacidade e recusa do núcleo
      (`RF-04-64`, `RF-04-65`, `RN-04-34`).
- [ ] 3.2 `TelaDeEntradaDoGuerreiro` (entrada pelo caminho das trilhas): o mesmo visor e o mesmo
...
- [ ] 3.3 `TelaDeMedicaoDoLimiar` (bancada): o mesmo visor nas duas capturas — referência e
      comparação —, sem mudar o que a tela guarda e descarta (`RF-04-64`, `RN-04-34`,
      `RN-04-32`).
- [ ] 3.4 Acrescentar a menção ao visor no aviso de coleta e na área detalhada de direitos: a
      câmera aparece na tela e nada dela sai do aparelho (`RF-04-26`, `RF-04-64`).

## 4. Testes

- [ ] 4.1 Em `captura.test.tsx` e `entrada.test.tsx`, cobrir os cenários do delta: o visor
...
- [ ] 4.2 Em `bancada.test.tsx`, cobrir o visor nas duas capturas da medição e a permanência do
      que a tela já garante — um descritor de referência por vez, comparada descartada no ato
      (`RF-04-64`, `RN-04-34`, `RN-04-32`).
- [ ] 4.3 Cobrir no módulo `comum/biometria` o laço de detecção e o desfecho do preparo, com a
      Human dublada: laço que aprova, laço que esgota o tempo e preparo que falha
      (`RF-04-64`, `RF-04-65`).

## 5. Documentação

- [ ] 5.1 Acrescentar ao bloco do PRD-04 do `openspec/cronograma-de-fatias.md` a linha desta
...
- [ ] 6.1 Depois do _deploy_, conferir no aparelho do encontro, com DevTools: as requisições a
