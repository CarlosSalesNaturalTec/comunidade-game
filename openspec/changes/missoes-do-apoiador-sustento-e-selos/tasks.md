...
- [ ] 1.1 Criar `backend/src/nucleo/missoes_do_apoiador/modelo.py` com `MissaoDoApoiador` — par
...
- [ ] 1.2 Criar `backend/src/nucleo/selos_do_apoiador/modelo.py` com `SeloDoApoiador` —
...
- [ ] 1.3 Acrescentar `missao` a `OrigemDaEscolhaDoAporte` e a coluna
      `missao_do_apoiador_id` em `AporteDeclarado` (`RF-14-63`, `RF-14-25`, design — Migration).
- [ ] 1.4 Gerar a migração Alembic com as duas tabelas e a coluna nova, e verificar que
      `alembic upgrade head` e `downgrade` correm limpos no banco de teste.

## 2. Núcleo — regra da missão

- [ ] 2.1 `missoes_do_apoiador/regra.py`: `publicar_missao()` — só Admin, conferindo o par
...
- [ ] 2.2 `despublicar_missao()` — só Admin, 409 na missão já concluída, sem estorno de aporte
      homologado (`RF-02-105`, `RN-14-34`).
- [ ] 2.3 `derivar_missoes()` — o coberto e o quanto falta a partir dos aportes homologados que
...
- [ ] 2.4 `concluir_se_fechou()` — chamada de `registrar_aporte()` na homologação de declaração
...
- [ ] 2.5 Estender `declarar_aporte()` com a origem `missao`, recusando com 409 a missão
...
- [ ] 3.1 `selos_do_apoiador/regra.py`: `derivar_sustento()` — nível 1 pelo primeiro aporte
...
- [ ] 3.2 `listar_selos()` agrupando por família, somente leitura do próprio Apoiador
      (`RF-14-68`, `RF-14-70`, `RN-14-38`).

## 4. Núcleo — rotas

- [ ] 4.1 `missoes_do_apoiador/rotas.py`: `POST /v1/missoes-do-apoiador`,
...
- [ ] 4.2 `GET /v1/eu/apoiador/sustento`, do Apoiador em sessão, com 403 para o sustento de
      outro (`RF-14-67`, `RF-14-68`).
- [ ] 4.3 Registrar os roteadores em `principal.py` e verificar que os caminhos novos aparecem
      no OpenAPI servido fora do `/v1`.

## 5. Testes do núcleo

- [ ] 5.1 `tests/test_missao_do_apoiador.py`: publicação com e sem necessidade por trás,
...
- [ ] 5.2 `tests/test_missao_do_apoiador_cobertura.py`: declaração pendente não abate,
...
- [ ] 5.3 `tests/test_sustento_do_apoiador.py`: nível 1 pelo primeiro aporte, frentes diferentes
...
- [ ] 5.4 `tests/test_missao_do_apoiador_rota.py`: as quatro rotas, com chave de aplicação,
      persona errada em 403 e a leitura pública sem sessão (`RF-14-60`, `RF-02-102`,
      `RF-02-104`, `RF-02-105`).

## 6. App 08 — Área do Apoiador

- [ ] 6.1 `src/missoes/TelaDeMissoes.tsx` e `api.ts`: missões abertas agrupadas por nível, com o
...
- [ ] 6.2 `src/missoes/DeclaracaoPorMissao.tsx`: cobrir a missão inteira ou parte, com
      comprovante, declarando que o aporte entra pendente e não abate nem conclui (`RF-14-63`,
      `RF-14-64`, `RN-14-32`).
- [ ] 6.3 `src/sustento/TelaDeSustento.tsx`: nível alcançado, selos por família e a frente que
      falta, dita uma vez e sem repetição nas demais telas (`RF-14-67` a `RF-14-70`,
      `RF-14-73`).
- [ ] 6.4 Acrescentar a opção "missão aberta" à porta pública de pré-cadastro e ao caminho de
      declaração do Apoiador em sessão (`RF-14-02`, `RF-14-25`).
- [ ] 6.5 Testes Vitest de `missoes` e `sustento`: agrupamento e coberto anônimo, aviso do
...
- [ ] 7.1 `src/missoes-do-apoiador/PublicacaoDeMissao.tsx` e `ListaDeMissoes.tsx`, na área
      ...
      `RF-02-105`).
- [ ] 7.2 Testes Vitest: publicação recusada por faltar necessidade apresentada em linguagem
...
- [ ] 8.1 Marcar a fatia 5 do PRD-14 como implementada no `openspec/cronograma-de-fatias.md`.
