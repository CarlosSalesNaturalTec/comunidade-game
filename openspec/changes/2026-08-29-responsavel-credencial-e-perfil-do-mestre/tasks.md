...
- [ ] 1.1 Acrescentar `Operacao.vinculo_com_guerreiros_e_guerreiras` ao conjunto `le` do Mestre
...
- [ ] 1.2 Criar `GET /v1/guerreiros/vinculaveis` em `backend/src/nucleo/responsaveis/`,
      ...
      (`RF-09-62`, `RN-09-18`).
- [ ] 1.3 Acrescentar `declarado_por_id` a `ArtefatoComprobatorio` e a migração Alembic
...
- [ ] 1.4 Criar `GET`, `POST` e `DELETE` de `/v1/mestres/{id}/artefatos` em
...
- [ ] 2.1 Testar a leitura dos vinculáveis em `backend/tests/`: o Mestre vê os ativos da sua
...
- [ ] 2.2 Testar os artefatos do próprio Mestre: publicação com endereço e rótulo, recusa de
...
- [ ] 3.1 Criar `apps/app-09-mestre/src/responsaveis/api.ts` com as chamadas de cadastro,
      vínculo, credencial provisória e leitura dos vinculáveis, e registrar a área na navegação
      do `App.tsx` (`RF-09-62` a `RF-09-65`).
- [ ] 3.2 Criar a tela do fluxo — cadastro pelo nome, com a declaração da apresentação
      ...
      decisão 4).
- [ ] 3.3 Apresentar a recusa do quarto vínculo como o teto de três por criança, em linguagem
      simples, sem a App contar vínculos por conta própria e sem desfazer o que já foi criado
      (`RF-09-64`, `RN-09-16`).

## 4. App 09 — Meu perfil

- [ ] 4.1 Criar `apps/app-09-mestre/src/perfil/` com a leitura e a publicação dos artefatos por
      ...
      (`RF-09-66`).
- [ ] 4.2 Declarar na área que o cadastro de Mestre é ato exclusivo de Admin, com habilidade
      comprovada, e apresentar nome, e-mail e papel em leitura, sem caminho de edição
      (`RF-09-67`, `RN-09-14`).

## 5. App 09 — camada de direitos

- [ ] 5.1 Criar `apps/app-09-mestre/src/direitos/` com `AvisoDeColeta`, `ContextoDeDireitos` e
...
- [ ] 5.2 Acrescentar o aviso de coleta, nomeando o dado daquela tela, às telas do responsável,
...
- [ ] 6.1 Testar a área Responsáveis: cadastro com a declaração presencial visível, vínculo com
...
- [ ] 6.2 Testar Meu perfil: publicação por endereço e rótulo, ausência de campo de anexo,
...
- [ ] 6.3 Testar a camada de direitos: a tabela da §11 apresentada com finalidade, base legal,
...
- [ ] 7.1 Gravar as duas decisões do fundador de 2026-08-29 no documento-fonte de cada uma —
      ...
      §1, em "Já decididos".
- [ ] 7.2 Aplicar as decisões nos PRDs: PRD-01 §4 (as duas operações na linha do Mestre) e §9 (as
...
- [ ] 7.3 Marcar as fatias 10 e 11 do PRD-09 como implementadas em
