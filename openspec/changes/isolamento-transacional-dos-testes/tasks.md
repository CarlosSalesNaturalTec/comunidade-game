...
- [ ] 1.1 Criar a fixture `conexao` em `backend/tests/conftest.py`: abre uma conexão do
      `engine`, inicia a transação e a desfaz no teardown (design — Decisions 1)
- [ ] 1.2 Reescrever a fixture `sessao` para `bind=conexao` com
      `join_transaction_mode="create_savepoint"`, sem `TRUNCATE` e sem `RESTART IDENTITY`
      (design — Decisions 1, 5)
- [ ] 1.3 Criar a fixture que substitui `obter_fabrica_de_sessao` de `nucleo.banco` por uma
      fábrica presa à `conexao`, aplicada onde o middleware de auditoria grava
      (design — Decisions 3)
- [ ] 1.4 Registrar o marcador `banco_compartilhado` em `backend/pyproject.toml` e dar a ele
...
- [ ] 2.1 `test_autoria.py`, `test_convencoes.py` e `test_cli.py` — este último passa a montar
      a `sessionmaker` sobre a `conexao`
- [ ] 2.2 Testes de gatilho de imutabilidade: `test_auditoria.py`, `test_consentimento.py`,
      `test_lancamento.py`, `test_ponto_extra.py` e `test_pontuacao.py`
- [ ] 2.3 `test_biometria.py`, `test_quiz.py`, `test_desafio_de_coleta.py` e
      `test_autorizacao_vigente.py`
- [ ] 2.4 `test_auditoria_middleware.py`, com a fixture da tarefa 1.3
- [ ] 2.5 `test_responsavel.py`: a criação simultânea do terceiro vínculo recebe
      `@pytest.mark.banco_compartilhado`, por precisar de duas conexões sobre dado gravado

## 3. Verificação

- [ ] 3.1 Suíte inteira verde, com o mesmo número de testes de antes (962) e nenhum
      `xfail`, `skip` ou cenário removido
- [ ] 3.2 Registrar no `design.md`, em uma linha, o tempo medido depois da conversão, ao lado
      dos 258 s de antes — a meta declarada na proposta é ficar abaixo de 90 s

## 4. Documentação

- [ ] 4.1 Nada a atualizar em `docs/`: a change não toma decisão nova, não muda requisito de
