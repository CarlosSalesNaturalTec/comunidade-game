...
- [ ] 1.1 Extrair para `pontuacao/regra.py` a derivação do ranking hoje embutida em
...
- [ ] 1.2 Escrever no mesmo módulo a consulta do **ranking da turma**: comunidade do
...
- [ ] 1.3 Publicar `GET /v1/rankings/{comunidade}` em `pontuacao/rotas.py` — módulo novo,
...
- [ ] 1.4 Escrever em `recompensas_de_marco/regra.py` a consulta das **recompensas
...
- [ ] 1.5 Publicar `GET /v1/eu/recompensas` em `recompensas_de_marco/rotas.py`, restrita ao
      Guerreiro(a) em sessão, com 403 para os demais papéis (`RF-05-45`, `RN-05-21`).
- [ ] 1.6 Acrescentar a `EuSaida`, em `sessoes/rotas.py`, o estado da **autorização de
...
- [ ] 2.1 Criar o cliente das rotas do bloco em `apps/app-05-guerreiro/src/api/`, no molde de
...
- [ ] 2.2 Abrir a área do bloco em `AreaDoGuerreiro.tsx`, ao lado da coleta, com as quatro
      telas — carteira, catálogo, conquistas e ranking —, lendo à entrada e **sem sondagem
      periódica** (design — decisão 6).
- [ ] 2.3 Tela da **carteira**: acumulado e saldo disponível separados e rotulados, sem soma e
      sem ponto regular, com a frase de que o acumulado só cresce e o trocável é o saldo
      (`RF-05-82`, `RN-05-39`, `RN-05-40`, `RN-05-42`).
- [ ] 2.4 Tela do **catálogo avulso**: itens ativos da comunidade com preço em pontos extras e
      ...
      `RF-05-87`).
- [ ] 2.5 **Histórico de trocas** na mesma tela do catálogo: item, preço cobrado na época e
      data, sem moedas e sem reais (`RF-05-88`).
- [ ] 2.6 Tela das **conquistas**: recompensa de marco conquistada, dizendo que o Mestre
      confirma a entrega, e a entregue com a data; nenhum caminho de compra em nenhuma tela do
      bloco (`RF-05-45`, `RF-05-46`, `RN-05-41`).
- [ ] 2.7 **Estado do perfil público** dentro da carteira: se a divulgação foi autorizada, em
...
- [ ] 2.8 Tela do **ranking**: turma inteira da comunidade, recorte por trilha ou poder, só
...
- [ ] 3.1 `tests/test_ranking_da_turma.py`: turma inteira contra ranking público filtrado,
...
- [ ] 3.2 Estender `tests/test_vitrine.py` com a garantia de que a extração da tarefa 1.1 não
...
- [ ] 3.3 `tests/test_recompensa_conquistada.py`: marco alcançado aparece, não alcançado não
      ...
      `recompensa-de-marco`.
- [ ] 3.4 Estender `tests/test_autorizacao_vigente.py` com o estado da divulgação em
...
- [ ] 3.5 Testes das telas em `apps/app-05-guerreiro/src/`, um arquivo por tela, cobrindo os
...
- [ ] 4.1 Corrigir no PRD-05 §9 a rota do ranking, que a §9 declara **pública** contra o
