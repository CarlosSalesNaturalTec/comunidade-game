...
- [ ] 1. Criar `backend/src/nucleo/culminancias/modelo.py` com `Culminancia` — `trilha_id`,
...
- [ ] 2. Acrescentar `despublicada` a `SituacaoDaTrilha` e as quatro colunas de procedência à
      ...
      `RF-09-11`, design 3)
- [ ] 3. Gerar a migração Alembic das duas tarefas acima: tabela nova e alteração da restrição
      de verificação da situação, sem retroalimentar trilha existente. (design 3)

## Núcleo — regra

- [ ] 4. Escrever `culminancias/regra.py`: declarar a culminância com posse do Mestre autor
...
- [ ] 5. Acrescentar a `trilhas/regra.py` a conferência das três travas — sondagem declarada,
...
- [ ] 6. Acrescentar a `trilhas/regra.py` a publicação pelo Mestre autor a partir de
...
- [ ] 7. Acrescentar a `trilhas/regra.py` a despublicação privativa de Admin, com motivo
...
- [ ] 8. Criar `culminancias/rotas.py` com `POST /v1/trilhas/{id}/culminancia` e registrar o
      roteador em `principal.py`. (`RF-09-29`, `RF-09-30`)
- [ ] 9. Acrescentar a `trilhas/rotas.py` `POST /v1/trilhas/{id}/publicacao`,
...
- [ ] 10. Fazer `GET /v1/trilhas/minhas` devolver o motivo da despublicação junto da situação,
      que é como o Mestre autor o lê. (`RF-09-10`)

## Testes do núcleo

- [ ] 11. `backend/tests/test_culminancia.py` — cenários da spec `culminancia`: declaração
      ...
      (`RF-09-29`, `RF-09-30`)
- [ ] 12. Acrescentar a `backend/tests/test_trilha.py` os cenários de situação e travas da
      ...
      `RF-09-82`, `RN-09-27`)
- [ ] 13. Acrescentar a `backend/tests/test_trilha_rota.py` os cenários de porta: publicação
...
- [ ] 14. Criar a tela da culminância dentro de `apps/app-09-mestre/src/trilhas/` —
...
- [ ] 15. Acrescentar a ação de publicar em `TelaDaTrilha.tsx`, com a recusa apresentada em
      linguagem simples nomeando todas as travas que faltam, sem código de erro nem jargão.
      (`RF-09-05`, `RF-09-08`, `RF-09-12`, `RF-09-82`)
- [ ] 16. Apresentar em `ListaDeTrilhas.tsx` as três situações e, na despublicada, o motivo
      registrado pelo Admin; não oferecer a publicação em trilha de outro Mestre. (`RF-09-04`,
      `RF-09-10`)
- [ ] 17. Acrescentar a `apps/app-09-mestre/src/trilhas/trilhas.test.tsx` os cenários da spec
...
- [ ] 18. Gravar as decisões novas e o que elas mudam: as duas decisões de 2026-08-22 no
