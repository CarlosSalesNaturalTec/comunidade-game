...
- [x] 1.1 Modelo e migração Alembic: `equipe.nome` (`String(20)`, `NOT NULL`), preenchimento
...
- [x] 1.2 `equipes/regra.py`: validação do nome (aparado, 1 a 20, `campo="nome"`) e da
...
- [x] 1.3 `equipes/rotas.py`: `nome` nas entradas de `POST /v1/aulas/{id}/equipes` e
...
- [x] 1.4 `painel_do_dia/regra.py`: `nome` em `EquipeDoPainelSaida` e integrantes como
      `IntegranteDoPainelSaida` com `papel` — `RF-02-08`, design 5. Verificar: testes da 4.3.
- [x] 1.5 Os testes que criam equipe passam a dar um nome, único por aula ou trilha dentro de
      cada teste — sem mudar o que cada teste confere. Na prática bastou a fixture
      `criar_equipe` do conftest ("Equipe N" por contador, ou o nome declarado), as chamadas
      diretas à regra em `test_equipe.py` e o corpo das criações em `test_equipe_rota.py` e
      `test_equipe_da_trilha_rota.py`. Verificar: a suíte do backend volta verde.

## 2. App 01 — nome ao criar e renomear

- [x] 2.1 `comum/react/Campo.tsx`: `maxLength` opcional repassado ao `<input>`, sem mudar quem
      não o usa — design 7. Verificar: teste do componente com e sem o limite.
- [x] 2.2 `src/api/equipes.ts`: `nome` em `Equipe`, `criarEquipe` e `criarEquipeDaTrilha`
      recebem `nome`, e nova `renomearEquipe(equipeId, nome, token)` —
      `RF-04-69`, `RF-04-70`. Verificar: testes da 4.4 e 4.5.
- [x] 2.3 `src/equipes/TelaDeEquipes.tsx`: campo "Nome da equipe" (máximo 20) com "Criar
...
- [x] 2.4 `src/trilhas/EquipeDaTrilha.tsx`: o mesmo campo na criação, a equipe da trilha pelo
      nome e "Trocar o nome" só antes da homologação — `RF-04-69`, `RF-04-70`, design 7.
      Verificar: testes da 4.5.

## 3. App 03 — painel do dia

- [x] 3.1 `src/painel-do-dia/api.ts` e `TelaDoPainelDoDia.tsx`: `nome` na equipe e `papel` no
...
- [x] 4.1 `backend/tests/test_equipe.py`: criar com nome, sem nome, só espaços, 21 caracteres,
      ...
      `equipe`.
- [x] 4.2 `backend/tests/test_equipe_rota.py` e `test_equipe_da_trilha_rota.py`: 201 com o nome
...
- [x] 4.3 `backend/tests/test_painel_do_dia.py`: equipe pelo nome, integrante com e sem papel,
      nome novo na consulta seguinte à troca — spec `painel-do-dia`.
- [x] 4.4 `apps/app-01-aula-presencial/src/equipes/equipes.test.tsx`: criar sem nome não envia,
...
- [x] 4.5 `apps/app-01-aula-presencial/src/trilhas/trilhas.test.tsx`: criação da equipe da
...
- [x] 4.6 `apps/app-03-gestao/src/painel-do-dia/painel-do-dia.test.tsx`: equipe pelo nome,
      integrante com papel e só pelo nick sem papel, nenhum caminho para renomear — spec
      `aplicacao-de-gestao`.

## 5. Documentação

- [x] 5.1 Gravar as duas regras decididas na elicitação (comparação sem caixa e sem espaços;
