...
- [ ] 1.1 Em `comum/tokens.css`, declarar `[data-temperamento="arena"]` com raio de carta de `12`
...
- [ ] 1.2 Em `comum/tokens.test.ts`, cobrir os cenários "A Arena tem camada de tema própria", "As
...
- [ ] 2.1 Em `apps/app-01-aula-presencial/index.html` e `apps/app-05-guerreiro/index.html`, trocar
      a declaração para o temperamento Arena (design — decisão 6, invariante 24).
- [ ] 2.2 Conferir que as quatro aplicações da Operação seguem declarando Operação, e que nenhuma
...
- [ ] 3.1 Em `comum/react`, criar o componente de ícone do documento 15 §11.1 — grade de `24` px,
...
- [ ] 3.2 Desenhar os seis glifos da tabela da `proposal` — onboarding, presença, equipes, quiz,
      medição do limiar e troca —, cada um sem cor declarada no arquivo.
- [ ] 3.3 Exportar o componente e os glifos em `comum/react/indice.ts`, acrescentando o que
      precisar em `comum/package.json`.
- [ ] 3.4 Cobrir, em teste do `comum`, os três cenários do requisito do ícone: nenhum ícone vem de
      fora, o ícone acompanha o tema e o ícone não aparece sozinho.

## 4. Os caminhos da tela inicial da App 01

- [ ] 4.1 Em `apps/app-01-aula-presencial/src/inicio/TelaInicial.tsx`, apresentar o glifo de cada
...
- [ ] 4.2 Em `apps/app-01-aula-presencial/src/index.css`, acomodar o glifo no `.cg-caminho` sem
      mexer no raio nem no alvo de toque de `48` px (design — decisão 5).
- [ ] 4.3 Em `apps/app-01-aula-presencial/src/inicio/inicio.test.tsx`, cobrir o cenário "Cada
...
- [ ] 5.2 Registrar no documento 09 §1 a pendência do **valor de densidade do temperamento Arena**,
      que o documento 15 §6 descreve sem número (design — decisão 2). Nada muda em
