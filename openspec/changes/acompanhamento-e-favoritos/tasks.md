...
- [x] 1.1 Criar `backend/src/nucleo/favoritos/` (`__init__.py`, `modelo.py`, `regra.py`,
...
- [x] 1.2 Escrever a migração Alembic aditiva de `favorito`, com o check e os dois índices
...
- [x] 2.1 Em `regra.py`, `favoritar()`: alvo Guerreiro(a) resolvido por
...
- [x] 2.2 Em `regra.py`, `remover_favorito()` apagando a linha por `(id, apoiador_id)` em uma
...
- [x] 2.3 Em `regra.py`, `montar_novidades()` derivando, por alvo e com janela de 30 dias a
...
- [x] 3.1 Em `rotas.py`, expor `GET`, `POST` e `DELETE` em `/v1/eu/favoritos`, restritas ao
...
- [x] 3.2 Cobrir a criação e a remoção em `backend/tests/test_favoritos.py`: nick exato de quem
...
- [x] 3.3 Cobrir a leitura em `backend/tests/test_favoritos.py`: os quatro fatos aparecem com a
...
- [x] 4.1 Criar `apps/app-08-apoiador/src/acompanhamento/api.ts` com o cliente das três rotas
...
- [x] 4.2 Criar `apps/app-08-apoiador/src/acompanhamento/TelaDeAcompanhamento.tsx` com o bloco
      do painel público, alimentado só pelas rotas públicas e sem recorte adicional
      (`RF-14-48`).
- [x] 4.3 No mesmo arquivo, o bloco dos favoritos: campo de nick exato declarando que o nick
...
- [x] 4.4 Ligar a área "Acompanhamento" na navegação de `apps/app-08-apoiador/src/App.tsx`, no
      padrão das demais áreas (`RF-14-48`, design — decisão 10).
- [x] 4.5 Cobrir a área em `apps/app-08-apoiador/src/acompanhamento/acompanhamento.test.tsx`: o
...
- [x] 5.1 Marcar a fatia 7 do PRD-14 como `implementado` em
