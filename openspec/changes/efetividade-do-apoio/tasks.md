...
- [ ] 1.1 Criar `ConclusaoDeDesafioExtra` em `backend/src/nucleo/desafios_extras/modelo.py` —
...
- [ ] 1.2 Escrever a migração Alembic aditiva da tabela nova, com o índice único e os gatilhos
...
- [ ] 1.3 Acrescentar `registrar_conclusao_de_desafio_extra()` a
...
- [ ] 1.4 Cobrir a entidade e a regra em `backend/tests/test_conclusao_de_desafio_extra.py`:
...
- [ ] 2.1 Criar o módulo `backend/src/nucleo/efetividade_do_apoio/` (`__init__.py`, `regra.py`,
...
- [ ] 2.2 Em `regra.py`, montar os desafios do proponente separados entre propostos, publicados
      e concluídos, com a contagem de conclusões, a trilha e o período — data da primeira e da
      última conclusão (`RF-14-41`, `RF-14-42`).
- [ ] 2.3 Em `regra.py`, resolver os concluintes exibíveis filtrando pela expressão de
      ...
      7).
- [ ] 2.4 Em `regra.py`, montar as moedas aportadas com o que custearam — necessidade ou
...
- [ ] 2.5 Em `regra.py`, montar a cobertura de ODS nos dois níveis: as etiquetas herdadas por
...
- [ ] 2.6 Em `rotas.py`, expor `GET /v1/eu/desafios-extras/efetividade`, restrita ao Apoiador em
...
- [ ] 2.7 Cobrir a rota e a regra em `backend/tests/test_efetividade_do_apoio.py`: painel vivo
...
- [ ] 3.1 Criar `apps/app-08-apoiador/src/efetividade/api.ts` com o cliente de
      `GET /v1/eu/desafios-extras/efetividade`, no padrão dos demais `api.ts` da aplicação
      (`RF-14-40`).
- [ ] 3.2 Criar `apps/app-08-apoiador/src/efetividade/TelaDeEfetividade.tsx` com os desafios por
...
- [ ] 3.3 Exibir os concluintes só por avatar e nick, o restante apenas como contagem, e o
...
- [ ] 3.4 Ligar a área "Efetividade" na navegação de `apps/app-08-apoiador/src/App.tsx`, no
      padrão das demais áreas (`RF-14-40`).
- [ ] 3.5 Cobrir a tela em `apps/app-08-apoiador/src/efetividade/efetividade.test.tsx`: a área
...
- [ ] 4.1 Marcar a fatia 6 do PRD-14 como `implementado` em
