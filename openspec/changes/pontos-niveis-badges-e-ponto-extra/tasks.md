...
- [x] 1.1 Criar o modelo `Resultado` com Guerreiro(a), atividade, data do fato, produção e
...
- [x] 1.2 Criar o modelo `PontoRegular` por Guerreiro(a) e trilha, e o modelo `Nivel` certificado
      por Guerreiro(a), trilha e valor (1, 2 ou 4), com data de certificação (`RF-01-21`, 11 §6,
      design — decisões).
- [x] 1.3 Criar o modelo `Badge` certificado por Guerreiro(a), trilha ou poder e tipo — `de_nivel`
      ou `de_valores_e_causas` —, com data de certificação (`RF-01-21`, 11 §7).
- [x] 1.4 Criar o modelo `PontoExtra` por Guerreiro(a), com `acumulado` e `saldo_disponivel`
      (`RF-01-56`, 11 §5, design — decisões).
- [x] 1.5 Escrever a sexta migração Alembic criando `resultado`, `ponto_regular`, `nivel`,
      `badge` e `ponto_extra`, sem tocar as tabelas das fatias anteriores, e conferir que ela
      sobe e desce.

## 2. Resultado

- [x] 2.1 Implementar o registro de Resultado exigindo Guerreiro(a), atividade, data do fato e
      produção, recusando com 422 o que faltar (`RF-01-20`, 11 §§2.2, 4).
- [x] 2.2 Implementar o lançamento do desfecho restrito ao Mestre autor da trilha da atividade e
...
- [x] 2.3 Verificar: Resultado sem atividade, sem Guerreiro(a) ou sem produção recebe 422;
...
- [x] 3.1 Implementar a regra de crédito: valor-base 20 se a modalidade da atividade for
...
- [x] 3.2 Implementar a recusa de qualquer operação de débito sobre `PontoRegular` (`RN-01-38`).
- [x] 3.3 Verificar: Resultado "realizada" credita o valor-base pela modalidade; "realizada com
      mérito" credita o valor-base mais 5; nenhuma via debita o ponto regular (`RF-01-21`,
      `RN-01-38`).

## 4. Nível

- [x] 4.1 Implementar a certificação do nível 1 — inscrito na trilha e primeira atividade com
      Resultado — por trilha (`RF-01-21`, 11 §6).
- [x] 4.2 Implementar a certificação do nível 2 — um terço das missões obrigatórias com Resultado
      — por trilha (`RF-01-21`, 11 §6).
- [x] 4.3 Implementar a certificação do nível 4 — todas as obrigatórias com Resultado e ao menos
      um Resultado `merito_extra_por_auxilio` — por trilha (`RF-01-21`, 11 §6).
- [x] 4.4 Implementar a não regressão: nível certificado nunca é removido, mesmo que o critério
      deixe de valer depois (`RF-01-21`, 11 §6, design — decisões).
- [x] 4.5 Verificar: primeira atividade certifica nível 1; um terço das obrigatórias certifica
...
- [x] 5.1 Implementar a concessão do badge `de_nivel` a cada nível certificado (`RF-01-21`,
      11 §7).
- [x] 5.2 Implementar a concessão do badge `de_valores_e_causas` a Resultado de atividade de
      natureza "valores e temas transversais" (`RF-01-21`, 11 §7).
- [x] 5.3 Verificar: certificar um nível concede o badge de nível correspondente; Resultado de
...
- [x] 6.1 Implementar o crédito simultâneo de `acumulado` e `saldo_disponivel` a partir de
...
- [x] 6.2 Implementar a recusa de qualquer operação que reduza o `acumulado` ou que deixe o
      `saldo_disponivel` negativo (`RN-01-39`, `RN-01-40`).
- [x] 6.3 Verificar: "realizada com mérito" credita 5 nas duas contas de extra; "mérito extra por
...
- [x] 8.1 Conferir que nenhuma decisão nova desta fatia ficou fora de `docs/` — a régua de
