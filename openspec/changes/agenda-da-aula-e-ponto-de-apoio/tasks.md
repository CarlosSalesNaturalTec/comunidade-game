...
- [ ] 1.1 `GET /pontos-de-apoio` em `pontos_de_apoio/rotas.py`, com a regra de listagem em
...
- [ ] 1.2 `GET /aulas` em `aulas/rotas.py`, com a regra em `aulas/regra.py`: mesmo contrato de
...
- [ ] 1.3 `GET /aulas/vigentes` em `aulas/rotas.py`, consumindo `aulas_vigentes` **sem
...
- [ ] 2.1 Em `backend/tests/test_ponto_de_apoio.py`, os cenários da leitura: Admin lê filtrado
...
- [ ] 2.2 Em `backend/tests/test_aula.py`, os cenários da agenda: Admin lê, Mestre lê só as
      ...
      `RF-01-28`, `RF-01-18`).
- [ ] 2.3 Ainda em `backend/tests/test_aula.py`, os cenários das vigentes pela rota: aplicação
...
- [ ] 3.1 Campo de data e hora com fuso em `comum/react/`, exportado pelo `indice.ts`, com o
...
- [ ] 3.2 Teste do campo em `comum/react/`: valor devolvido carrega fuso, valor inválido
      aponta o erro no próprio campo, e o campo é operável pelo teclado.

## 4. Ponto de apoio na App 03

- [ ] 4.1 `apps/app-03-gestao/src/pontos-de-apoio/`: cliente de API (lista e cadastro), tela
...
- [ ] 5.1 `apps/app-03-gestao/src/agenda/`: cliente de API (agenda, agendamento e
      ...
      `RF-01-18`, `RN-02-09`).
- [ ] 5.2 Formulário de agendamento: comunidade, data, horário inicial, horário final e ponto
...
- [ ] 5.3 Cancelamento com motivo obrigatório, alcançável pelo Admin e pelo Mestre da
...
- [ ] 6.1 `apps/app-03-gestao/src/pontos-de-apoio/pontos-de-apoio.test.tsx`: cadastro
      ...
      `RF-07-49`).
- [ ] 6.2 `apps/app-03-gestao/src/agenda/agenda.test.tsx`: agendamento completo com a aula
...
- [ ] 7.1 `docs/09-topicos-em-aberto-e-sugestoes.md` §1 recebe a pendência da **modalidade da
