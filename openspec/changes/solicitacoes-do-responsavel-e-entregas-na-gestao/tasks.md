...
- [ ] 1.1 Criar `backend/src/nucleo/solicitacoes_do_responsavel/modelo.py` com
...
- [ ] 1.2 Criar `backend/src/nucleo/solicitacoes_do_responsavel/regra.py` com
...
- [ ] 1.3 Criar os erros do módulo em `backend/src/nucleo/erros.py` no padrão dos demais:
      solicitação idêntica em aberto (409) e solicitação já tratada (`RF-13-22`, `RF-02-24`).
- [ ] 1.4 Criar `backend/src/nucleo/solicitacoes_do_responsavel/rotas.py` com
...
- [ ] 1.5 Registrar o roteador em `backend/src/nucleo/principal.py` e criar a migração Alembic da
      tabela `solicitacao_do_responsavel`, encadeada na última revisão (`RF-13-24`).

## 2. Núcleo — a saída da entrega

- [ ] 2.1 Em `backend/src/nucleo/recompensas_de_marco/rotas.py`, acrescentar a
...
- [ ] 3.1 Em `apps/app-03-gestao/src/filas/api.ts`, acrescentar a leitura da fila
...
- [ ] 3.2 Em `ListaDeFilas.tsx` e `TelaDeFilas.tsx`, acrescentar a fila das solicitações do
...
- [ ] 3.3 Criar `apps/app-03-gestao/src/filas/TratamentoDaSolicitacaoDoResponsavel.tsx` no molde
      ...
      tarefa 4.4.
- [ ] 3.4 Em `apps/app-03-gestao/src/acervo/api.ts`, acrescentar a leitura das entregas
      (`GET /v1/entregas`) com os campos novos (`RF-02-50`, `RF-02-51`).
- [ ] 3.5 Criar `apps/app-03-gestao/src/acervo/ListaDeEntregas.tsx` e ligá-la a `TelaDoAcervo`:
...
- [ ] 4.1 Criar `backend/tests/test_solicitacao_do_responsavel.py` cobrindo a regra: abertura nos
      ...
      `RN-13-22`).
- [ ] 4.2 Criar `backend/tests/test_solicitacao_do_responsavel_rota.py` cobrindo as quatro rotas:
...
- [ ] 4.3 Em `backend/tests/test_entrega_de_recompensa.py`, cobrir a saída ampliada: tipo de
...
- [ ] 4.4 Em `apps/app-03-gestao/src/filas/filas.test.tsx`, cobrir a fila nova: protocolo, tipo,
      ...
      `RF-02-24`, `RF-02-66`).
- [ ] 4.5 Em `apps/app-03-gestao/src/acervo/acervo.test.tsx`, cobrir a lista de entregas:
...
- [ ] 5.1 Em `openspec/cronograma-de-fatias.md`: marcar a fatia 14 do PRD-02 e a linha
      **Entregas confirmadas** como implementadas, com o slug desta change; reescrever o recorte
      ...
      `RN-13-22`).
- [ ] 5.2 Em `docs/`: registrar no documento 09 §1, como decisão do fundador de 2026-08-29, a
