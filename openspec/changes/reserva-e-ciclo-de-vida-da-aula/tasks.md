...
- [ ] 1.1 Criar `backend/src/nucleo/reservas/modelo.py` com `EstadoDaReserva` (reservada,
...
- [ ] 1.2 Criar `RecursoDeclaradoDaAula` em `backend/src/nucleo/aulas/modelo.py` — aula, tipo de
...
- [ ] 1.3 Acrescentar `SituacaoDaAula` (prevista, pendente de lastro, confirmada, realizada,
      cancelada) e a coluna `situacao` a `Aula`, com o motivo do cancelamento (`RF-01-72`,
      `RF-07-09`, PRD-01 §8, design — Decisions 9)
- [ ] 1.4 Acrescentar `aula_id` obrigatório a `Resultado` em `resultados/modelo.py`
      (`RF-07-09`, `RF-02-35`, design — Decisions 8)
- [ ] 1.5 Gerar a migration Alembic com as duas tabelas novas, o índice composto
      `(tipo_de_recurso_id, ponto_de_apoio_id, estado)`, as colunas de `aula` e o `aula_id` de
      `resultado` (design — Migration Plan)

## 2. Saldo disponível

- [ ] 2.1 Escrever `quantidade_reservada(sessao, tipo, ponto_de_apoio)` em
      `reservas/regra.py`, por agregação sobre as reservas no estado reservada, sem tabela de
      saldo (`RF-07-07`, design — Decisions 2)
- [ ] 2.2 Escrever `disponivel_de(sessao, tipo, ponto_de_apoio)` — o saldo derivado dos
      lançamentos menos o reservado (`RF-07-07`, `RF-07-08`, design — Decisions 2)
- [ ] 2.3 Verificar que a reserva não altera o saldo derivado e que a disponível cai por ela
      (`RF-07-07`, spec `reserva-de-recurso` e `livro-razao`)

## 3. Reserva no agendamento

- [ ] 3.1 Estender `agendar_aula(...)` em `aulas/regra.py` para receber os recursos declarados,
      recusando quantidade menor ou igual a zero e tipo inexistente com 422 (`RF-07-08`,
      `RF-01-27`)
- [ ] 3.2 Avaliar a disponível de cada tipo declarado dentro da transação, com
      `SELECT ... FOR UPDATE` sobre o par tipo/ponto de apoio (`RF-07-08`, design — Decisions 4)
- [ ] 3.3 Havendo disponível para todos, gravar uma `Reserva` por recurso e a aula como
...
- [ ] 3.4 Aula sem recurso declarado nasce **confirmada**, sem reserva (`RF-07-08`, spec
      `aula-e-presenca`)
- [ ] 3.5 Verificar os três desfechos do agendamento e a ausência de reserva parcial
      (`RF-07-08`, `RN-07-01`, PRD-07 §12)

## 4. Confirmação pelo aporte

- [ ] 4.1 Escrever `confirmar_aulas_pendentes(sessao, tipo, ponto_de_apoio, operador)` em
...
- [ ] 4.2 Chamar a confirmação em `aportes/regra.py` logo depois de `lancar_credito`, nas três
      formas que creditam — registro da gestão, absorção e homologação do pré-cadastro
      (`RN-07-37`, `RN-07-35`, design — Decisions 5)
- [ ] 4.3 Verificar que aporte que fecha a falta confirma a aula, que aporte insuficiente não
...
- [ ] 5.1 Escrever `lancar_debito(...)` em `livro_razao/regra.py`, no ponto de apoio da aula que
      consumiu (`RF-07-09`, `RN-07-36`)
- [ ] 5.2 Escrever `lancar_atividade_realizada(sessao, aula, resultados, operador)` em
...
- [ ] 5.3 Recusar com 422 o lançamento em aula já realizada ou já cancelada (`RF-07-09`,
      spec `aula-e-presenca`)
- [ ] 5.4 Verificar que a baixa gera um débito por reserva, que o saldo cai pelo débito e que
      aula sem reserva apenas registra os resultados (`RF-07-09`, PRD-07 §12)

## 6. Cancelamento e liberação

- [ ] 6.1 Escrever `cancelar_aula(sessao, aula, operador, motivo)` em `aulas/regra.py`: aceita
...
- [ ] 6.2 Levar as reservas da aula a **liberada**, devolvendo as quantidades à disponível
      (`RF-07-09`, spec `reserva-de-recurso`)
- [ ] 6.3 Recusar com 422 o cancelamento de aula já realizada ou já cancelada (`RF-01-72`,
      spec `aula-e-presenca`)
- [ ] 6.4 Verificar que a reserva **não expira**: aula cuja data passou sem lançamento nem
      cancelamento mantém as reservas no estado reservada (`RF-07-09`, PRD-07 §5.3)

## 7. Rotas

- [ ] 7.1 Criar `backend/src/nucleo/aulas/rotas.py` com `POST /aulas`, de Admin, que agenda e
      dispara a reserva, devolvendo a situação e o que falta (`RF-07-08`, `RF-02-31`,
      `RF-01-16`)
- [ ] 7.2 Acrescentar `POST /aulas/{id}/reservas`, de gestão, idempotente: tenta a reserva de
      aula pendente de lastro e devolve o estado corrente quando já confirmada (PRD-07 §9,
      design — Decisions 6)
- [ ] 7.3 Acrescentar `POST /aulas/{id}/lancamentos`, de Admin, com os resultados dos
      participantes (`RF-07-09`, `RF-02-35`)
- [ ] 7.4 Acrescentar `POST /aulas/{id}/cancelamento`, de Admin ou Mestre da comunidade, com
      motivo (`RF-01-72`, `RF-02-95`)
- [ ] 7.5 Registrar o roteador em `principal.py`, sob `/v1` e a chave de aplicação (`RF-01-02`)
- [ ] 7.6 Verificar a matriz de permissões das quatro rotas e o formato único de erro
      (`RF-01-16`, `RF-01-27`)

## 8. Esteira e documentação

- [ ] 8.1 Ajustar os testes existentes que criam `Resultado` sem aula (`test_resultado.py` e os
...
- [ ] 8.3 Atualizar a narrativa da fatia entregue em `docs/prds/index.md` — o PRD-07 segue **em
