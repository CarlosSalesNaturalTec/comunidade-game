...
- [x] 1.1 Criar `PrecoDeReferencia` em `backend/src/nucleo/recursos/modelo.py`, ao lado de
      `ValorDeReferencia`: tipo de recurso, preço inteiro, vigência inicial e final, autoria
      (`RF-07-42`, `RF-07-43`)
- [x] 1.2 Escrever a regra de vigência em `recursos/regra.py` — abrir vigência encerra a anterior
...
- [x] 1.3 Aplicar o piso de 20 pontos extras e a recusa de preço fracionário, negativo ou zero,
      com 422 indicando o piso (`RF-07-44`, `RN-07-30`)
- [x] 1.4 Expor `POST` e `GET /tipos-de-recurso/{id}/precos-de-referencia` em `recursos/rotas.py`,
      exigindo Admin em sessão e devolvendo 403 às demais personas (`RF-07-42`, `RF-01-16`)
- [x] 1.5 Garantir que registrar preço em pontos não toca o valor em moedas e que nenhuma resposta
      traz equivalência entre as réguas (`RF-07-38`, `RN-07-24`, `RN-07-25`)

## 2. Item do catálogo avulso

- [x] 2.1 Criar o módulo `backend/src/nucleo/catalogo_avulso/` com `ItemDeCatalogoAvulso`: nome,
...
- [x] 2.2 Escrever o cadastro em `catalogo_avulso/regra.py`: Mestre ou Apoiador em sessão, Mestre
      ...
      `RN-07-29`, `RN-07-33`)
- [x] 2.3 Implementar a homologação: item de Mestre entra sem homologação, item de Apoiador nasce
      pendente e inativo, e o Admin homologa ou recusa com motivo (`RF-09-100`, `RN-14-42`,
      `RN-07-26`)
- [x] 2.4 Implementar o lastro — quantidade disponível do tipo no ponto de apoio do item igual ou
...
- [x] 2.5 Implementar a ativação, que reverifica o lastro e recusa item pendente de homologação ou
      recusado (`RF-07-34`, `RN-09-37`)
- [x] 2.6 Implementar a alteração de estoque e a retirada — ambas de Admin ou Mestre vinculado,
      com autoria; estoque acima do lastro desativa o item, e a retirada preserva o registro
      (`RF-07-33`, `RF-09-102`)
- [x] 2.7 Implementar a leitura do catálogo por comunidade, com o preço da vigência corrente, o
      ...
      `RF-01-24`)
- [x] 2.8 Expor as rotas de `catalogo_avulso/rotas.py` conforme a tabela do `design.md` e
      registrá-las na aplicação, sob `/v1` e com chave de aplicação (`RF-07-33`, `RF-01-03`)

## 3. Persistência

- [x] 3.1 Escrever a migração Alembic com `preco_de_referencia` e `item_de_catalogo_avulso`, sem
      alterar tabela existente, e conferir que a reversão derruba as duas

## 4. Testes

- [x] 4.1 `tests/test_preco_de_referencia.py`: primeira vigência, vigência nova preservando a
      ...
      `RF-07-44`, `RN-07-30`)
- [x] 4.2 `tests/test_preco_de_referencia.py`: as duas réguas convivem sem se tocar — registrar
...
- [x] 4.3 `tests/test_catalogo_avulso.py`: cadastro por Mestre e por Apoiador, ponto de apoio de
...
- [x] 4.4 `tests/test_catalogo_avulso.py`: homologação — item de Mestre dispensa, item de Apoiador
...
- [x] 4.5 `tests/test_catalogo_avulso.py`: lastro — item com saldo igual ao estoque nasce ativo,
...
- [x] 4.6 `tests/test_catalogo_avulso.py`: manutenção e leitura — estoque alterado dentro e acima
...
- [x] 5.1 Gravar as três decisões novas nos documentos-fonte: a janela de troca como garantia da
...
- [x] 5.2 Mover as três para "Já decididos" no documento 09 §1, cada uma com o documento-fonte
- [x] 5.3 Aplicar as decisões nos PRDs: `ItemDeCatalogoAvulso` ganha o ponto de apoio no PRD-07
      §8, as decisões entram na tabela do PRD-07 §13, e o PRD-04 registra que o `RF-04-49` é
      garantia da aplicação
- [x] 5.4 Atualizar `docs/prds/index.md` com a sétima fatia do PRD-07 e conferir que nada mudou na
