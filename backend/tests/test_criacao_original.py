...
def test_entrega_com_equipe_e_producao_grava_o_registro(
    ...
    criacao = entregar_criacao_original(
        sessao, guerreiro=guerreiro, equipe=equipe, producao="Meu robô de sucata."
    )
    sessao.commit()
...
def test_entrega_sem_equipe_e_recusada(sessao, criar_persona):
    guerreiro = criar_persona(Papel.guerreiro)

    with pytest.raises(ErroDeValidacao) as excinfo:
        entregar_criacao_original(
            sessao, guerreiro=guerreiro, equipe=None, producao="Meu robô de sucata."
        )
    assert excinfo.value.campo == "equipe_id"
    assert sessao.query(CriacaoOriginal).count() == 0


def test_entrega_com_equipe_da_aula_e_recusada(
    ...
    equipe_da_aula = criar_equipe(guerreiro, aula=aula)

    with pytest.raises(ErroDeValidacao) as excinfo:
        entregar_criacao_original(
            sessao, guerreiro=guerreiro, equipe=equipe_da_aula, producao="Produção."
        )
    assert excinfo.value.campo == "equipe_id"
    assert sessao.query(CriacaoOriginal).count() == 0


def test_entrega_sem_producao_e_recusada(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    guerreiro = criar_persona(Papel.guerreiro)
    trilha = criar_trilha(mestre)
    equipe = criar_equipe(guerreiro, trilha=trilha)
    ...
    with pytest.raises(ErroDeValidacao) as excinfo:
        entregar_criacao_original(sessao, guerreiro=guerreiro, equipe=equipe, producao=None)
    assert excinfo.value.campo == "producao"
    assert sessao.query(CriacaoOriginal).count() == 0


def test_quem_nao_integra_a_equipe_nao_entrega(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    ...
    with pytest.raises(PermissaoNegada):
        entregar_criacao_original(
            sessao, guerreiro=guerreiro_de_fora, equipe=equipe, producao="Produção."
        )
    assert sessao.query(CriacaoOriginal).count() == 0


def test_segunda_entrega_da_mesma_equipe_e_recusada(
    sessao, criar_persona, criar_trilha, criar_equipe
):
    mestre = criar_persona(Papel.mestre)
    guerreiro = criar_persona(Papel.guerreiro)
    trilha = criar_trilha(mestre)
    equipe = criar_equipe(guerreiro, trilha=trilha)
    entregar_criacao_original(sessao, guerreiro=guerreiro, equipe=equipe, producao="Produção.")
    sessao.commit()

    with pytest.raises(ErroDeValidacao) as excinfo:
        entregar_criacao_original(
            sessao, guerreiro=guerreiro, equipe=equipe, producao="Outra produção."
        )
    assert excinfo.value.campo == "equipe_id"
    assert sessao.query(CriacaoOriginal).count() == 1


def test_qualquer_integrante_entrega_pela_equipe(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    ...
    criacao = entregar_criacao_original(
        sessao, guerreiro=colega, equipe=equipe, producao="Produção da equipe."
    )
    sessao.commit()
...
def test_mestre_autor_valida_a_entrega(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    guerreiro = criar_persona(Papel.guerreiro)
    trilha = criar_trilha(mestre)
    equipe = criar_equipe(guerreiro, trilha=trilha)
...
def test_mestre_autor_devolve_a_entrega(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    guerreiro = criar_persona(Papel.guerreiro)
    trilha = criar_trilha(mestre)
    equipe = criar_equipe(guerreiro, trilha=trilha)
...
def test_admin_valida_mesmo_sem_ser_o_autor(sessao, criar_persona, criar_trilha, criar_equipe):
...
def test_validar_criacao_ja_validada_e_recusado(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    guerreiro = criar_persona(Papel.guerreiro)
    trilha = criar_trilha(mestre)
    equipe = criar_equipe(guerreiro, trilha=trilha)
...
def test_devolucao_preserva_a_autoria_original(sessao, criar_persona, criar_trilha, criar_equipe):
    mestre = criar_persona(Papel.mestre)
    guerreiro = criar_persona(Papel.guerreiro)
    trilha = criar_trilha(mestre)
    equipe = criar_equipe(guerreiro, trilha=trilha)
